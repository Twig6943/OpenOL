	...
	,	PreventCapture(FALSE)
{
	...
	if (!GOrbisController)
	{
		GOrbisController = new FPCOrbisController();
	}

	OrbisController = GOrbisController;
#endif
}
...
#if WITH_ORBISCONTROLLEREMULATION
	//delete OrbisController;	
	OrbisController = NULL;
#endif
}
...
{
	// ImGui manages the cursor itself via ImGui_ImplWin32_NewFrame — don't interfere.
	if ( OLImGui_IsOverlayVisible() )
		return TRUE;
		...
		for(INT JoystickIndex = 0;JoystickIndex < UWindowsClient::Joysticks.Num();JoystickIndex++)
		{
			...
			if ( bIsConnected )
			{
				...
				else if( JoystickInfo.JoystickType == JOYSTICK_PS2_Old_Converter || JoystickInfo.JoystickType == JOYSTICK_PS2_New_Converter )
				{
					...
					{
						// rcharpentier - negated leftY to make sense with the logitech f710 directinput mode (to be tested with other ps2 controllers)
...
#if WITH_ORBISCONTROLLEREMULATION
		// PCOrbisController Try to initialize if not setup, then Tick input
		if( OrbisController  )
		{
			FPCOrbisController::OrbisControllerContext context;
			context.Viewport = this;
			context.Client = Client;			
			context.ViewportClient = ViewportClient;
			OrbisController->Tick(context, DeltaTime);

			if (OrbisController->GetControllerHandle() >= 0)
			{
				UPCOrbisForceFeedbackManager* orbisFFManager = Cast<UPCOrbisForceFeedbackManager>(ViewportClient->GetForceFeedbackManager(0));
				if (orbisFFManager)
				{
					if( GWorld->IsPaused() )
					{
						orbisFFManager->bIsPaused = TRUE;					
					}
					
					orbisFFManager->ApplyForceFeedback(0, DeltaTime);
				}
			}
		}
#endif
...
#if DINGO
void ProcessWinRTOnKeyDown(UINT KeyDown)
{
	if( (KeyDown & 0xff00) == 0x5800 )
	{
		// Ignore rogue keyboard input coming from joystick
		return;
	}
	UWindowsClient* WinClient = (UWindowsClient*)GEngine->Client;
	WinClient->DeferMessage( WinClient->Viewports(0), WM_KEYDOWN, KeyDown, NULL );
	WinClient->DeferMessage( WinClient->Viewports(0), WM_CHAR, KeyDown, NULL );
	WinClient->DeferMessage( WinClient->Viewports(0), WM_KEYUP, KeyDown, NULL );
}
void ProcessWinRTOnKeyUp(UINT KeyUp)
{
	if( (KeyUp & 0xff00) == 0x5800 )
	{
		// Ignore rogue keyboard input coming from joystick
		return;
	}
	UWindowsClient* WinClient = (UWindowsClient*)GEngine->Client;
	WinClient->DeferMessage( WinClient->Viewports(0), WM_KEYUP, KeyUp, NULL );
}
void appDingoKeyEvent(unsigned int InCode, unsigned int InChar, bool InWasPressed)
{
	if (InCode == 0 || !InWasPressed)
	{
		return;
	}
	if( (InCode & 0xff00) == 0x5800 )
	{
		// Ignore rogue keyboard input coming from joystick
		return;
	}
	UWindowsClient* WinClient = (UWindowsClient*)GEngine->Client;
	WinClient->DeferMessage( WinClient->Viewports(0), WM_KEYDOWN, InCode, NULL );
	WinClient->DeferMessage( WinClient->Viewports(0), WM_CHAR, InCode, NULL );
	WinClient->DeferMessage( WinClient->Viewports(0), WM_KEYUP, InCode, NULL );
}
#endif
