...
//@zombie_ps4_begin
#elif ORBIS
	#define SOCKET_API TEXT("OrbisSockets")
//@zombie_ps4_end
#elif NGP
...
//@zombie_ps4_end
	...
	#include <sys/time.h>
	#include <pthread.h>
	// mathieu.gauthier: This gets defined in socket.h and happens
	// to have the same name as the pixel format enum max value.
	#ifdef PF_MAX
	#undef PF_MAX
	#endif
#endif
...
// Forward declaration for AUdpLink::UdpLinkProxy field (defined in UdpLink.cpp)
class FUdpLinkProxy;
