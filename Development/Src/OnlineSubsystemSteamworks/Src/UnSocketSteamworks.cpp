...
public:
	...
	virtual void Finalize()
	{
		...
		debugf(NAME_Log, TEXT("[P2P] ConnectFail UID=%llu Reason=%s"),
			(unsigned long long)RemoteUID, *FailReasonStr);
...
// Direct implementation of the client-side P2PSessionRequest_t callback.
// Called on the game thread by SteamAPI_RunCallbacks() via the CCallback registered
// in FOnlineAsyncTaskManagerSteam (OnClientP2PSessionRequestCallback).
void FOnlineAsyncTaskManagerSteam::OnClientP2PSessionRequestFunc(P2PSessionRequest_t* pParam)
{
	QWORD RemoteUID = pParam->m_steamIDRemote.ConvertToUint64();
	debugf(NAME_Log, TEXT("[P2P] SessionRequest from SteamID=%llu"), (unsigned long long)RemoteUID);

	// Accept immediately on the game thread (Steam API is not thread-safe;
	// calling AcceptP2PSessionWithUser from the bridge thread is unreliable).
	if (GSteamNetworking)
	{
		bool bOk = GSteamNetworking->AcceptP2PSessionWithUser(pParam->m_steamIDRemote);
		debugf(NAME_Log, TEXT("[P2P] AcceptP2PSessionWithUser SteamID=%llu result=%d"),
			(unsigned long long)RemoteUID, (int)bOk);
	}

	extern void GP2PBridge_OnSessionRequest(QWORD SteamID);
	GP2PBridge_OnSessionRequest(RemoteUID);
}
