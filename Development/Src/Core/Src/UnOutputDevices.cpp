...
void FOutputDeviceWindowsError::Serialize( const TCHAR* Msg, enum EName Event )
{
	...
	{
		// rejean.charpentier -- don't crash
#if !FINAL_RELEASE
		GErrorHist[ErrorType==NAME_FriendlyError ? ErrorPos : ARRAY_COUNT(GErrorHist)-1]=0;
		// Dump the error and flush the log.
		debugf(TEXT("=== Error ===") LINE_TERMINATOR TEXT("%s"), GErrorHist);
		GLog->Flush();
...
/** Writes UTF-8 log output directly to stdout fd=1 (used with -stdout under Wine/terminal). */
void FOutputDeviceStdout::Serialize( const TCHAR* Data, enum EName Event )
{
	FTCHARToUTF8 UTF8(Data);
	_write( 1, (ANSICHAR*)UTF8, UTF8.Length() );
	_write( 1, "\n", 1 );
}
