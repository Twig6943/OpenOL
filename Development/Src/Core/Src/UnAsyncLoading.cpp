...
// mathieu.gauthier: Ugly upstream reference to Engine package.
extern void DrawLoadingScreenStats();
...
void UObject::ProcessAsyncLoading( UBOOL bUseTimeLimit, FLOAT TimeLimit, FName ExcludeType )
{
	...
	{
#if DINGO
		// When flushing async loading, we can spend quite a while looping here.
		// We still need to remain responsive in case the application receives a
		// suspend event, so we process the messages here.
		if (bUseTimeLimit == FALSE)
		{
 ...
 * @param   Event				Event to be triggered on completion, Used instead of Counter for multithreading purposes
...
// WWISEMODIF_START, alessard, oct-14-2008, WwiseAudioIntegration - Add new parameter allowing to receive signal on completion instead of using the Counter.
	FEvent* Event,
// WWISEMODIF_END
...
// WWISEMODIF_END
	...
	// NOTE: This should be the ONLY place where we add/set things to the hashtable,
	// everywhere else we should only do lookups. 
	IORequest.HandleEntry				= &NameToHandleMap.FindOrAdd(FileName);

#if !FINAL_RELEASE
	// We need to be absolutely sure this never gets reallocated.
	check(NameToHandleMap.GetAllocatedSize() == NameToHandleMapAllocatedSize);
#endif
	...
	INC_MEMORY_STAT_BY( STAT_AsyncIO_OutstandingReadSize, IORequest.Size );
	...
	EAsyncIOPriority Priority )
{
	...
	{
// WWISEMODIF_START, alessard, oct-14-2008, WwiseAudioIntegration - Add new parameter allowing to receive signal on completion instead of using the Counter.		
		TheRequestIndex = QueueIORequest( FileName, Offset, Size, 0, Dest, COMPRESS_None, Counter, NULL, Priority );
// WWISEMODIF_END
	}
#if BLOCK_ON_ASYNCIO
...
// WWISEMODIF_START, alessard, oct-14-2008, WwiseAudioIntegration - Add new function allowing to receive signal on completion instead of using the Counter.
/**
 * Requests data to be loaded async. Returns immediately. 
 *
 * @param	FileName	File name to load
	...
	EAsyncIOPriority Priority )
{
	...
	{
		FScopeLock ScopeLock( CriticalSection );	
		TheRequestIndex = QueueIORequest( FileName, Offset, Size, 0, Dest, COMPRESS_None, NULL, Event, Priority );
	}
#if BLOCK_ON_ASYNCIO
	BlockTillAllRequestsFinished(); 
#endif
	...
	EAsyncIOPriority Priority )
{
	...
	{
// WWISEMODIF_START, alessard, oct-14-2008, WwiseAudioIntegration - Add new parameter allowing to receive signal on completion instead of using the Counter.	
		TheRequestIndex = QueueIORequest( FileName, Offset, Size, UncompressedSize, Dest, CompressionFlags, Counter, NULL, Priority );
// WWISEMODIF_END
	}
#if BLOCK_ON_ASYNCIO
	BlockTillAllRequestsFinished(); 
	...
	for( INT OutstandingIndex=OutstandingRequests.Num()-1; OutstandingIndex>=0 && RequestsCanceled<NumIndices; OutstandingIndex-- )
	{
		...
		for( INT TheRequestIndex=0; TheRequestIndex<NumIndices; TheRequestIndex++ )
		{
			...
			if( IORequest.RequestIndex == RequestIndices[TheRequestIndex] )
			{
				...
				INC_MEMORY_STAT_BY( STAT_AsyncIO_CanceledReadSize, IORequest.Size );
				DEC_DWORD_STAT( STAT_AsyncIO_OutstandingReadCount );
				DEC_MEMORY_STAT_BY( STAT_AsyncIO_OutstandingReadSize, IORequest.Size );				
...
{
#if ORBIS
	extern FRunnableThread* AsyncIOThread;
#endif
	...
	while( IsRunning.GetValue() > 0 )
	{
		...
		// Create file handles.
		{
			...
			{
#if ORBIS
				// Since we are shading the cores of the thread pool, we dont 
				// want to be pre-empted while still holding this lock. This
				// could create big bubbles of idle time on the game/render thread.
				AsyncIOThread->SetThreadPriority(TPri_AboveNormal);
#endif
				FScopeLock ScopeLock( CriticalSection );
				...
				for( INT RequestIndex=0; RequestIndex<OutstandingRequests.Num(); RequestIndex++ )
				{
					...
					&&	OutstandingRequest.HandleEntry->bIsNull )
					{
...
#if ORBIS
				AsyncIOThread->SetThreadPriority(TPri_BelowNormal);
#endif
			}
		...
		if( bIsRequestPending )
		{
			...
			else
			{
				...
				if( PlatformIsHandleValid(FileHandle) )
				{
					...
					INC_MEMORY_STAT_BY( STAT_AsyncIO_FulfilledReadSize, IORequest.Size );
				}
				else
				{
...
void FAsyncIOSystemBase::FlushHandles()
{
	...
	{
		FAsyncIOHandle& Handle = It.Value();
		if (Handle.Handle != (void*)INDEX_NONE)
		{
			check(!Handle.bIsNull);
			PlatformDestroyHandle( Handle );
			Handle = FAsyncIOHandle();
		}
		else
		{
			check(Handle.bIsNull);
...
void FArchiveAsync::FlushCache()
{
	...
	DEC_MEMORY_STAT_BY(STAT_StreamingAllocSize, PrecacheEndPos[CURRENT] - PrecacheStartPos[CURRENT]);
	...
	DEC_MEMORY_STAT_BY(STAT_StreamingAllocSize, PrecacheEndPos[NEXT] - PrecacheStartPos[NEXT]);
...
void FArchiveAsync::PrecacheCompressedChunk( INT ChunkIndex, INT BufferIndex )
{
	...
	DEC_MEMORY_STAT_BY(STAT_StreamingAllocSize, PrecacheEndPos[BufferIndex] - PrecacheStartPos[BufferIndex]);
	...
	INC_MEMORY_STAT_BY(STAT_StreamingAllocSize, PrecacheEndPos[BufferIndex] - PrecacheStartPos[BufferIndex]);
...
UBOOL FArchiveAsync::Precache( INT RequestOffset, INT RequestSize )
{
	...
	else
	{
		...
		else
		{
			...
			INC_MEMORY_STAT_BY(STAT_StreamingAllocSize, PrecacheEndPos[CURRENT] - PrecacheStartPos[CURRENT]);
