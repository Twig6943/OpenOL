...
/** TLS total linker call depth across all linker functions — catches cross-function recursion. */
static __declspec(thread) INT GLinkerTotalDepth = 0;

/** AssetLinker created by AssetRedir to load assets from the .upk alongside a .u script package.
 *  Script exports from this linker must be skipped to avoid duplicating incompatible old classes. */
static ULinkerLoad* GAssetRedirLinker = NULL;
...
void ULinkerLoad::GatherImportDependencies(INT ImportIndex, TSet<FDependencyRef>& Dependencies, UBOOL bSkipLoadedObjects)
{
	...
	static INT GatherDepth = 0;
	GatherDepth++;
	GLinkerTotalDepth++;
	struct FGatherGuard { ~FGatherGuard() { GatherDepth--; GLinkerTotalDepth--; } } _gg;
...
void ULinkerLoad::VerifyImport( INT i )
{
	...
	static INT VerifyImportDepth = 0;
	VerifyImportDepth++;
	GLinkerTotalDepth++;
	struct FVIDepthGuard { ~FVIDepthGuard() { VerifyImportDepth--; GLinkerTotalDepth--; } } _vig;
...
UBOOL ULinkerLoad::VerifyImportInner(INT ImportIndex, FString& WarningSuffix)
{
	...
	// Depth tracking to detect recursion
	static INT VerifyDepth = 0;
	VerifyDepth++;
	GLinkerTotalDepth++;
	struct FVerifyDepthGuard { ~FVerifyDepthGuard() { VerifyDepth--; GLinkerTotalDepth--; } } _guard;
	...
	if( Import.OuterIndex == ROOTPACKAGE_INDEX )
	{
...
#endif
		{
			...
			// Before calling GetPackageLinker (which calls appErrorf on failure when
			// EXCEPTIONS_DISABLED), check if the package exists as a standalone file.
			// If it doesn't, try to find it as a group inside the owner's .upk asset package.
			// Only do this redirect when loading a .u script file (not a .upk asset file),
			// and guard against re-entrancy from the asset linker's own import verification.
			{
				static UBOOL bInAssetRedirect = FALSE;
				FString PkgFile;
				const UBOOL bPkgExists = GPackageFileCache->FindPackageFile( *Import.ObjectName.ToString(), NULL, PkgFile );
				const FString OwnerExt = FFilename(Filename).GetExtension().ToLower();
				if ( !bPkgExists && !bInAssetRedirect && OwnerExt == TEXT("u") )
				{
					const FString OwnerPkgName = FFilename(Filename).GetBaseFilename();
					const FString AssetDir = FFilename(Filename).GetPath() + PATH_SEPARATOR + TEXT("..");

					// First: if a standalone .upk with the same name as the import exists in
					// CookedPCConsole/, register it so the normal loader picks it up directly.
					// This lets individual packages (e.g. menuassets.upk) override the group
					// copy baked into OLGame.upk without requiring a full asset package rebuild.
					const FString StandaloneUpk = AssetDir + PATH_SEPARATOR + Import.ObjectName.ToString() + TEXT(".upk");
					if ( GFileManager->FileSize(*StandaloneUpk) > 0 )
					{
						GPackageFileCache->CachePackage( *StandaloneUpk, FALSE, FALSE );
						// Fall through to normal GetPackageLinker below.
					}
					else
					{
						// Second: redirect to the group inside OLGame.upk.
						const FString CandidateUpk = AssetDir + PATH_SEPARATOR + OwnerPkgName + TEXT(".upk");
						if ( GFileManager->FileSize(*CandidateUpk) > 0 )
						{
							const FString AssetPkgName = OwnerPkgName + TEXT("_Assets");
							UPackage* AssetPkg = CreatePackage( NULL, *AssetPkgName );
							bInAssetRedirect = TRUE;
							ULinkerLoad* AssetLinker = GetPackageLinker( AssetPkg, *CandidateUpk, LOAD_NoWarn|LOAD_Quiet|LOAD_NoVerify, NULL, NULL );
							bInAssetRedirect = FALSE;
							if ( AssetLinker )
							{
								GAssetRedirLinker = AssetLinker;
								static UBOOL bWarnedAssetRedir = FALSE;
								if ( !bWarnedAssetRedir )
								{
									bWarnedAssetRedir = TRUE;
									GLog->Logf( NAME_Warning, TEXT("AssetRedir: linking %s with asset package %s"), *FFilename(Filename).GetBaseFilename(), *CandidateUpk );
								}
								UPackage* GroupPkg = CreatePackage( NULL, *Import.ObjectName.ToString() );
								Import.XObject      = GroupPkg;
								Import.SourceLinker = AssetLinker;
								Import.SourceIndex  = ROOTPACKAGE_INDEX;
							}
						}
					}
				}
	...
	else
	{
		...
		// If this import is a Package-class group that exists as a standalone .upk file,
		// point it directly at that file instead of inheriting the Outer's (OLGame.upk) linker.
		// This lets e.g. menuassets.upk in CookedPCConsole/ override the baked-in group.
		if ( Import.ClassName == NAME_Package )
		{
			FString StandalonePath;
			if ( GPackageFileCache->FindPackageFile( *Import.ObjectName.ToString(), NULL, StandalonePath ) )
			{
				const FFilename OwnerExt = FFilename(Filename).GetExtension().ToLower();
				if ( OwnerExt == TEXT("u") )
				{
					UPackage* StandalonePkg = CreatePackage( NULL, *Import.ObjectName.ToString() );
					ULinkerLoad* StandaloneLinker = GetPackageLinker( StandalonePkg, *StandalonePath, LOAD_NoWarn|LOAD_Quiet, NULL, NULL );
					if ( StandaloneLinker )
					{
						Import.XObject      = StandalonePkg;
						Import.SourceLinker = NULL;
		...
		// If the Outer was redirected to a standalone package (XObject set, SourceLinker NULL),
		// retrieve the linker from that package so child imports resolve from the standalone file.
		{
			FObjectImport& OuterImp = ImportMap(-Import.OuterIndex-1);
			if ( OuterImp.SourceLinker == NULL && OuterImp.XObject != NULL )
			{
				ULinkerLoad* StandaloneLinker = UObject::GetPackageLinker( (UPackage*)OuterImp.XObject, NULL, LOAD_NoWarn|LOAD_Quiet, NULL, NULL );
				Import.SourceLinker = StandaloneLinker;
			}
			else
			{
				Import.SourceLinker = OuterImp.SourceLinker;
			}
		}
		...
		if( Import.SourceLinker )
		{
				...
				&&  (Import.SourceLinker->GetExportClassPackage(j)==Import.ClassPackage) )
				{
					...
					if( IS_IMPORT_INDEX(Import.OuterIndex) )
					{
						...
						if( OuterImport.SourceLinker )
						{
							...
							else if( OuterImport.SourceIndex == ROOTPACKAGE_INDEX
								&& OuterImport.SourceLinker
								&& FFilename(OuterImport.SourceLinker->Filename).GetExtension().ToLower() == TEXT("upk") )
							{
								// Outer is a group-package redirected to an asset .upk (SourceIndex
								// set to ROOTPACKAGE_INDEX as a sentinel). Accept exports whose
								// immediate outer export name matches the group import name.
								if( SourceExport.OuterIndex != ROOTPACKAGE_INDEX )
								{
									UBOOL bGroupMatch = FALSE;
									if ( SourceExport.OuterIndex > 0
										&& SourceExport.OuterIndex <= OuterImport.SourceLinker->ExportMap.Num() )
									{
										const FName OuterExportName = OuterImport.SourceLinker->ExportMap(SourceExport.OuterIndex - 1).ObjectName;
										bGroupMatch = ( OuterExportName == OuterImport.ObjectName );
									}
									if ( !bGroupMatch )
										continue;
								}
			...
			// If the object was not found in the script-only .u linker, fall back to the .upk with
			// the same base name one directory above (e.g. CookedPCConsole/OLGame.upk for
			// CookedPCConsole/MultiplayerContent/OLGame.u).
			// Do NOT fall back for Class or Package objects — those must come from the .u scripts,
			// not from the .upk (which contains a cooked copy under a different package name).
			if ( Import.SourceIndex == INDEX_NONE && Import.SourceLinker != NULL
				&& Import.ClassName != NAME_Class && Import.ClassName != NAME_Package )
			{
				const FFilename ScriptFilename = Import.SourceLinker->Filename;
				if ( ScriptFilename.GetExtension().ToLower() == TEXT("u") )
				{
					const FString PkgBaseName  = ScriptFilename.GetBaseFilename();
					const FString AssetPkgName = PkgBaseName + TEXT("_Assets");
					// Build path: one directory up from the .u file, same base name + .upk
					const FString AssetFile = ScriptFilename.GetPath() + PATH_SEPARATOR + TEXT("..") + PATH_SEPARATOR + PkgBaseName + TEXT(".upk");
					if ( GFileManager->FileSize(*AssetFile) > 0 )
					{
						UPackage* AssetPkg = CreatePackage( NULL, *AssetPkgName );
						ULinkerLoad* AssetLinker = GetPackageLinker( AssetPkg, *AssetFile, LOAD_NoWarn|LOAD_Quiet|LOAD_NoVerify, NULL, NULL );
						if ( AssetLinker )
						{
							INT iHashAsset = HashNames( Import.ObjectName, Import.ClassName, Import.ClassPackage ) & (ARRAY_COUNT(ExportHash)-1);
							for ( INT j = AssetLinker->ExportHash[iHashAsset]; j != INDEX_NONE; j = AssetLinker->ExportMap(j)._iHashNext )
							{
								FObjectExport& AssetExport = AssetLinker->ExportMap(j);
								if (  AssetExport.ObjectName                == Import.ObjectName
								&&    AssetLinker->GetExportClassName(j)    == Import.ClassName
								&&    AssetLinker->GetExportClassPackage(j) == Import.ClassPackage )
								{
									Import.SourceLinker = AssetLinker;
									Import.SourceIndex  = j;
									break;
							...
							// Hash miss can happen when ClassPackage in the .upk differs (e.g. OLGame_Assets
							// instead of OLGame). Fall back to linear scan matching only ObjectName+ClassName.
							if ( Import.SourceIndex == INDEX_NONE && IS_IMPORT_INDEX(Import.OuterIndex) )
							{
								const FName& OuterName = ImportMap(-Import.OuterIndex-1).ObjectName;
								for ( INT j = 0; j < AssetLinker->ExportMap.Num(); j++ )
								{
									FObjectExport& E = AssetLinker->ExportMap(j);
									if ( E.ObjectName == Import.ObjectName
									&&   AssetLinker->GetExportClassName(j) == Import.ClassName )
									{
										// Verify outer name matches to avoid false positives
										UBOOL bOuterOk = FALSE;
										if ( E.OuterIndex > 0 && E.OuterIndex <= AssetLinker->ExportMap.Num() )
											bOuterOk = ( AssetLinker->ExportMap(E.OuterIndex-1).ObjectName == OuterName );
										else if ( E.OuterIndex == ROOTPACKAGE_INDEX )
											bOuterOk = TRUE;
										if ( bOuterOk )
										{
											Import.SourceLinker = AssetLinker;
				...
				else if ( ScriptFilename.GetExtension().ToLower() == TEXT("upk") )
				{
					// Outer was redirected to the asset .upk but the object wasn't found via hash.
					// This can happen when ClassPackage in the .upk differs (OLGame_Assets vs OLGame).
					// Try a linear scan matching only ObjectName+ClassName, ignoring ClassPackage.
					ULinkerLoad* SL = Import.SourceLinker;
					if ( IS_IMPORT_INDEX(Import.OuterIndex) )
					{
						const FName& OuterName = ImportMap(-Import.OuterIndex-1).ObjectName;
						for ( INT k = 0; k < SL->ExportMap.Num(); k++ )
						{
							FObjectExport& E = SL->ExportMap(k);
							if ( E.ObjectName == Import.ObjectName
							&&   SL->GetExportClassName(k) == Import.ClassName )
							{
								UBOOL bOuterOk = FALSE;
								if ( E.OuterIndex > 0 && E.OuterIndex <= SL->ExportMap.Num() )
									bOuterOk = ( SL->ExportMap(E.OuterIndex-1).ObjectName == OuterName );
								else if ( E.OuterIndex == ROOTPACKAGE_INDEX )
									bOuterOk = TRUE;
								if ( bOuterOk )
								{
									Import.SourceIndex = k;
									break;
								}
							}
						}
...
void ULinkerLoad::Preload( UObject* Object )
{
	...
	struct FPreloadDepthGuard { ~FPreloadDepthGuard() { GLinkerTotalDepth--; } } _pg;
	GLinkerTotalDepth++;
...
UObject* ULinkerLoad::CreateExport( INT Index )
{
	...
	if( !Export._Object && (Export.ObjectFlags & _ContextFlags) )
	{
		...
		// If this is the asset-redirect linker (old .upk loaded for assets alongside a .u),
		// do not create script objects from the .upk — it has an older incompatible version.
		// Instead, redirect UClass exports to already-loaded classes from the .u package.
		// CDO and other script objects are suppressed (NULL) since their layout is wrong.
		extern ULinkerLoad* GAssetRedirLinker;
		if ( this == GAssetRedirLinker )
		{
			if ( Export.OuterIndex == ROOTPACKAGE_INDEX )
			{
				const FName ExportClass = GetExportClassName(Index);
				if ( ExportClass == NAME_Class )
				{
					// Reuse the already-loaded UClass from the .u script package.
					UClass* Existing = FindObject<UClass>( ANY_PACKAGE, *Export.ObjectName.ToString(), TRUE );
					if ( Existing )
					{
						return Existing;
					}
				...
				// Skip all other script objects (Enum, ScriptStruct, Function, State, CDO).
				const UBOOL bIsScriptObject =
					ExportClass == FName(TEXT("Enum"))
					|| ExportClass == FName(TEXT("ScriptStruct"))
					|| ExportClass == FName(TEXT("Function"))
					|| ExportClass == FName(TEXT("State"))
					|| ExportClass == FName(TEXT("Const"))
			...
			{
				// Asset object inside a top-level group -- if a standalone .upk for this group exists,
				// suppress creation here so the object is loaded from the standalone package instead.
				// Only applies to groups directly under the root package (OuterIndex of group == ROOTPACKAGE_INDEX).
				const FObjectExport& OuterExport = ExportMap( Export.OuterIndex - 1 );
				if ( OuterExport.ObjectName != NAME_None
					&& OuterExport.OuterIndex == ROOTPACKAGE_INDEX )
				{
					FString StandalonePath;
					if ( GPackageFileCache->FindPackageFile( *OuterExport.ObjectName.ToString(), NULL, StandalonePath ) )
					{
						// Only suppress if the standalone .upk is a different file from this linker.
						if ( FFilename(StandalonePath).GetExtension().ToLower() == TEXT("upk")
							&& FFilename(StandalonePath).GetBaseFilename().ToLower() != FFilename(Filename).GetBaseFilename().ToLower() )
						{
							return NULL;
						}
					}
...
UObject* ULinkerLoad::CreateImport( INT Index )
{
	...
	GLinkerTotalDepth++;
	struct FCIDepthGuard { ~FCIDepthGuard() { GLinkerTotalDepth--; } } _cig;
