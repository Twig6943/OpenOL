...
ECompressionFlags FUntypedBulkData::GetDecompressionFlags() const
{
	return
		...
		(BulkDataFlags & BULKDATA_SerializeCompressedLZ4) ? COMPRESS_LZ4 : 
		COMPRESS_None;
}
...
void FUntypedBulkData::StoreCompressedOnDisk( ECompressionFlags CompressionFlags )
{
	...
	else
	{
		...
		check(CompressionFlags & (COMPRESS_ZLIB | COMPRESS_LZO | COMPRESS_LZX | COMPRESS_LZ4));
		BulkDataFlags |=	(CompressionFlags & COMPRESS_ZLIB) ? BULKDATA_SerializeCompressedZLIB : 
							...
							(CompressionFlags & COMPRESS_LZ4) ? BULKDATA_SerializeCompressedLZ4 :
							 BULKDATA_None;
	}
}
