			...
			if (DoesProfileExist())
			{
				...
				if (appLoadFileToArray(Buffer,*CreateProfileName()))
				{
					...
					if (Reader.SerializeFromBuffer(CachedProfile->ProfileSettings))
					{
						...
						debugf(NAME_DevOnline,TEXT("Profile version: file=%d expected=%d"),ReadVersion,CachedProfile->VersionNumber);
	...
	UOnlineProfileSettings* ProfileSettings)
{
	...
	if (Return != ERROR_IO_PENDING)
	{
		...
		OnlineSubsystemPC_eventOnWriteProfileSettingsComplete_Parms Results(EC_EventParm);
		Results.LocalUserNum = 0;
		Results.bWasSuccessful = (Return == ERROR_SUCCESS);
		TriggerOnlineDelegates(this,WriteProfileSettingsDelegates,&Results);
	}
	return Return == S_OK || Return == ERROR_IO_PENDING;
}
