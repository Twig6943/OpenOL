...
//@zombie_ps4_end
#if DINGO_USES_POOLED_MALLOC
#include "FMallocDingoPooled.h"
#endif
...
#if DINGO
	#include "DingoViewport.h"
#endif

//@zombie_ps4_begin
#if ORBIS
	#include "OrbisViewport.h"
#endif
//@zombie_ps4_end
...
//@zombie_ps4_begin
extern void AutoInitializeRegistrantsOrbisDrv( INT& Lookup );
//@zombie_ps4_end
...
extern void AutoInitializeRegistrantsDingoDrv( INT& Lookup );
...
extern void AutoInitializeRegistrantsOnlineSubsystemNP( INT& Lookup );
extern void AutoInitializeRegistrantsOnlineSubsystemDingo( INT& Lookup );
...
#elif GAMENAME == OLGAME
extern void AutoInitializeRegistrantsUDKBase( INT& Lookup );
extern void AutoInitializeRegistrantsOLGame( INT& Lookup );
extern void AutoInitializeRegistrantsMultiplayer( INT& Lookup );
extern void AutoInitializeRegistrantsOLEditor( INT& Lookup );
...
extern void AutoGenerateNamesOnlineSubsystemNP();
extern void AutoGenerateNamesOnlineSubsystemDingo();
...
#elif GAMENAME == OLGAME
extern void AutoGenerateNamesUDKBase();
extern void AutoGenerateNamesOLGame();
extern void AutoGenerateNamesMultiplayer();
extern void AutoGenerateNamesOLEditor();
...
//@zombie_ps4_begin
#elif ORBIS
extern "C" {TCHAR GPackage[64]=TEXT("OrbisLaunch");}
//@zombie_ps4_end
#elif NGP
...
#elif DINGO_USES_XMEM_MALLOC
#include "FMallocDingoXMem.h"
	GMalloc = new FMallocDingoXMem();
#elif DINGO_USES_POOLED_MALLOC
	GMalloc = new FMallocDingoPooled();
#else
#include "FMallocDingo.h"
	GMalloc = new FMallocDingo();
#endif
#elif _WIN64
	GMalloc = new FMallocTBB();
#elif IPHONE
...
static FOutputDeviceAnsiError				Error;
static FFeedbackContextAnsi					GameWarn;
static FFileManagerDingo					FileManager;
#elif PS3
...
//@zombie_ps4_begin
#elif ORBIS
static FSynchronizeFactoryOrbis				SynchronizeFactory;
static FThreadFactoryOrbis					ThreadFactory;
static FOutputDeviceFile					Log;
static FOutputDeviceAnsiError				Error;
static FFeedbackContextAnsi					GameWarn;
static FFileManagerOrbis					FileManager;
static FQueuedThreadPoolOrbis				ThreadPool;
//@zombie_ps4_end
#elif NGP
...
#elif GAMENAME == OLGAME
INT			GGameIcon	= IDICON_OLGame;
INT			GEditorIcon	= IDICON_OLEditor;
#elif GAMENAME == EXOGAME
INT			GGameIcon	= IDICON_DemoGame;
INT			GEditorIcon	= IDICON_DemoEditor;
#elif GAMENAME == SWORDGAME
...
void appInitFullScreenMoviePlayer()
{
	...
	#elif DINGO
		GFullScreenMovieViewport = new FDingoViewport(NULL,NULL,TEXT("Movie"),GScreenWidth,GScreenHeight,TRUE);
	#endif
#elif IPHONE
...
const TCHAR* appGetOSSPackageName()
{
	...
	return TEXT("PC");
		...
		return TEXT("PC");
	#endif
//@zombie_ps4_begin - Adding onlinesubsystem support for Orbis
#elif PLATFORM_ORBIS
	return TEXT("NP");
//@zombie_ps4_end
#elif DINGO
...
//@zombie_ps4_end
	...
	// if we are running the game, or cooking for Windows, we check for how the code was compiled
		...
		if (GIsCooking || GSteamworksClientInitialized)
		{
			return TEXT("Steamworks");	
		}		
		return TEXT("PC"); // rcharpentier: if steam is disabled (config, command-line or SteamAPI_Init() failed), revert to PC
	#elif WITH_PANORAMA
...
void appGetScriptPackageNames(TArray<FString>& PackageNames, UINT ScriptTypes, const TCHAR* EngineConfigFilename)
{
	...
	if (ScriptTypes & SPT_EngineNative)
	{
		...
		if( ParseParam(appCmdLine(),TEXT("CHECK_NATIVE_CLASS_SIZES")) == FALSE )
		{
			...
			// rcharpentier - force cooking OnlineSubsytemPC
			if (GIsCooking && !(GCookingTarget & UE3::PLATFORM_Console))
			{
				FString OSSPackage(TEXT("OnlineSubsystemPC"));
...
// WWISEMODIF_START
	PackageNames.AddItem(TEXT("AkAudio"));
// WWISEMODIF_END
...
void CheckNativeClassSizes()
{
	...
	extern void AutoCheckNativeClassSizesOLGame( UBOOL& Mismatch );
	AutoCheckNativeClassSizesOLGame( Mismatch );

#if !CONSOLE && !PLATFORM_MACOSX
	extern void AutoCheckNativeClassSizesOLEditor( UBOOL& Mismatch );
	AutoCheckNativeClassSizesOLEditor( Mismatch );
#endif
...
INT FEngineLoop::PreInit( const TCHAR* CmdLine )
{
	...
	GIsSeekFreePCConsole = GUseSeekFreeLoading; // rcharpentier - assumes that the cook is pcconsole
...
#else  // XBOX
	// This is done in appXenonInit on Xenon and appDingoInit on Dingo.
	GSynchronizeFactory			= &SynchronizeFactory;
	GThreadFactory				= &ThreadFactory;
	GThreadPool					= &ThreadPool;
//@zombie_ps4_begin
#if ORBIS
	INT NumThreadsInThreadPool = 3;
#else
...
//@zombie_ps4_end
...
#if _WINDOWS && !USE_NULL_RHI && !CONSOLE && !DINGO
	...
	if( GConfig->GetInt( TEXT( "Startup" ), TEXT( "RenderMode" ), RenderModeMode, GEditorUserSettingsIni ) )
	{
		...
		else if ( RenderModeMode == RENDER_MODE_DX10 )
		{
			GRenderMode = RENDER_MODE_DX10;
		}
	}
...
#if DINGO_AS_PC
	UBOOL bRunningRemoteDesktop = FALSE;
#else
...
//@zombie_ps4_begin
#elif ORBIS
	FAsyncIOSystemOrbis*	AsyncIOSystem = new FAsyncIOSystemOrbis();
//@zombie_ps4_end
#elif DINGO
	FAsyncIOSystemDingo*	AsyncIOSystem = new FAsyncIOSystemDingo();
#elif _MSC_VER
...
//@zombie_ps4_begin
#if ORBIS
	AsyncIOThread->SetProcessorAffinityMask(POOL_THREAD_AFFINITY_MASK);
#endif
//@zombie_ps4_end
...
// handle launching dedicated server on console
	...
	// mathieu.gauthier: We dont want to see stuff pop-in when recording a movie.
	if (GIsDumpingMovie)
	{
...
INT FEngineLoop::Init()
{
...
#if XBOX || PS3 || IPHONE || DINGO // The movie playing can only create the special startup movie viewport on XBOX and PS3 (or DINGO).
	if (GFullScreenMovie && !GIsEditor)
	{
		// play a looping movie from RAM while booting up
		GFullScreenMovie->GameThreadInitiateStartupSequence(TRUE);
	}
#endif
	...
	// rcharpentier - force steam to be running and valid (copy-protection)
#if WITH_STEAMWORKS 
	if (!GSteamworksClientInitialized) 
	{
		debugf(TEXT("Steamworks is disabled"));
	}
	else if (!GSteamApps)
	{
		debugf(TEXT("Steamworks is disabled (invalid GSteamApps)"));
	}
	else if (!GSteamApps->BIsSubscribed())
	{
		debugf(TEXT("Steamworks is disabled (not subscribed)"));
	}
#endif
...
void FEngineLoop::Exit()
{
...
#if USE_BINK_CODEC
#if DINGO
	// Other platforms dont seem to shutdown the fullscreen movie & associated viewport but there are cases where we need to on Dingo if we want a clean shutdown. 
	// Specifically the async poller is ref-counted off of the view port so we need to make sure all the view ports are shutdown to shutdown that thread.
	// We also need to make sure the render thread is shutdown before we shutdown the movies since a Bink movie might be currently/actively rendering.
	if( GFullScreenMovie )
	{
	...
	if( GFullScreenMovieViewport )
	{
		delete GFullScreenMovieViewport;
		GFullScreenMovieViewport = NULL;
	}
#endif //DINGO
...
#endif	//#if WITH_UE3_NETWORKING
//@zombie_ps4_begin
#if !PS3 && !MOBILE && !PLATFORM_MACOSX && !WIIU && !ORBIS && !DINGO
//@zombie_ps4_end
...
//@zombie_ps4_begin
#elif ORBIS
	AutoInitializeRegistrantsOrbisDrv( Lookup );	
//@zombie_ps4_end
	// rcharpentier
	AutoInitializeRegistrantsAkAudio( Lookup );
#elif IPHONE
	...
	AutoInitializeRegistrantsAkAudio( Lookup );
#elif DINGO
	AutoInitializeRegistrantsDingoDrv( Lookup );	
	AutoInitializeRegistrantsAkAudio( Lookup );
...
// WWISEMODIF_END
	...
	#elif WITH_ONLINESUBSYSTEMNP
		AutoInitializeRegistrantsOnlineSubsystemNP( Lookup );
		AutoGenerateNamesOnlineSubsystemNP();
	#elif DINGO
		AutoInitializeRegistrantsOnlineSubsystemDingo( Lookup );
		AutoGenerateNamesOnlineSubsystemDingo();		
	#else
		AutoInitializeRegistrantsOnlineSubsystemPC( Lookup );
		AutoGenerateNamesOnlineSubsystemPC();
	#endif
...
#elif GAMENAME == OLGAME
	AutoInitializeRegistrantsUDKBase( Lookup );
	AutoInitializeRegistrantsOLGame( Lookup );
	AutoInitializeRegistrantsMultiplayer( Lookup );
#if _WINDOWS && WITH_EDITOR
	AutoInitializeRegistrantsOLEditor( Lookup );
#endif
...
#elif GAMENAME == OLGAME
	AutoGenerateNamesUDKBase();
	AutoGenerateNamesOLGame();
	AutoGenerateNamesMultiplayer();
	#if _WINDOWS && WITH_EDITOR
		AutoGenerateNamesOLEditor();
	#endif
