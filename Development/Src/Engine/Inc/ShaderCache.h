...
public:
	...
	/** Loads the shader cache. bSkipVersionCheck=TRUE allows loading shaders from older package versions (e.g. original RefShaderCache). */
	void Load(FArchive& Ar, UBOOL bSkipVersionCheck = FALSE);
 ...
 */
extern void SerializeShaders(const TMap<FGuid,FShader*>& InShaders, FArchive& Ar, UBOOL bSkipVersionCheck = FALSE);
