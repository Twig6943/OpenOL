...
#define VER_MIN_SHADER	VER_DINGO
 ...
 */
#define INIT_SHADERS_ON_DEMAND	(XBOX || PS3_SHADER_COMPRESSION_METHOD || DINGO)
...
public:
	...
	void ForceBaseIndex(WORD InBaseIndex) { BaseIndex = InBaseIndex; }
	...
	)
{
	if(Parameter.IsBound())
	{
#if CONSOLE && !PLATFORM_SUPPORTS_D3D10_PLUS	// DX11 platforms must use the full RHISetSamplerState call below
		RHISetVertexTexture(Parameter.GetBaseIndex(), TextureRHI);
#else // CONSOLE && !PLATFORM_SUPPORTS_D3D10_PLUS
		check(ElementIndex < Parameter.GetNumResources());
...
public:
	...
	/** Set to TRUE when loaded from an old RefShaderCache with SIZE mismatch — rebind parameters after InitRHI decompresses bytecode. */
	UBOOL bNeedsParameterRebind;
	...
	// Called after loading from an old (868) ref shader cache to fix up parameter bindings from bytecode reflection.
...
public:
	...
	*/
	...
	void SetName(const TCHAR* InName)
	{
		...
		Name = InName;
		GetNameToTypeMap().Set(FName(Name), this);
	}
...
public:
	...
	ShaderType* GetShader() const
	{
		...
		if (ShaderRef == NULL || ShaderRef->GetReference() == NULL)
		{
			warnf(TEXT("OldRefCache: missing global shader %s"), ShaderType::StaticType.GetName());
			return NULL;
		}
...
public:
	 ...
	 Shader(ShaderIndex->template GetShader<ShaderType>()) // gcc3 needs the template quantifier so it knows the < is not a less-than
	{
		if (!Shader)
		{
			warnf(TEXT("OldRefCache: NULL global shader: %s"), ShaderType::StaticType.GetName());
		}
	}
