...
{
	return Platform == SP_PCD3D_SM3	|| Platform == SP_PCD3D_SM4	|| Platform == SP_PCD3D_SM5 || Platform == SP_PCOGL;
}
...
/** Returns TRUE if we allow deferred shading for the given platform. */
extern UBOOL ShouldCompileDeferredShaders(EShaderPlatform Platform = GRHIShaderPlatform);
...
public:
	...
	*/
	...
	void LogAllParameters(const TCHAR* Prefix) const
	{
		for(TMap<FString,FParameterAllocation>::TConstIterator It(ParameterMap);It;++It)
		{
			const FParameterAllocation& PA = It.Value();
			warnf(TEXT("%s  [%u/%u/%u] %s"), Prefix, (UINT)PA.BufferIndex, (UINT)PA.BaseIndex, (UINT)PA.Size, *It.Key());
		}
	}
...
public:
	...
	friend void VerifyGlobalShaders(EShaderPlatform Platform, UBOOL bAllowMissingShaders);
