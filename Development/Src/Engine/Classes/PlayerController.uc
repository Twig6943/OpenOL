...
/** If true, this controller is excluded from SeqVar_Player lookups (Kismet Matinee binding).
    Set on multiplayer observer controllers so remote cutscenes don't capture them. */
var		bool		bExcludeFromKismetPlayer;
...
cpptext
{
	...
	virtual const APlayerController* GetAPlayerController() const { return this; }
...
// WWISEMODIF_END
	...
	virtual UBOOL TickPaused(FLOAT DeltaSeconds) { return FALSE; }

	// rcharpentier - allow overriding the listener location, if different from camera (default values passed in)
...
event CheckPrivileges()
{
	...
	{
		PlayerIdx = class'UIInteraction'.static.GetPlayerIndex(LP.ControllerId);
		bCanPlayOnline = OnlineSub.PlayerInterface.CanPlayOnline(PlayerIdx) == FPL_Disabled;
...
simulated event PostBeginPlay()
{
	...
	// Reset our privileges, these will be checked once we get a valid Player
	bCanPlayOnline = FALSE;
	bCanShareUserCreatedContent =  FALSE;
	...
	if ( LP != None )
	{
		// Register for privilege checks, do this here instead of RegisterOnlineDelegates since we initiate the privilege check before
		// that is called and we need to ensure the callback registered
...
// WWISEMODIF_START, alessard, nov-28-2008, WwiseAudioIntegration
		if( ASound.FaceFXAnimName != "" &&
			SourceActor.PlayActorFaceFXAnim(ASound.FaceFXAnimSetRef, ASound.FaceFXGroupName, ASound.FaceFXAnimName, ASound, none, FFXP_Normal) )
		{
...
// WWISEMODIF_START, alessard, nov-28-2008, WwiseAudioIntegration
reliable client function ClientPlayActorFaceFXAnim(Actor SourceActor, FaceFXAnimSet AnimSet, string GroupName, string SeqName, SoundCue SoundCueToPlay, AkEvent AkEventToPlay, FaceFXPriority Priority)
{
	if (SourceActor != None)
	{
		SourceActor.PlayActorFaceFXAnim(AnimSet, GroupName, SeqName, SoundCueToPlay, AkEventToPlay, Priority);
	}
}
// WWISEMODIF_END
...
event Destroyed()
{
	...
	// Clear our privilege check delegate
	if (OnlineSub.PlayerInterface != None)
	{
...
reliable client event ClientSetCameraFade(bool _bEnableFading, optional color _FadeColor, optional vector2d _FadeAlpha, optional float _FadeTime, optional bool _bFadeAudio)
{
	if (PlayerCamera != None)
	{
		PlayerCamera.bEnableFading = _bEnableFading;
		if (PlayerCamera.bEnableFading)
		{
			PlayerCamera.FadeColor = _FadeColor;
			PlayerCamera.FadeAlpha = _FadeAlpha;
			PlayerCamera.FadeTime = _FadeTime;
			PlayerCamera.FadeTimeRemaining = _FadeTime;
			PlayerCamera.bFadeAudio = _bFadeAudio;
		}
 		else
 		{
...
function OnToggleCinematicMode(SeqAct_ToggleCinematicMode Action)
{
	...
	if (bExcludeFromKismetPlayer || Action.bObserverOnly)
	{
		return;
	}
 ...
 */
event SetCinematicMode( SeqAct_ToggleCinematicMode Action, bool bInCinematicMode, bool bHidePlayer, bool bAffectsHUD, bool bAffectsMovement, bool bAffectsTurning, bool bAffectsButtons )
{
	...
	if ( bAdjustMoveInput || !bInCinematicMode )
	{
		IgnoreMoveInput(bCinematicMode);
		//bCinemaDisableInputMove = bCinematicMode;
	}
	if ( bAdjustLookInput || !bInCinematicMode)
	{
...
reliable client event ClientSetCinematicMode(bool bInCinematicMode, bool bAffectsMovement, bool bAffectsTurning, bool bAffectsHUD)
{
	/*bCinematicMode = bInCinematicMode;
