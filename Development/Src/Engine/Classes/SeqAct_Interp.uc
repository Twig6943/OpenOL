...
cpptext
{
	...
	virtual void ForceActivateInput(INT InputIdx);
}
...
var() bool bPlayThroughWorldReset;
...
/** If true, InitInterp will skip binding PlayerControllers and Pawns to groups.
    Set by multiplayer observer sync so remote Matinees only move scene objects. */
var transient bool bObserverOnly;

/** If true, this Matinee was triggered remotely via TRIGGER_ACT and must not capture the local player.
    Survives across ticks so TermInterp can clear bExcludeFromKismetPlayer on the PC. */
var transient bool bRemoteObserverMatinee;
...
native final function InitInterp();
native final function UpdateInterp(float NewPosition, optional bool bPreview = false, optional bool bJump = false, optional bool bOnlyAIGroup = false);
native final function TermInterp();
