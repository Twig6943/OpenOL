...
IMPLEMENT_SHADER_TYPE(,FDownsampleScene,TEXT("DownSampleColorPixelShader"),TEXT("DownsampleScene"),SF_Pixel,0,0); \

/** A macro to declaring a filter shader type for a specific number of samples. */
// typedef нужен чтобы убрать запятую из аргумента IMPLEMENT_SHADER_TYPE (ограничение препроцессора)
// Имя typedef намеренно совпадает с именем в оригинальном 868 GlobalShaderCache
#define IMPLEMENT_FILTER_SHADER_TYPE(NumSamples) \
	typedef TFilterPixelShader<NumSamples,FALSE> TFilterPixelShader##NumSamples; \
	IMPLEMENT_SHADER_TYPE(template<>,TFilterPixelShader##NumSamples,TEXT("FilterPixelShader"),TEXT("Main"),SF_Pixel,0,0); \
	typedef TFilterPixelShader<NumSamples,TRUE> TFilterPixelShaderRenormalized##NumSamples; \
	IMPLEMENT_SHADER_TYPE(template<>,TFilterPixelShaderRenormalized##NumSamples,TEXT("FilterPixelShader"),TEXT("Main"),SF_Pixel,0,0); \
...
// Переименовываем TFilterPixelShader<N,FALSE> → TFilterPixelShader<N>
// чтобы имена совпадали с оригинальным 868 GlobalShaderCache
struct FFilterShaderNameFixer
{
	FFilterShaderNameFixer()
	{
#define RENAME_FILTER_PIXEL_SHADER(N) \
		TFilterPixelShader<N,FALSE>::StaticType.SetName(TEXT("TFilterPixelShader<") TEXT(#N) TEXT(">"));
		RENAME_FILTER_PIXEL_SHADER(1)
		RENAME_FILTER_PIXEL_SHADER(2)
		RENAME_FILTER_PIXEL_SHADER(3)
		RENAME_FILTER_PIXEL_SHADER(4)
		RENAME_FILTER_PIXEL_SHADER(5)
		RENAME_FILTER_PIXEL_SHADER(6)
		RENAME_FILTER_PIXEL_SHADER(7)
		RENAME_FILTER_PIXEL_SHADER(8)
		RENAME_FILTER_PIXEL_SHADER(9)
		RENAME_FILTER_PIXEL_SHADER(10)
		RENAME_FILTER_PIXEL_SHADER(11)
		RENAME_FILTER_PIXEL_SHADER(12)
		RENAME_FILTER_PIXEL_SHADER(13)
		RENAME_FILTER_PIXEL_SHADER(14)
		RENAME_FILTER_PIXEL_SHADER(15)
		RENAME_FILTER_PIXEL_SHADER(16)
#undef RENAME_FILTER_PIXEL_SHADER
	}
} GFilterShaderNameFixer;
...
void SetFilterShaders(
	...
	UBOOL bRenormalizeClippedSamples = FALSE
	)
{
...
#define SET_FILTER_SHADER_TYPE(NumSamples, bRenormalizeClipped) \
	case NumSamples: \
	{ \
		TShaderMapRef<TFilterVertexShader<NumSamples> > VertexShader(GetGlobalShaderMap()); \
		TShaderMapRef<TFilterPixelShader<NumSamples, bRenormalizeClipped> > PixelShader(GetGlobalShaderMap()); \
...
{
	// Use a single triangle instead of a 2-tri quad if its a full screen effect
	if (((UINT) X == 0 && (UINT) Y <= 0 && (UINT) SizeX == TargetSizeX && (UINT) SizeY == TargetSizeY))
	{
		SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,TEXT("Single Triangle Quad Draw"));

		// Set up the vertices.
		FFilterVertex Vertices[3];

		Vertices[0].Position = FVector4(X,			Y,			ClipSpaceQuadZ,	1);
		Vertices[1].Position = FVector4(X + SizeX * 2.0f,	Y,			ClipSpaceQuadZ,	1);
		Vertices[2].Position = FVector4(X,			Y + SizeY * 2.0f,	ClipSpaceQuadZ,	1);

		Vertices[0].UV = FVector2D(U,			V);
		Vertices[1].UV = FVector2D(U + SizeU * 2.0f,	V);
		Vertices[2].UV = FVector2D(U,			V + SizeV * 2.0f);
		...
		RHIDrawPrimitiveUP(PT_TriangleList,1,Vertices,sizeof(Vertices[0]));
	}
	else
	{
		SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,TEXT("Two Triangle Quad Draw"));
...
void GaussianBlurFilterBuffer(
	...
	FVector2D SampleMaskMin,
	FVector2D SampleMaskMax,
	UBOOL bRenormalizeClippedSamples)
{
	...
	UINT NumSamples1D;
	{
		...
		if(SinglePass)
		{
			...
			ApplyGaussianBlurStep(SizeX, SizeY, BlurOffsets, BlurWeights, NumSamples2D, FilterColorIndex, FilterColorIndex, SampleMaskMin, SampleMaskMax, bRenormalizeClippedSamples);
		}
		else
		{
			// compute weights as weighted contributions of white
...
#endif
			ApplyGaussianBlurStep(SizeX, SizeY, BlurOffsets, BlurWeights, NumSamples1D, FilterColorIndex, TempFilterColorIndex, SampleMaskMin, SampleMaskMax, bRenormalizeClippedSamples);
			...
			ApplyGaussianBlurStep(SizeX, SizeY, BlurOffsets, BlurWeights, NumSamples1D, TempFilterColorIndex, FilterColorIndex, SampleMaskMin, SampleMaskMax, bRenormalizeClippedSamples);
		}
	}
}
	...
	FSceneRenderTargetIndex FilterColorIndexQuarterRes, FLOAT QuarterResScale)
{
	...
	// @igs(jgm) fast clear here to avoid garbage in the 1 pixel border caused by swapping the buffers instead of copying
	RHIClear(TRUE,FLinearColor(0,0,0,0),FALSE,0,FALSE,0);
#endif
