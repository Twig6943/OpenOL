...
void USeqAct_Interp::Initialize()
{
	...
	if (bIsPlaying && !bPlayThroughWorldReset)
	{
		// rcharpentier - stop all playing matinees if the level gets unloaded and reloaded
		SetPosition(0.0f, TRUE);
		Stop();
	}		
...
void USeqAct_Interp::ForceActivateInput(INT InputIdx)
{
	Super::ForceActivateInput(InputIdx);
}
...
void USeqAct_Interp::Activated()
{
	...
	if ((!bClientSideOnly || GWorld->GetNetMode() != NM_DedicatedServer) && (InputLinks(0).bHasImpulse || InputLinks(1).bHasImpulse || InputLinks(4).bHasImpulse))
	{
		...
		if( InputLinks(0).bHasImpulse)
		{
			...
			{
				warnf(TEXT("### Matinee has both Play and Stop activated at the same frame - ignoring Play"));
			}
			else
			{
				Play();
		...
		// Always notify PlayerControllers directly. When a DirectorGroup is present the PC
		// is bound to it and receives this via the LatentActors/objectVars loop above.
		// When the Matinee uses a plain SeqVar_Object Player group (Pawn variable), the PC
		// never appears in objectVars, so we must notify it here for both observer-only and
		// normal playback — otherwise MATINEE_PLAY is never sent to the remote player.
		{
			UBOOL bHasDirectorGroup = FALSE;
			for (INT g = 0; g < GroupInst.Num(); g++)
			{
				if (Cast<UInterpGroupInstDirector>(GroupInst(g)))
				{
					bHasDirectorGroup = TRUE;
					break;
				}
			}
			if (!bHasDirectorGroup && !bObserverOnly)
			{
				for (AController* C = GWorld->GetWorldInfo()->ControllerList; C != NULL; C = C->NextController)
				{
					APlayerController* PC = Cast<APlayerController>(C);
					if (PC != NULL)
						PC->eventInterpolationStarted(this, NULL);
				}
			}
		}
...
UBOOL USeqAct_Interp::UpdateOp(FLOAT deltaTime)
{
	...
	else if( InputLinks(0).bHasImpulse)
	{
		if (InputLinks(2).bHasImpulse)
		{
			warnf(TEXT("### Matinee has both Play and Stop activated at the same frame - STOPPING"));
			Stop();
		}
		else
		{
...
void USeqAct_Interp::DeActivated()
{
	...
	UBOOL bHasPCGroup = FALSE;
	for (INT i = 0; i < LatentActors.Num(); i++)
	{
		if (Cast<APlayerController>(LatentActors(i)))
		{
			bHasPCGroup = TRUE;
			break;
		}
	...
	// Do any interpolation sequence cleanup — resets bObserverOnly to FALSE.
	TermInterp();

	// Notify PCs that had no PC group. For observer-only Matinees we do this AFTER
	// TermInterp() so bObserverOnly is already FALSE when the notification fires.
	// This prevents the MultiplayerHero super chain from re-entering MultiplayerController
	// via the "InterpAction.bObserverOnly" guard that was in the old Hero code.
	if (!bHasPCGroup)
	{
		for (AController* C = GWorld->GetWorldInfo()->ControllerList; C != NULL; C = C->NextController)
		{
			APlayerController* PC = Cast<APlayerController>(C);
			if (PC != NULL)
				PC->eventInterpolationFinished(this);
		}
	}
...
void USeqAct_Interp::InitInterp()
{
	...
	if(InterpData)
	{
		...
		// If this Matinee was triggered remotely, force observer-only so PC is never bound.
		if( GIsGame && bRemoteObserverMatinee )
		{
			bObserverOnly = TRUE;
		}
		// If all local PCs are excluded from Kismet (observer mode), mark this Matinee as
		// observer-only before binding groups. This handles Matinees that use a plain
		// SeqVar_Object Player group instead of a DirectorGroup.
		else if( GIsGame && !bObserverOnly )
		{
			UBOOL bAllPCsExcluded = TRUE;
			for (AController *TmpCtrl = GWorld->GetFirstController(); TmpCtrl != NULL; TmpCtrl = TmpCtrl->NextController)
			{
				APlayerController* TmpPC = Cast<APlayerController>(TmpCtrl);
				if (TmpPC != NULL && IsMatineeCompatibleWithPlayer(TmpPC) && !TmpPC->bExcludeFromKismetPlayer)
				{
					bAllPCsExcluded = FALSE;
					break;
				}
			}
			if (bAllPCsExcluded)
		...
		for(INT i=0; i<InterpData->InterpGroups.Num(); i++)
		{
			...
			if(DirGroup)
			{
				...
				if(GIsGame)
				{
					...
					// Observer-only Matinee: skip all PlayerController binding so the remote
					// player isn't pulled into the cutscene camera.
					if( bObserverOnly )
					{
					...
					// If all local PCs are excluded (e.g. remote Matinee triggered via Kismet),
					// mark this Matinee as observer-only so non-director groups also skip binding.
					UBOOL bAllPCsExcluded = TRUE;
					for (AController *TmpCtrl = GWorld->GetFirstController(); TmpCtrl != NULL; TmpCtrl = TmpCtrl->NextController)
					{
						APlayerController* TmpPC = Cast<APlayerController>(TmpCtrl);
					...
					for (AController *Controller = GWorld->GetFirstController(); Controller != NULL; Controller = Controller->NextController)
					{
						...
						// If it's a player, this sequence is compatible, and the player isn't
						// excluded from Kismet binding (e.g. multiplayer observer)...
						if (PC != NULL && IsMatineeCompatibleWithPlayer( PC ) && !PC->bExcludeFromKismetPlayer)
						{
			...
			else
			{
				if( !Group->bIsFolder )
				{
					...
					for(INT j=0; j<ObjectVars.Num() || j==0; j++)
					{
						...
						// Skip player actors in observer-only Matinees or excluded controllers.
						if( Actor != NULL )
						{
							// Case 1: observer-only Matinee — replace PC/Pawn with NULL so Event
							// tracks in the group still tick (Event track ignores GroupActor).
							if( bObserverOnly )
							{
								APlayerController* ActorAsPC = Cast<APlayerController>(Actor);
								APawn* ActorAsPawn = Cast<APawn>(Actor);
								if( ActorAsPC || (ActorAsPawn && Cast<APlayerController>(ActorAsPawn->Controller)) )
									Actor = NULL;
							}
							// Case 2: actor is a PlayerController itself
							if( Actor != NULL )
							{
								APlayerController* ActorAsPC = Cast<APlayerController>(Actor);
								if( ActorAsPC && ActorAsPC->bExcludeFromKismetPlayer )
									Actor = NULL;
							}
							// Case 3: actor is a Pawn whose controller is excluded
							if( Actor != NULL )
							{
								APawn* ActorAsPawn = Cast<APawn>(Actor);
								if( ActorAsPawn && ActorAsPawn->Controller )
								{
									APlayerController* ActorPC = Cast<APlayerController>(ActorAsPawn->Controller);
									if( ActorPC && ActorPC->bExcludeFromKismetPlayer )
										Actor = NULL;
								}
							}
						}
...
void USeqAct_Interp::TermInterp()
{
	...
	// Reset observer-only flag so this Matinee can be re-used by local player later.
	bObserverOnly = FALSE;

	// Clear remote observer flag so this Matinee can be re-triggered locally later.
	bRemoteObserverMatinee = FALSE;
...
void UInterpTrackFloatProp::UpdateTrack(FLOAT NewPosition, UInterpTrackInst* TrInst, UBOOL bJump)
{
	...
	FLOAT OldFloatValue = *((FLOAT*)(PropInst->FloatProp));
	FLOAT NewFloatValue = FloatTrack.Eval( NewPosition, OldFloatValue );
	*((FLOAT*)(PropInst->FloatProp)) = NewFloatValue;
	...
	{
		// mathieu.gauthier: we limit this optimization to lights only because
		// i am not 100% sure of all the ramifications, but for lights its
		// probably safe.
		...
		if (Light != NULL)
		{
			...
			{
				// You dont want to update a light, since it triggers a remove/add
				// on the render thread side. Suuuuper expensive.
				if (PropertyName == NAME_Brightness)
				{
					Light->LightComponent->GetScene()->UpdateLightColorAndBrightness(Light->LightComponent);
				}
				else 
				{
...
void UInterpTrackColorProp::UpdateTrack(FLOAT NewPosition, UInterpTrackInst* TrInst, UBOOL bJump)
{
	...
	*((FColor*)(PropInst->ColorProp)) = NewColorValue;
	...
	if(PropInst->CallPropertyUpdateCallback()==FALSE)
	{
		// mathieu.gauthier: detaching and attaching a light is super expensive since
		// it requires rebuilding all the light interactions.
		ALight* Light = Cast<ALight>(Actor);
		if (Light != NULL)
		{
			Light->LightComponent->GetScene()->UpdateLightColorAndBrightness(Light->LightComponent);
...
void UInterpTrackFade::UpdateTrack(FLOAT NewPosition, UInterpTrackInst* TrInst, UBOOL bJump)
{
	...
	if (!bJump || !GIsGame)
	{
		...
		// Actor for a Director group should be a PlayerController.
		AActor* GroupActor = GrInst->GetGroupActor();
		APlayerController* PC = GroupActor ? GroupActor->GetAPlayerController() : NULL;
		if (PC)
		{
			FLOAT fadeAmount = GetFadeAmountAtTime(NewPosition);
			if (PC->PlayerCamera && !PC->PlayerCamera->bDeleteMe)
			{
				PC->PlayerCamera->bEnableFading = true;
				PC->PlayerCamera->FadeAmount = fadeAmount;
				// disable the Kismet fade control so that we don't thrash
				PC->PlayerCamera->FadeTimeRemaining = 0.0f;
			}
			
			// rcharpentier - allow camera effects without using the PlayerCamera
			PC->bEnableFading = true;
			PC->FadeAmount = fadeAmount;
			PC->FadeTimeRemaining = 0.0f;
		}
	}
}
...
void UInterpTrackInstFade::TermTrackInst(UInterpTrack* Track)
{
	...
	if (FadeTrack && !FadeTrack->bPersistFade)
	{
		UInterpGroupInst* GrInst = CastChecked<UInterpGroupInst>( GetOuter() );
...
{
	if (bDisableWhenNoActiveAnim)
	{
		if (AnimSeqs.Num() == 0 || InTime < AnimSeqs(0).StartTime)
		{
			return 0.0f; // before 1st anim
		}
		else
		{
			for (INT i = 0; i < AnimSeqs.Num(); i++)
			{
				if ((AnimSeqs.Num() > i + 1) && AnimSeqs(i+1).StartTime <= InTime)
				{
					// skip to next
					continue;
				}
				
				// We're either during AnimSeqs(i) or after it, but before the next one

				UAnimSequence* activeSeq = FindAnimSequenceFromName(AnimSeqs(i).AnimSeqName);
				if (activeSeq)
				{
					FLOAT elapsedTime = InTime - AnimSeqs(i).StartTime;
					FLOAT seqLength = ::Max(activeSeq->SequenceLength - (AnimSeqs(i).AnimStartOffset + AnimSeqs(i).AnimEndOffset), 0.01f);

					FLOAT fadeInPct = 1.0f;
					FLOAT fadeOutPct = 1.0f;

					if (BlendInTime > 0.0f)
					{
						fadeInPct = Clamp(elapsedTime / BlendInTime, 0.0f, 1.0f);
					}

					if (BlendOutTime > 0.0f)
					{
						fadeOutPct = Clamp((seqLength - elapsedTime) / BlendOutTime, 0.0f, 1.0f);
					}

					return Min(fadeInPct, fadeOutPct);
				}
			}
		}
	}
	else if (AnimSeqs.Num() > 0 && AnimSeqs(AnimSeqs.Num() - 1).AnimSeqName == FName(TEXT("SE_Miles_PriestIntro_StandingUp_01")))
	{
		// can't be a more obvious hack than this - to allow blending at the end of the main hall throwdown

		// This code should be generalized to allow blending out at the end, even if middle sections can't use the bDisableWhenNoActiveAnim

		FLOAT lastAnimIdx = AnimSeqs.Num() - 1;

		if (InTime > AnimSeqs(lastAnimIdx).StartTime)
		{
			UAnimSequence* activeSeq = FindAnimSequenceFromName(AnimSeqs(lastAnimIdx).AnimSeqName);
			if (activeSeq)
			{
				FLOAT elapsedTime = InTime - AnimSeqs(lastAnimIdx).StartTime;
				FLOAT seqLength = activeSeq->SequenceLength;
				FLOAT fadeOutPct = 1.0f;
		...
		while(CurrentSeqIndex <= EndSeqIndex)
		{
			...
			else
			{
				...
				else
				{
					...
					if( CurrentAnimName != NAME_None )
					{
						UBOOL bWithRootMotion = bEnableRootMotion || (bActivePositionAdjustment && NewPosition < BlendInTime);
						...
						Actor->eventSetAnimPosition(SlotName, ChannelIndex, CurrentAnimName, AnimPos, (AnimPos != CurrentStartOffset)?!bSkipAnimNotifiers:FALSE , FALSE, bWithRootMotion);

						if (bActivePositionAdjustment && NewPosition >= BlendInTime)
						{
							AActor* effectiveActor = (Actor->GetAController() ? Actor->GetAController()->Pawn : Actor);
							ApplyPositionCorrection(effectiveActor, NewPosition, AnimInst->RefPos, AnimInst->RefRot);
						}
					}
				}
...
void UInterpTrackAnimControl::ApplyPositionCorrection(AActor* actor, FLOAT newPosition, const FVector& refPos, const FRotator& refRot)
{
	// Get the skelcomp
	...
	{
		ASkeletalMeshActor* sma = Cast<ASkeletalMeshActor>(actor);
		if (sma)
		{
			skelComp = sma->SkeletalMeshComponent;
		}
	}
	...
	FMatrix precisePos = FRotationTranslationMatrix(refRot, refPos);

	INT lastAnimIdx = AnimSeqs.Num()-1;

	// Find the last relevant animation
	for (INT seqIdx = 0; seqIdx < AnimSeqs.Num(); seqIdx++)
	{
		FAnimControlTrackKey &animSeq = AnimSeqs(seqIdx);
		
		if (newPosition < animSeq.StartTime)
		{
			lastAnimIdx = seqIdx - 1;
			break;
		}
	}

	// Accumulate root motion from the previous anims
	for (INT seqIdx = 0; seqIdx <= lastAnimIdx; seqIdx++)
	{
		FAnimControlTrackKey &animSeq = AnimSeqs(seqIdx);
		FBoneAtom rootMotion = FBoneAtom::Identity;
		
		UAnimSequence* seq = FindAnimSequenceFromName(animSeq.AnimSeqName);		

		if (seq)
		{			
			FLOAT endRatio = (seqIdx == lastAnimIdx) ? ((newPosition - animSeq.StartTime) / seq->SequenceLength) : 1.0f;

			FBoneAtom meshToCompSpace(FRotationMatrix(skelComp->SkeletalMesh->RotOrigin));
			
			const INT AnimLinkupIndex = seq->GetAnimSet()->GetMeshLinkupIndex(skelComp->SkeletalMesh);
			FAnimSetMeshLinkup* AnimLinkup = &seq->GetAnimSet()->LinkupCache(AnimLinkupIndex);
			const INT TrackIndex = AnimLinkup->BoneToTrackTable(0);

			FBoneAtom firstFrameAtom;
			FBoneAtom lastFrameAtom;
			seq->GetBoneAtom(firstFrameAtom, TrackIndex, 0.0f, FALSE, skelComp->bUseRawData);	
			seq->GetBoneAtom(lastFrameAtom, TrackIndex, endRatio * seq->SequenceLength, FALSE, skelComp->bUseRawData);

			FBoneAtom delta = (firstFrameAtom.Inverse() * lastFrameAtom);
			rootMotion = delta * meshToCompSpace;
			rootMotion.SetRotation(delta.GetRotation());

			
			/*UAnimNodeSequence* animSeqNode = Cast<UAnimNodeSequence>(skelComp->Animations);
			if (animSeqNode)
			{
				animSeqNode->SetRootBoneAxisOption(RBA_Discard, RBA_Discard, RBA_Discard);
				animSeqNode->SetRootBoneRotationOption(RRO_Discard, RRO_Discard, RRO_Discard);
			}*/

		}

		precisePos = rootMotion.ToMatrix() * precisePos;
	}

	FVector deltaPos = precisePos.GetOrigin() - actor->Location;
	
	FCheckResult Hit(1.f);
	GWorld->MoveActor(actor, deltaPos, precisePos.Rotator(), MOVE_NoFail, Hit);
}
...
void UInterpTrackInstAnimControl::InitTrackInst(UInterpTrack* Track)
{
	...
	// rcharpentier
	AActor * groupActor = GetGroupActor();
	if (groupActor && AnimTrack)
	{
		if (AnimTrack->bActivePositionAdjustment)
		{		
			UInterpGroupInstAI* aiGroupInst = Cast<UInterpGroupInstAI>( GetOuter() );
			if (aiGroupInst && aiGroupInst->StageMarkActor)
			{
				RefPos = aiGroupInst->StageMarkActor->Location;
				RefRot = aiGroupInst->StageMarkActor->Rotation;
			}
			else
			{
				RefPos = groupActor->Location;
				RefRot = groupActor->Rotation;
			}

			USkeletalMeshComponent* skelComp = NULL;
			APawn* pawn = groupActor->GetAController() ? groupActor->GetAController()->Pawn : Cast<APawn>(groupActor);
			if (pawn)
			{
				skelComp = pawn->Mesh;
			}
			...
			{
				ASkeletalMeshActor* sma = Cast<ASkeletalMeshActor>(groupActor);
				if (sma)
				{
					skelComp = sma->SkeletalMeshComponent;
				}
			...
			if (skelComp)
			{
				bWasDiscardingRootMotion = skelComp->bForceDiscardRootMotion;
				skelComp->bForceDiscardRootMotion = TRUE;
			}		
		}
		else if (AnimTrack->bEnableRootMotion)
		{			
			// Set root motion mode on the skel mesh actor if we want root motion
			// Not sure how it's supposed to work otherwise.	
			if ( groupActor->IsA(ASkeletalMeshActor::StaticClass()) )
			{
				ASkeletalMeshActor * SMA = CastChecked<ASkeletalMeshActor>(groupActor);
				if ( SMA && SMA->SkeletalMeshComponent )
				{
					SMA->SkeletalMeshComponent->RootMotionMode = RMM_Translate;
					SMA->SkeletalMeshComponent->RootMotionRotationMode = RMRM_RotateActor; 
				}
			}
		}
	}
...
void UInterpTrackInstAnimControl::TermTrackInst(UInterpTrack* Track)
{
	...
	if (Actor && AnimTrack)
	{
		...
		APawn* pawn = Actor->GetAController() ? Actor->GetAController()->Pawn : Cast<APawn>(Actor);
		if (pawn && pawn->Mesh)
		{
		...
		if ( SkeletalMeshComponent )
		{
			if (AnimTrack->bActivePositionAdjustment)
			{	
				SkeletalMeshComponent->bForceDiscardRootMotion = bWasDiscardingRootMotion;
			}
			else if (AnimTrack->bEnableRootMotion)
			{
...
void UInterpTrackFaceFX::GetSeqInfoForTime( FLOAT InTime, FString& OutGroupName, FString& OutSeqName, FLOAT& OutPosition, FLOAT& OutSeqStart, USoundCue*& OutSoundCue, UAkEvent*& OutAkEvent)
// WWISEMODIF_END
{
...
// WWISEMODIF_START, alessard, nov-28-2008, WwiseAudioIntegration
			OutAkEvent = FaceFXSoundCueKeys( i ).FaceFXAkEvent;
// WWISEMODIF_END
		}
	}
}
...
// WWISEMODIF_START, alessard, nov-28-2008, WwiseAudioIntegration
		UAkEvent* Event = NULL;
		GetSeqInfoForTime( NewPosition, GroupName, SeqName, Position, StartPos, SoundCue, Event );
// WWISEMODIF_END
...
// WWISEMODIF_START, alessard, nov-28-2008, WwiseAudioIntegration
		UAkEvent* OldEvent = NULL;
		GetSeqInfoForTime( FaceFXTrInst->LastUpdatePosition, OldGroupName, OldSeqName, DummyPos, OldPos, OldSoundCue, OldEvent );

		FLOAT NewPos;
		FString NewGroupName, NewSeqName;
		USoundCue* NewSoundCue = NULL;
		UAkEvent* NewEvent = NULL;
		GetSeqInfoForTime( NewPosition, NewGroupName, NewSeqName, DummyPos, NewPos, NewSoundCue, NewEvent );
			...
			NewSoundCue != OldSoundCue)
		{
			...
			if(GIsEditor && !GWorld->HasBegunPlay())
			{
				Actor->PreviewActorPlayFaceFX(NewGroupName, NewSeqName, NewEvent ? (UAkBaseSoundObject*) NewEvent : (UAkBaseSoundObject*) NewSoundCue );
			}
			else
			{
				Actor->eventPlayActorFaceFXAnim(NULL, NewGroupName, NewSeqName, NewSoundCue, NewEvent, Priority);
			}
...
{
#if WITH_FACEFX
#ifdef _WITH_WWISE_FACEFX_MODIFS_
	using namespace OC3Ent;
	using namespace Face;
...
// WWISEMODIF_START, alessard, nov-11-2008, WwiseAudioIntegration
			...
			if ( fActor )
			{
				...
				if(FxInvalidIndex != GroupIndex)
				{
					...
					if(FxInvalidIndex != SeqIndex)
					{
						...
						FxString EventPath = fAnim.GetAkEventPath();
						if( EventPath.Length() > 0 )
						{
							NewAkEvent = LoadObject<UAkEvent>(NULL, ANSI_TO_TCHAR(EventPath.GetData()), NULL, LOAD_NoWarn, NULL);
						}
					}
				}
			...
			if( SoundCueKey.FaceFXAkEvent != NewAkEvent )
			{
				SoundCueKey.FaceFXAkEvent = NewAkEvent;
				
				// Mark the package dirty so that the new AkEvent reference will be resaved!
				MarkPackageDirty();
			}
// WWISEMODIF_END
		}
...
void UInterpGroupAI::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyThatChanged)
	{
		...
		else if ( PropertyName == TEXT("StageMarkGroup") || PropertyName == TEXT("bNoEncroachmentCheck") )
		{
			// refresh variable, so that it can refresh info when next update
...
void UInterpGroupInstAI::InitGroupInst(UInterpGroup* InGroup, AActor* InGroupActor)
{
	...
	// Exception: observer-only Matinee always initialises immediately with NULL so
	// Event tracks can still tick (GroupActor not needed for event firing).
	if ( GIsGame && AIGroup && InGroupActor == NULL )
	{
		USeqAct_Interp* Seq = Cast<USeqAct_Interp>( GetOuter() );
		if ( !Seq || !Seq->bObserverOnly )
			return;
		// Observer-only with NULL actor — fall through to Super::InitGroupInst(NULL)
		// so TrackInst are created and Event tracks can tick.
	}

	// Observer-only Matinee: null-bind player actors in AIGroups (e.g. "Player" group)
	// so the remote observer isn't pulled into the cutscene camera.
	// Skip if already initialised to preserve TrackInst state (LastUpdatePosition etc).
	if ( GIsGame && InGroupActor != NULL )
	{
		if ( TrackInst.Num() > 0 )
			return;
		USeqAct_Interp* Seq = Cast<USeqAct_Interp>( GetOuter() );
		if ( Seq && Seq->bObserverOnly )
		{
			APlayerController* ActorAsPC = Cast<APlayerController>(InGroupActor);
			APawn* ActorAsPawn = Cast<APawn>(InGroupActor);
			if ( ActorAsPC || (ActorAsPawn && Cast<APlayerController>(ActorAsPawn->Controller)) )
			{
					InGroupActor = NULL;
			}
	...
	if (PawnActor)
	{
		if (StageMarkActor)
		{
			...
			PawnActor->eventMAT_BeginAIGroup(StageMarkActor->Location, StageMarkActor->Rotation, AIGroup->StageMarkBlendInTime);
		}
		else
		{
			// rcharpentier - not sure why, sometimes the timing is off and the stage mark isn't yet initialized
			PawnActor->eventMAT_BeginAIGroupNoStageMark();
		}
	}
}
...
void UInterpGroupAI::UpdateGroup(FLOAT NewPosition, UInterpGroupInst* GrInst, UBOOL bPreview, UBOOL bJump)
{
	...
	USeqAct_Interp* SeqInterpForAttach = Cast<USeqAct_Interp>(GrInst->GetOuter());
	UBOOL bTryAttachActor = (GIsGame && GrInst && GrInst->GetGroupActor() == NULL
		&& !(SeqInterpForAttach && SeqInterpForAttach->bObserverOnly));
	if ( bTryAttachActor )
	{
		// search for it
		USeqAct_Interp * SeqInterp = Cast<USeqAct_Interp>(GrInst->GetOuter());
...
//				debugf(TEXT("AIGroup: Adding Actor %s to the group %s"), *Actor->GetName(), *GroupName.GetNameString());
	...
	{
		// Still tick tracks that don't require a GroupActor (e.g. Event tracks),
		// but only if TrackInst are already initialised (InterpTracks count must match).
		USeqAct_Interp* SeqInterp = Cast<USeqAct_Interp>(GrInst->GetOuter());
		if ( SeqInterp && SeqInterp->bObserverOnly && GrInst->TrackInst.Num() == InterpTracks.Num() )
		{
			Super::UpdateGroup( NewPosition, GrInst, bPreview, bJump );
		}
		return;
