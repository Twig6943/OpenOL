...
// Very hacky. Engine should not know our derived engine...
#include "OLGame.h"

IMPLEMENT_CLASS(UUberPostProcessEffect);
IMPLEMENT_CLASS(UOLUberPostProcessEffect);
...
public:
	...
	FShaderParameter				UVScaleBiasParameter;
		...
		,	MotionBlurShaderParameters(Initializer.ParameterMap)
	{
		...
		UVScaleBiasParameter.Bind(Initializer.ParameterMap,TEXT("UVScaleBias"),TRUE);
	...
	virtual UBOOL Serialize(FArchive& Ar)
	{
		...
		Ar	<< LowResPostProcessBuffer << ImageAdjustments1Parameter << ImageAdjustments2Parameter
			...
			<< UVScaleBiasParameter << SceneCoordinate1ScaleBiasParameter << SceneCoordinate2ScaleBiasParameter << SceneCoordinate3ScaleBiasParameter;
	...
	{
		FDOFAndBloomBlendPixelShader::RebindParameters(ParameterMap);
		LowResPostProcessBuffer.Bind(ParameterMap, TEXT("LowResPostProcessBuffer"), TRUE);
		ImageAdjustments1Parameter.Bind(ParameterMap, TEXT("ImageAdjustments1"), TRUE);
		ImageAdjustments2Parameter.Bind(ParameterMap, TEXT("ImageAdjustments2"), TRUE);
		ImageAdjustments3Parameter.Bind(ParameterMap, TEXT("ImageAdjustments3"), TRUE);
		ColorGradingLUTTextureParameter.Bind(ParameterMap, TEXT("ColorGradingLUT"), TRUE);
		HalfResMaskRectParameter.Bind(ParameterMap, TEXT("HalfResMaskRect"), TRUE);
		BokehDOFLayerTextureParameter.Bind(ParameterMap, TEXT("BokehDOFLayerTexture"), TRUE);
		MotionBlurShaderParameters = FMotionBlurShaderParameters(ParameterMap);
		NoiseTextureParameter.Bind(ParameterMap, TEXT("NoiseTexture"), TRUE);
		UVScaleBiasParameter.Bind(ParameterMap, TEXT("UVScaleBias"), TRUE);
		SceneCoordinate1ScaleBiasParameter.Bind(ParameterMap, TEXT("SceneCoordinate1ScaleBias"), TRUE);
		SceneCoordinate2ScaleBiasParameter.Bind(ParameterMap, TEXT("SceneCoordinate2ScaleBias"), TRUE);
		SceneCoordinate3ScaleBiasParameter.Bind(ParameterMap, TEXT("SceneCoordinate3ScaleBias"), TRUE);
	}
};
...
template<UINT CameraMode, UBOOL bCameraShattered, UBOOL bMovieEffect>
class TOLUberPostProcessBlendPixelShader : public FUberPostProcessBlendPixelShaderBase
{
	DECLARE_SHADER_TYPE( TOLUberPostProcessBlendPixelShader, Global );

	static void ModifyCompilationEnvironment( EShaderPlatform Platform, FShaderCompilerEnvironment& OutEnvironment )
	{
		FUberPostProcessBlendPixelShaderBase::ModifyCompilationEnvironment( Platform, OutEnvironment, DOFType_SimpleDOF, Tonemapper_Off, FALSE, FALSE, FALSE );
		OutEnvironment.Definitions.Set(TEXT("OUTLAST_UBER"), TEXT("1"));
		OutEnvironment.Definitions.Set(TEXT("OUTLAST_CAMERA_MODE"), *FString::Printf(TEXT("%u"), CameraMode));
		OutEnvironment.Definitions.Set(TEXT("OUTLAST_CAMERA_SHATTERED"), bCameraShattered ? TEXT("1") : TEXT("0"));
		OutEnvironment.Definitions.Set(TEXT("OUTLAST_MOVIE_EFFECT"), bMovieEffect ? TEXT("1") : TEXT("0"));
	}

protected:
	/** Default constructor. */
	TOLUberPostProcessBlendPixelShader() {}

public:
	/** Initialization constructor. */
	TOLUberPostProcessBlendPixelShader(const ShaderMetaType::CompiledShaderInitializerType& Initializer)
		:	FUberPostProcessBlendPixelShaderBase(Initializer)
	{
		VignetteParameter.Bind(Initializer.ParameterMap,TEXT("VignetteParams"),TRUE);
		GrainTextureParameter.Bind(Initializer.ParameterMap, TEXT("GrainTexture"), TRUE );
		GrainParameter.Bind(Initializer.ParameterMap,TEXT("GrainParams"),TRUE);
		HurtCircleParameter.Bind(Initializer.ParameterMap,TEXT("HurtCircleParams"),TRUE);
		HurtColorParameter.Bind(Initializer.ParameterMap,TEXT("HurtColor"),TRUE);
		HurtParameter.Bind(Initializer.ParameterMap,TEXT("HurtParams"),TRUE);
		CameraColorParameter.Bind(Initializer.ParameterMap,TEXT("CameraColor"),TRUE);
		CameraDistortionAndEffectStrengthParameter.Bind(Initializer.ParameterMap,TEXT("CameraDistortionAndEffectStrength"),TRUE);
		CameraGlassDiffuseTextureParameter.Bind(Initializer.ParameterMap,TEXT("CameraGlassDiffuseTexture"),TRUE);
		CameraGlassNormalTextureParameter.Bind(Initializer.ParameterMap,TEXT("CameraGlassNormalTexture"),TRUE);
		CameraGlassCubeMapTextureParameter.Bind(Initializer.ParameterMap,TEXT("CameraGlassCubeMapTexture"),TRUE);
		ViewToWorldParameter.Bind(Initializer.ParameterMap,TEXT("ViewToWorldMatrix"),TRUE);
		CameraGlassLightColorParameter.Bind(Initializer.ParameterMap,TEXT("CameraGlassLightColor"),TRUE);
		MovieIntensityParameter.Bind(Initializer.ParameterMap,TEXT("MovieIntensity"),TRUE);
		MovieColorLoParameter.Bind(Initializer.ParameterMap,TEXT("MovieColorLo"),TRUE);
		MovieColorHiParameter.Bind(Initializer.ParameterMap,TEXT("MovieColorHi"),TRUE);
		MovieColorParameter.Bind(Initializer.ParameterMap,TEXT("MovieColor"),TRUE);
		MovieTextureParameter.Bind(Initializer.ParameterMap,TEXT("MovieTexture"),TRUE);
		MovieGradTextureParameter.Bind(Initializer.ParameterMap,TEXT("MovieGradTexture"),TRUE);
		MovieLightMaskTextureParameter.Bind(Initializer.ParameterMap,TEXT("MovieLightMaskTexture"),TRUE);
	}

	// FShader interface.
	virtual UBOOL Serialize(FArchive& Ar)
	{
		UBOOL bShaderHasOutdatedParameters = FUberPostProcessBlendPixelShaderBase::Serialize(Ar);
		Ar	<< GrainTextureParameter << VignetteParameter << GrainParameter << HurtCircleParameter << HurtColorParameter << HurtParameter
			<< CameraColorParameter << CameraDistortionAndEffectStrengthParameter
			<< CameraGlassDiffuseTextureParameter << CameraGlassNormalTextureParameter << CameraGlassCubeMapTextureParameter << CameraGlassLightColorParameter
			<< ViewToWorldParameter << MovieIntensityParameter << MovieColorLoParameter << MovieColorHiParameter << MovieColorParameter 
			<< MovieTextureParameter << MovieGradTextureParameter << MovieLightMaskTextureParameter;
	...
	virtual void RebindParameters(const FShaderParameterMap& ParameterMap)
	{
		FUberPostProcessBlendPixelShaderBase::RebindParameters(ParameterMap);
		VignetteParameter.Bind(ParameterMap, TEXT("VignetteParams"), TRUE);
		GrainTextureParameter.Bind(ParameterMap, TEXT("GrainTexture"), TRUE);
		GrainParameter.Bind(ParameterMap, TEXT("GrainParams"), TRUE);
		HurtCircleParameter.Bind(ParameterMap, TEXT("HurtCircleParams"), TRUE);
		HurtColorParameter.Bind(ParameterMap, TEXT("HurtColor"), TRUE);
		HurtParameter.Bind(ParameterMap, TEXT("HurtParams"), TRUE);
		CameraColorParameter.Bind(ParameterMap, TEXT("CameraColor"), TRUE);
		CameraDistortionAndEffectStrengthParameter.Bind(ParameterMap, TEXT("CameraDistortionAndEffectStrength"), TRUE);
		CameraGlassDiffuseTextureParameter.Bind(ParameterMap, TEXT("CameraGlassDiffuseTexture"), TRUE);
		CameraGlassNormalTextureParameter.Bind(ParameterMap, TEXT("CameraGlassNormalTexture"), TRUE);
		CameraGlassCubeMapTextureParameter.Bind(ParameterMap, TEXT("CameraGlassCubeMapTexture"), TRUE);
		ViewToWorldParameter.Bind(ParameterMap, TEXT("ViewToWorldMatrix"), TRUE);
		CameraGlassLightColorParameter.Bind(ParameterMap, TEXT("CameraGlassLightColor"), TRUE);
		MovieIntensityParameter.Bind(ParameterMap, TEXT("MovieIntensity"), TRUE);
		MovieColorLoParameter.Bind(ParameterMap, TEXT("MovieColorLo"), TRUE);
		MovieColorHiParameter.Bind(ParameterMap, TEXT("MovieColorHi"), TRUE);
		MovieColorParameter.Bind(ParameterMap, TEXT("MovieColor"), TRUE);
		MovieTextureParameter.Bind(ParameterMap, TEXT("MovieTexture"), TRUE);
		MovieGradTextureParameter.Bind(ParameterMap, TEXT("MovieGradTexture"), TRUE);
		MovieLightMaskTextureParameter.Bind(ParameterMap, TEXT("MovieLightMaskTexture"), TRUE);
	}

	FShaderResourceParameter	GrainTextureParameter;
	FShaderParameter			VignetteParameter;
	FShaderParameter			GrainParameter;
	FShaderParameter			HurtCircleParameter;
	FShaderParameter			HurtColorParameter;
	FShaderParameter			HurtParameter;
	FShaderParameter			CameraColorParameter;
	FShaderParameter			CameraDistortionAndEffectStrengthParameter;
	FShaderParameter			ViewToWorldParameter;
	FShaderParameter			CameraGlassLightColorParameter;
	FShaderResourceParameter	CameraGlassDiffuseTextureParameter;
	FShaderResourceParameter	CameraGlassNormalTextureParameter;
	FShaderResourceParameter	CameraGlassCubeMapTextureParameter;
	FShaderParameter			MovieIntensityParameter;
	FShaderParameter			MovieColorLoParameter;
	FShaderParameter			MovieColorHiParameter;
	FShaderParameter			MovieColorParameter;
	FShaderResourceParameter	MovieTextureParameter;
	FShaderResourceParameter	MovieGradTextureParameter;
	FShaderResourceParameter	MovieLightMaskTextureParameter;
};
...
#define VARIATION2(A,B)		VARIATION3(A,B,0)		VARIATION3(A,B,1)
#define VARIATION3(A,B,C) typedef TOLUberPostProcessBlendPixelShader<A, B, C> FOLUberPostProcessBlendPixelShader##A##B##C; \
	IMPLEMENT_SHADER_TYPE2(template<>, FOLUberPostProcessBlendPixelShader##A##B##C, SF_Pixel, 0, 0);
...
public:
	...
	{
		FDOFAndBloomBlendPixelShader::RebindParameters(ParameterMap);
		MotionBlurParameters = FMotionBlurShaderParameters(ParameterMap);
		LowResPostProcessBufferPointParameter.Bind(ParameterMap, TEXT("LowResPostProcessBufferPoint"), TRUE);
		BokehDOFLayerTextureParameter.Bind(ParameterMap, TEXT("BokehDOFLayerTexture"), TRUE);
	}
};
...
public:
	...
	*/
	...
	void RenderVariationHalfRes(FViewInfo& View, EDOFType LocalDepthOfFieldType, FLOAT EffectSizeScale, UBOOL bSeparateTranslucency)
	{
		...
		if (!*BlendVertexShader || !*BlendPixelShader) { return; }
		...
		UBOOL bSeparateTranslucency)
	{
		...
		if(bUseSoftEdgeMotionBlur)
		{
			...
			FSceneRenderTargetIndex MotionBlurIndex = SRTI_RGBFilterColor2;

#if (DINGO || ORBIS) && !FINAL_RELEASE
			extern UBOOL GEnableR11G11B10MotionBlur;
			if (!GEnableR11G11B10MotionBlur)
			{
				// Use 16-bit target if disabled
				MotionBlurIndex = SRTI_FilterColor2;
			}
#endif

			// render quarter res MotionBlur vector
			RenderGatherPass(View, EGD_MotionBlur, MotionBlurIndex, 1.0f);

			// set to renormalize in filter shader so that a 32-bit target can be used instead of using alpha channel to renormalize'
			// offset slightly to avoid errors from bilinear filtering. remove offset if switched to compute shader for filtering (no bilinear samples)
			FVector2D MotionBlurSampleMaskMin(1.2f / (FLOAT)GSceneRenderTargets.GetFilterBufferSizeX(), 1.2f / (FLOAT)GSceneRenderTargets.GetFilterBufferSizeY());
			FVector2D MotionBlurSampleMaskMax((GSceneRenderTargets.GetFilterBufferSizeX() - 1.2f) / (FLOAT)GSceneRenderTargets.GetFilterBufferSizeX(), (GSceneRenderTargets.GetFilterBufferSizeY() - 1.2f) / (FLOAT)GSceneRenderTargets.GetFilterBufferSizeY());
			GaussianBlurFilterBuffer(View,View.SizeX, DownsampledSizeX + AntiLeakBorder, DownsampledSizeY + AntiLeakBorder, MotionBlurSoftEdgeKernelSize, 1.0f, MotionBlurIndex, MotionBlurSampleMaskMin, MotionBlurSampleMaskMax, TRUE);

			// RenderTarget at MotionBlurIndex now has the blurred velocity vector content	
		}
	...
	*/
	...
	void RenderBloom(FViewInfo& View, FLOAT EffectSizeScale, UBOOL bSeparateTranslucency)
	{
		...
		RenderGatherPass(View, EGD_Bloom, SRTI_RGBFilterColor1, MAX_SCENE_COLOR, BloomQuality, bSeparateTranslucency);
		...
		if(BloomType == 0)
		{
			...
			// blur #1
			GaussianBlurFilterBuffer(View,View.SizeX, DownsampledSizeX + AntiLeakBorder, DownsampledSizeY + AntiLeakBorder, BloomKernelSizeMedium * EffectSizeScale, 1.0f, SRTI_RGBFilterColor1, SampleMaskMin, SampleMaskMaxA);
		}
		else if(BloomType == 1)
		{
			...
			{	
				...
				DrawDownsampledTexture(
					GSceneRenderTargets.GetFilterColorSurface(SRTI_RGBFilterColor2),
					GSceneRenderTargets.GetFilterColorTexture(SRTI_RGBFilterColor2),
					GSceneRenderTargets.GetFilterColorTexture(SRTI_RGBFilterColor1),
					Origin,
			...
			// * 0.5f on kernel size as this is 1/2 resolution
			{
				...
				GaussianBlurFilterBuffer(View,View.SizeX, DownsampledSizeX / 2 + AntiLeakBorder, DownsampledSizeY / 2 + AntiLeakBorder, Pass1KernelSize * EffectSizeScale * 0.5f, 1.0f, SRTI_RGBFilterColor2, SampleMaskMin, SampleMaskMaxB);
			}
			...
			// #1 = #1 * .. + #2 * ..
			{
				...
				CombineFilterBuffer(DownsampledSizeX + AntiLeakBorder, DownsampledSizeY + AntiLeakBorder, SRTI_RGBFilterColor1, BloomWeightSmall, SRTI_RGBFilterColor2, BloomWeightMedium);
			}
			...
			// blur #1
			{
				...
				GaussianBlurFilterBuffer(View,View.SizeX, DownsampledSizeX + AntiLeakBorder, DownsampledSizeY + AntiLeakBorder, BloomKernelSizeSmall * EffectSizeScale, 1.0f, SRTI_RGBFilterColor1, SampleMaskMin, SampleMaskMaxA);
			}
		}
		else if(BloomType == 2)
		{				
...
//@zombie_ps4_end
			...
			// downsample #2 to #0
			{
				...
				DrawDownsampledTexture(
					GSceneRenderTargets.GetFilterColorSurface(SRTI_RGBFilterColor0),
					GSceneRenderTargets.GetFilterColorTexture(SRTI_RGBFilterColor0),
					GSceneRenderTargets.GetFilterColorTexture(SRTI_RGBFilterColor2),
					Origin,
			...
			// * 0.25f on kernel size as this is 1/4 resolution
			{
				...
				GaussianBlurFilterBuffer(View,View.SizeX, DownsampledSizeX / 4 + AntiLeakBorder, DownsampledSizeY / 4 + AntiLeakBorder, Pass2KernelSize * EffectSizeScale * 0.25f, 1.0f, SRTI_RGBFilterColor0, SampleMaskMin, SampleMaskMaxC);
			}
			...
			// #1 = #1 * .. + #2 *.. + #0 * ..
			{
				...
				CombineFilterBuffer(DownsampledSizeX + AntiLeakBorder, DownsampledSizeY + AntiLeakBorder, SRTI_RGBFilterColor1, BloomWeightSmall, SRTI_RGBFilterColor2, BloomWeightMedium, SRTI_RGBFilterColor0, BloomWeightLarge);
			}
		...
		else if(BloomType == 3)
		{
			...
			for (INT BlurPassIndex = 0; BlurPassIndex < NumPasses; BlurPassIndex++)
			{
				...
				GaussianBlurFilterBuffer(View,View.SizeX, DownsampledSizeX + AntiLeakBorder, DownsampledSizeY + AntiLeakBorder, PassBlurKernel * EffectSizeScale * FilterDownsampleFactor, PassBlurScale, SRTI_RGBFilterColor1, SampleMaskMin, SampleMaskMaxA);
			}
		}
	...
	void RenderVariationFullRes(
		...
		ESamplerFilter SceneColorFilter = SF_Point)
	{
		// full resolution (this can be much bigger than the viewport needed size, half and quarter is computed from that size)
		const UINT BufferSizeX = GSceneRenderTargets.GetBufferSizeX();
		const UINT BufferSizeY = GSceneRenderTargets.GetBufferSizeY();
		...
		// Set the color LUT texture parameter (ColorGradingRHI may be NULL when no LUT is active)
		if (ColorGradingRHI)
		{
			SetTextureParameterDirectly(
				BlendPixelShader->GetPixelShader(),
		...
		// Setup the scene texture
		BlendPixelShader->SceneTextureParameters.Set(&View, BlendPixelShader, SceneColorFilter);
...
//			SetTextureParameterDirectly(BlendPixelShader->GetPixelShader(), BlendPixelShader->LowResPostProcessBuffer, TStaticSamplerState<SF_Bilinear>::GetRHI(), GSceneRenderTargets.GetHalfResPostProcessTexture() );
		...
		FVector4 SceneCoordinate2ScaleBias(
			0.5f / HalfSizeX,
			0.5f / HalfSizeY,
		...
		SetVertexShaderValue(
			...
			SceneCoordinate2ScaleBias);

		SetPixelShaderValue(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->SceneCoordinate2ScaleBiasParameter,
			SceneCoordinate2ScaleBias);

		// used to scale to quarter res (which is left top screen +1 border texel)
		{
			FVector4 SceneCoordinate3ScaleBias(
			...
			SetVertexShaderValue(
				BlendVertexShader->GetVertexShader(),
				BlendVertexShader->SceneCoordinate3ScaleBiasParameter,
				SceneCoordinate3ScaleBias);

			SetPixelShaderValue(
				BlendPixelShader->GetPixelShader(),
				BlendPixelShader->SceneCoordinate3ScaleBiasParameter,
				SceneCoordinate3ScaleBias);
		}
		
		SetPixelShaderValue(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->UVScaleBiasParameter,
			FVector4(
				1.0f / (FLOAT)BufferSizeX,
				1.0f / (FLOAT)BufferSizeY,
				View.Y / (FLOAT)BufferSizeY,
				View.X / (FLOAT)BufferSizeX));
		...
		{	
			...
			SetVertexShaderValue(
				...
				SceneCoordinate1ScaleBias);

			SetPixelShaderValue(
				BlendPixelShader->GetPixelShader(),
				BlendPixelShader->SceneCoordinate1ScaleBiasParameter,
				SceneCoordinate1ScaleBias);
...
#undef VARIATION
		...
		else
		{
			...
			if (FinalEffectInGroup
				&& !GSystemSettings.NeedsUpscale())
			{
				if (View.Family->bScreenCaptureRenderTarget)
				{
					RHISetRenderTarget(View.Family->RenderTarget->GetRenderTargetSurface(),FSurfaceRHIRef());	
				}
				else
				{
...
class FOLUberPostProcessSceneProxy : public FUberPostProcessSceneProxy
{
public:

	FOLUberPostProcessSceneProxy(const UOLUberPostProcessEffect* InEffect, const FPostProcessSettings* WorldSettings, UINT InColorGradingCVar, UINT InTonemapperType, UBOOL bInMotionBlur, UBOOL bInImageGrain) :
		FUberPostProcessSceneProxy(InEffect, WorldSettings, InColorGradingCVar, InTonemapperType, bInMotionBlur, bInImageGrain),
		GrainTexture(InEffect->GrainTexture),
		VignetteParams(InEffect->VignetteBlack, InEffect->VignetteWhite),
		GrainParams(InEffect->GrainBrightness, InEffect->GrainOpacity, InEffect->GrainScale, 0.0f),
		HurtCircleParams(InEffect->HurtRadiusMin, InEffect->HurtRadiusMax, InEffect->HurtHardnessMin, InEffect->HurtHardnessMax),
		HurtColor(InEffect->HurtColor),
		HurtTimeScale(InEffect->HurtTimeScale),
		HurtScale(InEffect->HurtScale),
		HurtParams(powf(InEffect->HurtAmount, InEffect->HurtExp), 0.0f),
		CameraMode(InEffect->CameraMode),
		bCameraShattered(InEffect->bCameraGlassShattered),
		CameraColor(Lerp(FLinearColor::White, InEffect->CameraColor * InEffect->CameraColor.A, InEffect->CameraColorEffect)),
		CameraDistortionAndEffectStrength(0, 0, InEffect->CameraColorEffect),
		CameraDistortionScale(InEffect->CameraScale),
		CameraGlassLightColor(InEffect->CameraGlassLightColor.R * InEffect->CameraGlassLightIntensity, InEffect->CameraGlassLightColor.G * InEffect->CameraGlassLightIntensity, InEffect->CameraGlassLightColor.B * InEffect->CameraGlassLightIntensity),
		CameraGlassDiffuseTexture(InEffect->CameraGlassDiffuse),
		CameraGlassNormalTexture(InEffect->CameraGlassNormal),
		CameraGlassCubeMapTexture(InEffect->CameraGlassCubeMap),
		MovieIntensity(InEffect->MovieIntensity),
		MovieColorLo(InEffect->MovieColorLo),
		MovieColorHi(InEffect->MovieColorHi),
		MovieColor(InEffect->MovieColor),
		MovieTexture(InEffect->MovieTexture),
		MovieGradTexture(InEffect->MovieGradTexture),
		MovieLightMaskTexture(InEffect->MovieLightMaskTexture)
	{
	}

	template<UINT CameraMode, UBOOL bCameraShattered, UBOOL bMovieEffect>
	void RenderOLVariationFullResTempl(FViewInfo& View, const FTextureRHIRef& ColorGradingRHI)
	{
		static FGlobalBoundShaderState UberBlendBoundShaderState;
		TShaderMapRef<FUberPostProcessVertexShader> BlendVertexShader(GetGlobalShaderMap());
		TShaderMapRef<TOLUberPostProcessBlendPixelShader<CameraMode,bCameraShattered, bMovieEffect> > BlendPixelShader(GetGlobalShaderMap());
		if (!*BlendVertexShader || !*BlendPixelShader)
		{
			return;
		}
		SetGlobalBoundShaderState(UberBlendBoundShaderState, GFilterVertexDeclaration.VertexDeclarationRHI, *BlendVertexShader, *BlendPixelShader, sizeof(FFilterVertex), 0, EGST_UberPostProcess);

		GrainParams.W = View.Family->CurrentWorldTime;
		HurtParams.Y = cosf(View.Family->CurrentWorldTime * HurtTimeScale) * HurtScale;
		CameraDistortionAndEffectStrength.X = (CameraDistortionScale * 0.01f) * (View.SizeX / (FLOAT)GSceneRenderTargets.GetBufferSizeX());
		CameraDistortionAndEffectStrength.Y = (CameraDistortionScale * 0.01f) * (View.SizeY / (FLOAT)GSceneRenderTargets.GetBufferSizeY());;

		SetTextureParameter(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->GrainTextureParameter,
			TStaticSamplerState<SF_Bilinear,AM_Wrap,AM_Wrap,AM_Wrap>::GetRHI(),
			GrainTexture->Resource->TextureRHI );

		SetPixelShaderValue(
			BlendPixelShader->GetPixelShader(), 
			BlendPixelShader->GrainParameter,
			GrainParams);

		SetPixelShaderValue(
			BlendPixelShader->GetPixelShader(), 
			BlendPixelShader->VignetteParameter,
			VignetteParams);

		SetPixelShaderValue(
			BlendPixelShader->GetPixelShader(), 
			BlendPixelShader->HurtCircleParameter,
			HurtCircleParams);

		SetPixelShaderValue(
			BlendPixelShader->GetPixelShader(), 
			BlendPixelShader->HurtColorParameter,
			HurtColor);

		SetPixelShaderValue(
			BlendPixelShader->GetPixelShader(), 
			BlendPixelShader->HurtParameter,
			HurtParams);

		SetPixelShaderValue(
			BlendPixelShader->GetPixelShader(), 
			BlendPixelShader->CameraDistortionAndEffectStrengthParameter,
			CameraDistortionAndEffectStrength);

		SetPixelShaderValue(
			BlendPixelShader->GetPixelShader(), 
			BlendPixelShader->CameraColorParameter,
			CameraColor);

		SetTextureParameter(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->CameraGlassDiffuseTextureParameter,
			TStaticSamplerState<SF_Bilinear,AM_Clamp,AM_Clamp,AM_Clamp>::GetRHI(),
			CameraGlassDiffuseTexture->Resource->TextureRHI );

		SetTextureParameter(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->CameraGlassNormalTextureParameter,
			TStaticSamplerState<SF_Bilinear,AM_Clamp,AM_Clamp,AM_Clamp>::GetRHI(),
			CameraGlassNormalTexture->Resource->TextureRHI );

		SetTextureParameter(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->CameraGlassCubeMapTextureParameter,
			TStaticSamplerState<SF_Bilinear,AM_Clamp,AM_Clamp,AM_Clamp>::GetRHI(),
			CameraGlassCubeMapTexture->Resource->TextureRHI );

		SetShaderValue(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->ViewToWorldParameter, 
			View.InvViewMatrix);

		SetShaderValue(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->CameraGlassLightColorParameter, 
			CameraGlassLightColor);

		SetShaderValue(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->MovieIntensityParameter, 
			MovieIntensity);

		SetShaderValue(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->MovieColorLoParameter, 
			MovieColorLo);

		SetShaderValue(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->MovieColorHiParameter, 
			MovieColorHi);

		SetShaderValue(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->MovieColorParameter, 
			MovieColor);

		SetTextureParameter(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->MovieTextureParameter,
			TStaticSamplerState<SF_Bilinear,AM_Clamp,AM_Clamp,AM_Clamp>::GetRHI(),
			MovieTexture->Resource->TextureRHI );

		SetTextureParameter(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->MovieGradTextureParameter,
			TStaticSamplerState<SF_Bilinear,AM_Wrap,AM_Wrap,AM_Wrap>::GetRHI(),
			MovieGradTexture->Resource->TextureRHI );

		SetTextureParameter(
			BlendPixelShader->GetPixelShader(),
			BlendPixelShader->MovieLightMaskTextureParameter,
			TStaticSamplerState<SF_Bilinear,AM_Clamp,AM_Clamp,AM_Clamp>::GetRHI(),
			MovieLightMaskTexture->Resource->TextureRHI );

		RenderVariationFullRes(View, ColorGradingRHI, *BlendVertexShader, *BlendPixelShader, SF_Bilinear);
	}

	virtual UBOOL HasOutlastNightVision() const
	{
		return CameraMode == (UINT)CM_OnNightVision;
	}

	UBOOL Render(const FScene* Scene, UINT InDepthPriorityGroup, FViewInfo& View, const FMatrix& CanvasTransform, FSceneColorLDRInfo& LDRInfo)
	{
		SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,TEXT("OLUberPostProcessing"));
...
#undef VARIATION
		...
		UBOOL bMovieEffect = MovieIntensity > 0.0001f;

		// full resolution pass -----------------
#define VARIATION1(A)		VARIATION2(A,0)			VARIATION2(A,1)
#define VARIATION2(A, B)	VARIATION3(A,B,0)		VARIATION3(A,B,1)
#define VARIATION3(A, B, C) \
		if(A == CameraMode && B == bCameraShattered && C == bMovieEffect) \
		{ \
			RenderOLVariationFullResTempl<A, B, C>(View, ColorGradingRHI); \
		} else
		VARIATION1(0) VARIATION1(1) VARIATION1(2)
...
private:
	...
	UTexture2D*    GrainTexture;
	FVector2D      VignetteParams;
	FVector4       GrainParams;
	FVector4       HurtCircleParams;
	FLinearColor   HurtColor;
	FVector2D      HurtParams;
	FLOAT          HurtTimeScale;
	FLOAT          HurtScale;
	FLinearColor   CameraColor;
	FVector        CameraDistortionAndEffectStrength;
	FLOAT          CameraDistortionScale;
	FVector        CameraGlassLightColor;
	UTexture2D*    CameraGlassDiffuseTexture;
	UTexture2D*    CameraGlassNormalTexture;
	UTextureCube*  CameraGlassCubeMapTexture;
	FLOAT          MovieIntensity;
	FLinearColor   MovieColorLo;
	FLinearColor   MovieColorHi;
	FLinearColor   MovieColor;
	UTextureMovie* MovieTexture;
	UTexture2D*    MovieGradTexture;
	UTexture2D*    MovieLightMaskTexture;

	UINT         CameraMode;
	UBOOL        bCameraShattered;

};

FPostProcessSceneProxy* UOLUberPostProcessEffect::CreateSceneProxy(const FPostProcessSettings* WorldSettings)
{
	UBOOL bLocalMotionBlur = FALSE;
	...
	return new FOLUberPostProcessSceneProxy(this, WorldSettings, GColorGrading, TonemapperType, bLocalMotionBlur, bEnableImageGrain);
}
