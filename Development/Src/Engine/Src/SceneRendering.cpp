...
#elif ORBIS
	// mathieu.gauthier: essentially disabling depth pre-pass. 
	static const FLOAT DefaultMinScreenRadiusForDepthPrepass = 100.0f;
#else
	// mathieu.gauthier: We used to compile with the mobile RHI enabled so 
	// we were basically skipping the depth prepass. Let's keep it that way
	// to avoid breaking anything.
	static const FLOAT DefaultMinScreenRadiusForDepthPrepass = 0.82f; //0.03f;
#endif
...
#if DINGO
				bHasOutlastNightVision |= PostProcessSceneProxies(ProxyIdx)->HasOutlastNightVision();
#endif
			}
		}
...
//@zombie_ps4_begin
#if ORBIS
		if(CompactPrimitiveSceneInfo.StaticDepthPriorityGroup != SDPG_World)
		{
			// Only allow primitives that are in the world DPG to be occluded.
			bOccludableCached = FALSE;
		}
		else
		{
			bOccludableCached = CompactPrimitiveSceneInfo.Proxy->RequiresOcclusion(NULL);
		}
#endif
	}

	inline PTRINT GetComponentPtr() const
	{
		return CompactPrimitiveSceneInfo.ComponentPtr;
	}
//@zombie_ps4_end
...
void FSceneRenderer::PerformViewFrustumCulling()
{
	...
	// mathieu.gauthier: Keep the skin primitives at the very end of the
	// dynamic array because they require a full depth buffer.
	for(INT ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		FViewInfo& View = Views(ViewIndex);
		INT StartIdx = View.VisibleDynamicPrimitives.Num();
		INT NumSkinPrims = View.VisibleDynamicSkinPrimitives.Num();
		if (NumSkinPrims > 0)
		{
			View.VisibleDynamicPrimitives.Add(NumSkinPrims);
			appMemcpy(
				&View.VisibleDynamicPrimitives(StartIdx), 
				&View.VisibleDynamicSkinPrimitives(0), 
				sizeof(const FPrimitiveSceneInfo*) * NumSkinPrims);
		}
	}
...
void FSceneRenderer::InitViews()
{
	...
	// mathieu.gauthier: This is a more approximate version that does not take
	// visibility into account, but still can avoid us doing a full pre-pass.
	bDominantShadowsActive = AreDominantShadowsActive(SDPG_World, TRUE);
	SET_DWORD_STAT(STAT_DominantShadowActivePrePass, bDominantShadowsActive);
	...
	for(TSparseArray<FLightSceneInfoCompact>::TConstIterator LightIt(Scene->Lights);LightIt;++LightIt)
	{
		...
		// mathieu.gauthier: If this light had cached shadow map that have
		// not been used in a while, just release them.
		const_cast<FLightSceneInfo*>(LightSceneInfo)->ReleaseUnusedCachedShadowMaps();
...
void FSceneRenderer::ClearView()
{
	...
	GSceneRenderTargets.BeginRenderingSceneColor(RTUsage_SwapToESRAM);
...
UBOOL FSceneRenderer::RenderDPGBasePassStaticDataMasked(UINT DPGIndex, FViewInfo& View)
{
	...
	RHISetShaderRegisterAllocation(128 - GBasePassMaskedPixelGPRs, GBasePassMaskedPixelGPRs);
	{
		...
		UBOOL bBasePassDirectionalVertexLightMapDrawListEmpty                                   = Scene->DPGs[DPGIndex].BasePassDirectionalVertexLightMapDrawList[MaskedDrawType].IsEmpty();
		UBOOL bBasePassSimpleVertexLightMapDrawListEmpty                                        = Scene->DPGs[DPGIndex].BasePassSimpleVertexLightMapDrawList[MaskedDrawType].IsEmpty();
		UBOOL bBasePassShadowedDynamicLightDirectionalVertexLightMapDrawListEmpty               = Scene->DPGs[DPGIndex].BasePassShadowedDynamicLightDirectionalVertexLightMapDrawList[MaskedDrawType].IsEmpty();
		UBOOL bBasePassDirectionalLightMapTextureDrawListEmpty                                  = Scene->DPGs[DPGIndex].BasePassDirectionalLightMapTextureDrawList[MaskedDrawType].IsEmpty();
		UBOOL bBasePassSimpleLightMapTextureDrawListEmpty                                       = Scene->DPGs[DPGIndex].BasePassSimpleLightMapTextureDrawList[MaskedDrawType].IsEmpty();
		UBOOL bBasePassShadowedDynamicLightDirectionalLightMapTextureDrawListEmpty              = Scene->DPGs[DPGIndex].BasePassShadowedDynamicLightDirectionalLightMapTextureDrawList[MaskedDrawType].IsEmpty();
		UBOOL bBasePassDistanceFieldShadowedDynamicLightDirectionalLightMapTextureDrawListEmpty = Scene->DPGs[DPGIndex].BasePassDistanceFieldShadowedDynamicLightDirectionalLightMapTextureDrawList[MaskedDrawType].IsEmpty();
		UBOOL bBasePassDirectionalLightDrawListEmpty                                            = Scene->DPGs[DPGIndex].BasePassDirectionalLightDrawList[MaskedDrawType].IsEmpty();
		UBOOL bBasePassSHLightDrawListEmpty                                                     = Scene->DPGs[DPGIndex].BasePassSHLightDrawList[MaskedDrawType].IsEmpty();
		UBOOL bBasePassDynamicallyShadowedDynamicLightDrawListEmpty                             = Scene->DPGs[DPGIndex].BasePassDynamicallyShadowedDynamicLightDrawList[MaskedDrawType].IsEmpty();
		UBOOL bBasePassSHLightAndDynamicLightDrawListEmpty                                      = Scene->DPGs[DPGIndex].BasePassSHLightAndDynamicLightDrawList[MaskedDrawType].IsEmpty();
		UBOOL bBasePassNoLightMapDrawListEmpty                                                  = Scene->DPGs[DPGIndex].BasePassNoLightMapDrawList[MaskedDrawType].IsEmpty();

		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticMaskedVertexLightmapped, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, MaskedDrawType, 
			!bBasePassDirectionalVertexLightMapDrawListEmpty || !bBasePassSimpleVertexLightMapDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
		...
		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticMaskedShadowedDLVertexLightmap, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, MaskedDrawType, 
			!bBasePassShadowedDynamicLightDirectionalVertexLightMapDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
		...
		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticMaskedTextureLightmapped, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, MaskedDrawType, 
			!bBasePassDirectionalLightMapTextureDrawListEmpty || !bBasePassSimpleLightMapTextureDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
		...
		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticMaskedShadowedDLTextureLightmap, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, MaskedDrawType, 
			!bBasePassShadowedDynamicLightDirectionalLightMapTextureDrawListEmpty || !bBasePassDistanceFieldShadowedDynamicLightDirectionalLightMapTextureDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
		...
		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticMaskedDynamicallyLit, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, MaskedDrawType, 
			!bBasePassDirectionalLightDrawListEmpty || !bBasePassSHLightDrawListEmpty || !bBasePassDynamicallyShadowedDynamicLightDrawListEmpty || !bBasePassSHLightAndDynamicLightDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
		...
		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticMaskedNoLightmap, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, MaskedDrawType, 
			!bBasePassNoLightMapDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
...
UBOOL FSceneRenderer::RenderDPGBasePassStaticDataDefault(UINT DPGIndex, FViewInfo& View)
{
	...
	RHISetShaderRegisterAllocation(128 - GBasePassOpaquePixelGPRs, GBasePassOpaquePixelGPRs);
	{
		...
		UBOOL bBasePassDirectionalVertexLightMapDrawListEmpty                                   = Scene->DPGs[DPGIndex].BasePassDirectionalVertexLightMapDrawList[OpaqueDrawType].IsEmpty();
		UBOOL bBasePassSimpleVertexLightMapDrawListEmpty                                        = Scene->DPGs[DPGIndex].BasePassSimpleVertexLightMapDrawList[OpaqueDrawType].IsEmpty();
		UBOOL bBasePassShadowedDynamicLightDirectionalVertexLightMapDrawListEmpty               = Scene->DPGs[DPGIndex].BasePassShadowedDynamicLightDirectionalVertexLightMapDrawList[OpaqueDrawType].IsEmpty();
		UBOOL bBasePassDirectionalLightMapTextureDrawListEmpty                                  = Scene->DPGs[DPGIndex].BasePassDirectionalLightMapTextureDrawList[OpaqueDrawType].IsEmpty();
		UBOOL bBasePassSimpleLightMapTextureDrawListEmpty                                       = Scene->DPGs[DPGIndex].BasePassSimpleLightMapTextureDrawList[OpaqueDrawType].IsEmpty();
		UBOOL bBasePassShadowedDynamicLightDirectionalLightMapTextureDrawListEmpty              = Scene->DPGs[DPGIndex].BasePassShadowedDynamicLightDirectionalLightMapTextureDrawList[OpaqueDrawType].IsEmpty();
		UBOOL bBasePassDistanceFieldShadowedDynamicLightDirectionalLightMapTextureDrawListEmpty = Scene->DPGs[DPGIndex].BasePassDistanceFieldShadowedDynamicLightDirectionalLightMapTextureDrawList[OpaqueDrawType].IsEmpty();
		UBOOL bBasePassDirectionalLightDrawListEmpty                                            = Scene->DPGs[DPGIndex].BasePassDirectionalLightDrawList[OpaqueDrawType].IsEmpty();
		UBOOL bBasePassSHLightDrawListEmpty                                                     = Scene->DPGs[DPGIndex].BasePassSHLightDrawList[OpaqueDrawType].IsEmpty();
		UBOOL bBasePassDynamicallyShadowedDynamicLightDrawListEmpty                             = Scene->DPGs[DPGIndex].BasePassDynamicallyShadowedDynamicLightDrawList[OpaqueDrawType].IsEmpty();
		UBOOL bBasePassSHLightAndDynamicLightDrawListEmpty                                      = Scene->DPGs[DPGIndex].BasePassSHLightAndDynamicLightDrawList[OpaqueDrawType].IsEmpty();
		UBOOL bBasePassNoLightMapDrawListEmpty                                                  = Scene->DPGs[DPGIndex].BasePassNoLightMapDrawList[OpaqueDrawType].IsEmpty();

		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticOpaqueVertexLightmapped, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, OpaqueDrawType, 
			!bBasePassDirectionalVertexLightMapDrawListEmpty || !bBasePassSimpleVertexLightMapDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
		...
		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticOpaqueShadowedDLVertexLightmap, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, OpaqueDrawType, 
			!bBasePassShadowedDynamicLightDirectionalVertexLightMapDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
		...
		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticOpaqueTextureLightmapped, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, OpaqueDrawType, 
			!bBasePassDirectionalLightMapTextureDrawListEmpty || !bBasePassSimpleLightMapTextureDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
		...
		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticOpaqueShadowedDLTextureLightmap, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, OpaqueDrawType, 
			!bBasePassShadowedDynamicLightDirectionalLightMapTextureDrawListEmpty || !bBasePassDistanceFieldShadowedDynamicLightDirectionalLightMapTextureDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
		...
		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticOpaqueDynamicallyLit, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, OpaqueDrawType, 
			!bBasePassDirectionalLightDrawListEmpty || !bBasePassSHLightDrawListEmpty || !bBasePassDynamicallyShadowedDynamicLightDrawListEmpty || !bBasePassSHLightAndDynamicLightDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
		...
		START_DEFERRED_CONTEXT_THREAD_ONE_PARAMETER(StaticOpaqueNoLightmap, FSceneRenderer, FDepthPriorityGroup::EBasePassDrawListType, OpaqueDrawType, 
			!bBasePassNoLightMapDrawListEmpty, DPGIndex == 1 && !bIsSceneCapture,
		{
...
UBOOL FSceneRenderer::RenderDPGBasePassDynamicPrimitives( UINT DPGIndex, FViewInfo& View, EBasePassDynamicBitFlags DrawFlags, INT& OutNumIgnoredPrimitives )
{
	...
	// This pass tends to be super expensive, so we break down the rendering in
	// small batches to better distribute the workload on multiple threads.
	// Also, in wireframe mode, pretty much everything becomes a dynamic primitive
	// so we make bigger batches.
	const INT BatchSize = View.Family->ShowFlags & SHOW_Wireframe ? 128 : 8;
	for(INT StartIndex = 0; StartIndex < View.VisibleDynamicPrimitives.Num(); StartIndex += BatchSize)
	{
		START_DEFERRED_CONTEXT_THREAD_FOUR_PARAMETERS(StaticOpaqueNoLightmap, FSceneRenderer, INT, StartIndex, const INT, BatchSize, EBasePassDynamicBitFlags, DrawFlags, UBOOL, bDrawShadowCasters,
			TRUE, DPGIndex == 1 && !bIsSceneCapture,
		{
			SCOPE_CONDITIONAL_CYCLE_COUNTER(STAT_DynamicPrimitiveDrawTime, !This->bIsSceneCapture);
			SCOPED_DRAW_EVENT(EventStaticOpaqueNoLightmapDynamic)(DEC_SCENE_ITEMS,TEXT("StaticOpaqueNoLightmapDynamic"));
			...
			//if( View.VisibleDynamicPrimitives.Num() > 0 )
			{
				...
				for(INT PrimitiveIndex = StartIndex; PrimitiveIndex < (StartIndex + BatchSize) && PrimitiveIndex < View.VisibleDynamicPrimitives.Num();PrimitiveIndex++)
				{
					...
					if( bVisible && bRelevantDPG && 
						...
						(PrimitiveViewRelevance.bOpaqueRelevance || View.Family->ShowFlags & SHOW_Wireframe) )
					{
...
UBOOL FSceneRenderer::RenderDPGBegin(UINT DPGIndex, UBOOL& bRequiresClear, UBOOL& bSceneColorDirty, UBOOL bIsOcclusionTesting)
{
	if( !GUsingMobileRHI )
	{
		...
		UBOOL bDominantShadowsActivePrePass = bDominantShadowsActive;
		bDominantShadowsActive = AreDominantShadowsActive(DPGIndex);
		SET_DWORD_STAT(STAT_DominantShadowActive, bDominantShadowsActive);

		// mathieu.gauthier: Last minute hack. It seems like the dominant light test is
		// flawed for light functions. It assumes the light is visible. The test in 
		// FSceneRenderer::CheckForLightFunction() should check for light visiblity.
		// Too risky to change right now.
		if (bDominantShadowsActive > bDominantShadowsActivePrePass)
		{
			bDominantShadowsActive = FALSE;
		}
		...
		GSceneRenderTargets.BeginRenderingSceneColor(RTUsage_SwapToESRAM, TRUE, !ShouldUseDeferredShading());
		...
		if( bWorldDpg && !bDominantShadowsActive)
		{
...
void FSceneRenderer::RenderDPGEnd(UINT DPGIndex, UBOOL bDeferPrePostProcessResolve, UBOOL& bSceneColorDirty, UBOOL bIsOcclusionTesting)
{
	...
	if( !GUsingMobileRHI )
	{
		...
		if(ViewFamily.ShowFlags & SHOW_Lighting)
		{
			...
			// Render (cheaper) skin shading.
			bSceneColorDirty |= RenderSkin(DPGIndex);
...
void FSceneRenderer::RenderFinish(UBOOL bDeferPrePostProcessResolve)
{
	...
	if( bDeferPrePostProcessResolve )
	{
		GSceneRenderTargets.ResolveSceneColor(FResolveRect(0, 0, FamilySizeX, FamilySizeY), FALSE);
	}
	...
	RHIWaitForCompletedDeferredContexts(TRUE);
#endif
...
void FSceneRenderer::ProcessAndRenderDebugOptions()
{
	...
	GSceneRenderTargets.UpdateCachedShadowMapsStats();
...
UBOOL FSceneRenderer::RenderDPGPrePass(UINT DPGIndex, FViewInfo& View)
{
	...
	START_DEFERRED_CONTEXT_THREAD(PosOnlyOpaque, FSceneRenderer,
		!Scene->DPGs[DPGIndex].PositionOnlyDepthDrawList.IsEmpty(), DPGIndex == 1 && !bIsSceneCapture,
	{
	...
	START_DEFERRED_CONTEXT_THREAD(Opaque, FSceneRenderer,
		!Scene->DPGs[DPGIndex].DepthDrawList.IsEmpty(), DPGIndex == 1 && !bIsSceneCapture,
	{
	...
	if( !GUsingMobileRHI )
	{
		...
		{
			START_DEFERRED_CONTEXT_THREAD(Masked, FSceneRenderer,
				!Scene->DPGs[DPGIndex].MaskedDepthDrawList.IsEmpty(), DPGIndex == 1 && !bIsSceneCapture,
			{
		...
		{
			START_DEFERRED_CONTEXT_THREAD(SoftMasked, FSceneRenderer,
				!Scene->DPGs[DPGIndex].SoftMaskedDepthDrawList.IsEmpty(), DPGIndex == 1 && !bIsSceneCapture,
			{
		...
		START_DEFERRED_CONTEXT_THREAD_FOUR_PARAMETERS(SoftMasked, FSceneRenderer, const EDepthDrawingMode, DepthDrawingMode, UBOOL, bShowShaderComplexity, UBOOL, bDominantShadowsActive, FLOAT, ScreenThresholdSq, 
			View.VisibleDynamicPrimitives.Num() > 0, DPGIndex == 1 && !bIsSceneCapture,
		{
			// Draw the dynamic occluder primitives using a depth drawing policy.
			TDynamicPrimitiveDrawer<FDepthDrawingPolicyFactory> DrawerReg(&View,DPGIndex,FDepthDrawingPolicyFactory::ContextType(DepthDrawingMode),TRUE);
			TDynamicPrimitiveDrawer<FDepthDrawingPolicyFactory> DrawerAll(&View,DPGIndex,FDepthDrawingPolicyFactory::ContextType(DDM_AllOccluders),TRUE);
			{
				SCOPED_DRAW_EVENT(EventSoftMaskedDynamic)(DEC_SCENE_ITEMS,TEXT("SoftMaskedDynamic"));
				for(INT PrimitiveIndex = 0;PrimitiveIndex < View.VisibleDynamicPrimitives.Num();PrimitiveIndex++)
				{
					...
					if( (PrimitiveSceneInfo->bUseAsOccluder || bShowShaderComplexity || PrimitiveViewRelevance.bSkinRelevance) &&
						...
						bShowShaderComplexity || 
						// mathieu.gauthier: We need valid depth around skin lighting model in
						// case its a multi-element mesh where not all elements use skin. It
						// needs to be able to occlude itself.
						PrimitiveViewRelevance.bSkinRelevance || 
					...
					{
						TDynamicPrimitiveDrawer<FDepthDrawingPolicyFactory>& Drawer = PrimitiveViewRelevance.bSkinRelevance ? DrawerAll : DrawerReg;
						Drawer.SetPrimitive(PrimitiveSceneInfo);
						PrimitiveSceneInfo->Proxy->DrawDynamicElements(
							&Drawer,
							&View,
			...
			bDirty |= DrawerReg.IsDirty() || DrawerAll.IsDirty();
		});
	}
	return bDirty;
}
...
public:
	...
	{ 
//@zombie_ps4_begin
		return IsPCPlatform(Platform) || Platform == SP_ORBIS || Platform == SP_DINGO;
//@zombie_ps4_end
	}
...
{
#if ORBIS || DINGO
	// mathieu.gauthier: We dont use the downsampled depth anywhere.
	return;
#endif
    ...
    )
{
	...
	{
		if (ViewRelevance.bSkinRelevance)
		{
			// Keep track of visible dynamic primitives.
			View.VisibleDynamicSkinPrimitives.AddItem(CompactPrimitiveSceneInfo.PrimitiveSceneInfo);
#if WITH_REALD
			if (bDoStereo)
			{
				View2->VisibleDynamicSkinPrimitives.AddItem(CompactPrimitiveSceneInfo.PrimitiveSceneInfo);
			}
#endif
		}
		else
	...
	if( ViewRelevance.IsRelevant() )
    {
			...
			)
		{
			...
			// We already merge the hero spot light in the base pass, we should not be doing it twice.
			check(!LightSceneInfo->bIsHeroSpotLight || !bMergedIntoBasePass);

			// Use multi-pass lighting to render dynamic, non-skylight lights that are not going to be merged into the base pass.
#if DINGO
			Interaction->SetNeedsLightRenderingPass(bRenderLight && !bMergedIntoBasePass && !LightSceneInfo->bIsHeroSpotLight);
#else
...
{
	FLOAT ScreenThresholdSq = (Scene->NumWholeSceneShadowLights > 0) || (Scene->NumDirectionalLightFunctions > 0) || bDominantShadowsActive ? GMinScreenRadiusForDepthPrepassWithShadowsSquared : GMinScreenRadiusForDepthPrepassSquared;
...
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Num normal shadow lights"),STAT_NumNormalShadowLights,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Num optimized shadow lights"),STAT_NumOptimShadowLights,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Num enabled lights"),STAT_NumEnabledLights,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Num disabled lights"),STAT_NumDisabledLights,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Cached Shadow 2D (L0)"),STAT_NumCachedShadowMap2DLevel0,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Cached Shadow 2D (L1)"),STAT_NumCachedShadowMap2DLevel1,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Cached Shadow 2D (L2)"),STAT_NumCachedShadowMap2DLevel2,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Cached Shadow 2D (L3)"),STAT_NumCachedShadowMap2DLevel3,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Cached Shadow 2D (L4)"),STAT_NumCachedShadowMap2DLevel4,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Cached Shadow Cube (L0)"),STAT_NumCachedShadowMapCubeLevel0,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Cached Shadow Cube (L1)"),STAT_NumCachedShadowMapCubeLevel1,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Cached Shadow Cube (L2)"),STAT_NumCachedShadowMapCubeLevel2,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Cached Shadow Cube (L3)"),STAT_NumCachedShadowMapCubeLevel3,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Cached Shadow Cube (L4)"),STAT_NumCachedShadowMapCubeLevel4,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Dominant Shadows Active (Pre)"),STAT_DominantShadowActivePrePass,STATGROUP_ShadowRendering);
DECLARE_DWORD_COUNTER_STAT(TEXT("(Outlast) Dominant Shadows Active"),STAT_DominantShadowActive,STATGROUP_ShadowRendering);
