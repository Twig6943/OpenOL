...
void FBoneDataTexture::InitDynamicRHI()
{
	...
	// mathieu.gauthier: We purposely dont create a dynamic texture. Update is MUCH faster
	// when running on D3D11.
	DWORD Flags = /*TexCreate_Dynamic |*/ TexCreate_NoTiling | TexCreate_NoMipTail | TexCreate_DiscardContentsOnLock;
	UINT NumMips = 1;
	EPixelFormat Format = PF_A32B32G32R32F;
	Texture2DRHI = RHICreateTexture2D(GetSizeX(), GetSizeY(), Format, NumMips, Flags, NULL );
	TextureRHI = Texture2DRHI;

	INC_MEMORY_STAT_BY( STAT_SkeletalMeshMotionBlurSkinningMemory, ComputeMemorySize());
}

/** Called when the resource is released. This is only called by the rendering thread. */
void FBoneDataTexture::ReleaseDynamicRHI()
{
	DEC_MEMORY_STAT_BY( STAT_SkeletalMeshMotionBlurSkinningMemory, ComputeMemorySize());
...
public:
	...
	*/
	...
	virtual void RebindFromMap(const FShaderParameterMap& ParameterMap)
	{
		Bind(ParameterMap);
	}
	...
	*/
	virtual void Set(FShader* VertexShader,const FVertexFactory* VertexFactory,const FSceneView& View) const
	{
		...
		if(GSceneRenderTargets.bRenderingVelocities && GSceneRenderTargets.PrevPerBoneMotionBlur.IsLocked())
		{
