...
template<UINT NumSamples, UBOOL bRenormalizeClippedSamples>
...
public:
	...
	{
		// Renormalized вариант отсутствует в оригинальном 868 GlobalShaderCache
		return !bRenormalizeClippedSamples;
	}

	static void ModifyCompilationEnvironment(EShaderPlatform Platform, FShaderCompilerEnvironment& OutEnvironment)
	{
		OutEnvironment.Definitions.Set(TEXT("NUM_SAMPLES"),*FString::Printf(TEXT("%u"),NumSamples));
		OutEnvironment.Definitions.Set(TEXT("RENORMALIZE_CLIPPED_SAMPLES"),bRenormalizeClippedSamples ? TEXT("1") : TEXT("0"));	

		//if (Platform == SP_ORBIS)
		//{
		//	OutEnvironment.Definitions.Set(TEXT("ORBIS_TARGET_OUTPUT_FORMAT_MRT0"), TEXT("FMT_UNORM16_ABGR"));
		//}
	}
...
public:
	...
	void SetParameters(const FSceneView* View, FSamplerStateRHIParamRef SamplerStateRHI,FTextureRHIParamRef TextureRHI)
	{
		...
		{
			SetTextureParameterDirectly(this->GetPixelShader(), VelocityTextureParameter, TStaticSamplerState<SF_Bilinear>::GetRHI(), GWhiteTexture->TextureRHI);
		}
	}
...
extern void GaussianBlurFilterBuffer(
	...
	UBOOL bRenormalizeClippedSample = FALSE);
