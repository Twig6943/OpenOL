...
FCritialSectionHash FMaterialRenderProxy::CacheCriticalSectionHash;
#endif
...
FMaterialViewRelevance UMaterialInterface::GetViewRelevance()
{
	...
	if(Material)
	{
		...
		MaterialViewRelevance.bSkin = Material->LightingModel == MLM_Skin;
...
// mathieu.gauthier: This option will be used when merge ref shader cache in 
// certain cooking scenarios. It will temporary suspend the detection of 
// outdated shaders.
UBOOL GForceNoShaderReload = FALSE;
...
UBOOL ShouldReloadChangedShaders()
{
	...
	if (GForceNoShaderReload || GUseSeekFreeLoading)
	{
		return FALSE;
	}
...
UBOOL FMaterial::InitShaderMap(FStaticParameterSet* StaticParameters, EShaderPlatform Platform, EMaterialShaderQuality Quality)
{
	...
	if(!bValidCompilationOutput || !ShaderMap || !ShaderMap->IsComplete(this, TRUE))
	{
		...
		{
			// Only force-compile fallback materials (DefaultMaterial / DefaultDecalMaterial) — those are
			// the ones that cause infinite recursion when their shader map is missing, because the renderer
			// tries to fall back to them from themselves.  All other special engine materials (level
			// coloration, shaded coloration, etc.) are editor-only and can safely fall back to DefaultMaterial.
			if (IsFallbackMaterial())
			{
				// In cooked/seekfree builds material expressions are stripped so Compile() will always fail.
				// Borrow the ShaderMap from DefaultMaterial — it is always present in the LocalShaderCache
				// and is compatible enough to prevent the renderer from crashing.
				// DefaultDecalMaterial uses this path too: without a ShaderMap it falls back to itself
				// (infinite recursion), so we must give it something valid.
				// GEngine pointers may not be set yet, so find the object directly.
				UMaterial* DefaultMat = FindObject<UMaterial>(NULL, TEXT("EngineMaterials.DefaultMaterial"));
				FMaterialResource* DefaultRes = DefaultMat ? DefaultMat->GetMaterialResource() : NULL;
				FMaterialShaderMap* BorrowedMap = DefaultRes ? DefaultRes->GetShaderMap() : NULL;
				if (BorrowedMap)
				{
					warnf(NAME_Warning, TEXT("Missing shader map for fallback material %s in seekfree mode; borrowing ShaderMap from DefaultMaterial."), *GetFriendlyName());
					ShaderMap = BorrowedMap;
					bSucceeded = TRUE;
				}
				if (!bSucceeded)
				{
					appErrorf(TEXT("Failed to find fallback ShaderMap for %s — DefaultMaterial has no ShaderMap either!"), *GetFriendlyName());
				}
			}
			else
			{
				...
				{
					warnf(NAME_Warning, TEXT("Missing shader map for special engine material %s in seekfree mode; using default material."), *GetFriendlyName());
				}
				else
				{
...
UBOOL FMaterialResource::IsFallbackMaterial() const
{
	// GEngine pointers are set after CacheShaders runs, so compare by name.
	// Both DefaultMaterial and DefaultDecalMaterial cause infinite fallback recursion when
	// their ShaderMap is NULL (renderer falls back to them from themselves).
	const FString PathName = Material->GetPathName();
	return PathName == TEXT("EngineMaterials.DefaultMaterial") ||
	       PathName == TEXT("EngineMaterials.DefaultDecalMaterial");
}
...
UBOOL FMaterialResource::IsTessellationFrustumCullingEnabled() const 
{ 
	return Material->bTessellationFrustumCulling;
}

UBOOL FMaterialResource::IsTessellationBackfaceCullingEnabled() const 
{ 
	return Material->bTessellationBackfaceCulling;
}
	...
	) const
{
	...
	case MLM_Skin: OutEnvironment.Definitions.Set(TEXT("MATERIAL_LIGHTINGMODEL_SKIN"),TEXT("1")); break;
...
//@zombie_ps4_begin
		const FString FetchFunctionName = Material->IsUsedWithMaterialEffect() ?  TEXT("GetMaterialEffectSceneColor") : TEXT("GetSceneColor");
//@zombie_ps4_end
		...
		if ( ScreenAlign && CoordinateIdx != INDEX_NONE )
		{
			// sampler
//@zombie_ps4_begin
			const FString SampleCode( TEXT("%s(ScreenAlignedUV(%s))") );
//@zombie_ps4_end
			// add the code string
			return AddCodeChunk(
				MCT_Float4,
		...
		else
		{
			// sampler
//@zombie_ps4_begin
			const FString SampleCode( TEXT("%s(%s)") );
			// replace default tex coords with user specified coords if available
			const FString DefaultScreenAligned(TEXT("float2(ScreenAlignedPosition(Parameters.ScreenPosition).xy)"));
			const FString TexCoordCode( (CoordinateIdx != INDEX_NONE) ? CoerceParameter(CoordinateIdx,MCT_Float2) : DefaultScreenAligned );
//@zombie_ps4_end
			// add the code string
			return AddCodeChunk(
				MCT_Float4,
	...
	*/
	...
	INT CustomExpression( class UMaterialExpressionCustom* Custom, TArray<INT>& CompiledInputs )
	{
		...
		for( INT i = 0; i < Custom->Inputs.Num(); i++ )
		{
			...
			InputParamDecl += TEXT(",");
			EMaterialValueType vt = GetParameterType(CompiledInputs(i));
			switch(vt)
			{
			case MCT_Float:
			case MCT_Float1:
			...
			case MCT_Float4:
				...
				break;
			case MCT_Texture2D:
				InputParamDecl += TEXT("SAMPLER2D_PARAM_DECL( ");
				break;
			default:
		...
		for( INT i = 0; i < CompiledInputs.Num(); i++ )
		{
			...
			const FShaderCodeChunk& ShaderCodeChunk = CodeChunks[MaterialProperty](CompiledInputs(i));

			// mathieu.gauthier: Needed for Orbis texture/sample pairs.
			if (ShaderCodeChunk.Type == MCT_Texture2D)
			{
				CodeChunk += FString::Printf(TEXT("SAMPLER2D_PARAM(%s)"), GetParameterCode(CompiledInputs(i)));
			}
			else
			{
...
UBOOL FMaterial::CacheShaders(FStaticParameterSet* StaticParameters, EShaderPlatform Platform, EMaterialShaderQuality Quality, UBOOL bFlushExistingShaderMap, UBOOL bDebugDump)
{
	...
	// rcharpentier - Do not flush if this is a new material (avoid stalls)
	// see https://udn.epicgames.com/lists/showpost.php?list=unprog3&id=59493&lessthan=&show=20
	extern UBOOL GCompileShaderFromPostLoad;
	if (ShaderMap || !GCompileShaderFromPostLoad) 
	{
