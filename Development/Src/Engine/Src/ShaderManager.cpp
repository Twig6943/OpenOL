...
void FShaderParameter::Bind(const FShaderParameterMap& ParameterMap,const TCHAR* ParameterName,UBOOL bIsOptional)
{
	...
	if(!ParameterMap.FindParameterAllocation(ParameterName,BufferIndex,BaseIndex,NumBytes,UnusedSamplerIndex))
	{
		// Parameter not found — clear so IsBound() returns FALSE
		BaseIndex = 0;
		NumBytes = 0;
		if (!bIsOptional)
		{
...
void FShaderResourceParameter::Bind(const FShaderParameterMap& ParameterMap,const TCHAR* ParameterName,UBOOL bIsOptional)
{
	...
	WORD SamplerIndex = 0;
#endif
	if(!ParameterMap.FindParameterAllocation(ParameterName,UnusedBufferIndex,BaseIndex,NumResources,SamplerIndex))
	{
		// Parameter not found — clear so IsBound() returns FALSE
		BaseIndex = 0;
		NumResources = 0;
	...
	bNeedsParameterRebind(FALSE)
{
	...
	INC_MEMORY_STAT_BY(GetMemoryStatType((EShaderFrequency)Target.Frequency), Key.Code.Num());
	INC_MEMORY_STAT_BY_FAST(STAT_ShaderCompression_UncompressedShaderMemory, Key.Code.Num());
	INC_DWORD_STAT_BY(STAT_ShaderCompression_NumShadersLoaded, 1);
}

FShader::~FShader()
{
	DEC_MEMORY_STAT_BY(GetMemoryStatType((EShaderFrequency)Target.Frequency), Key.Code.Num());
	DEC_MEMORY_STAT_BY_FAST(STAT_ShaderCompression_UncompressedShaderMemory, Key.Code.Num());
...
void FShader::InitRHI()
{
	...
	if (UseShaderCompression((EShaderPlatform)Target.Platform))
	{
		...
		if (bNeedsParameterRebind && Key.Code.Num() > 0)
		{
			FShaderParameterMap RebindMap;
			RHIBuildShaderParameterMap(Key.Code, RebindMap);
			RebindParameters(RebindMap);
			bNeedsParameterRebind = FALSE;
		}
	}
	...
	{
//@zombie_ps4_begin
		VertexShader = RHICreateVertexShader(Type,Key.Code);
//@zombie_ps4_end
	}
...
DWORD FShader::AddRef()
{
	++NumRefs;
	return NumRefs;
}

DWORD FShader::Release()
{
	check(NumRefs != 0);
	if(--NumRefs == 0)
	{
