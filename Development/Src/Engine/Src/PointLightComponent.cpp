...
public:
	...
	public:
		void Bind(const FShaderParameterMap& ParameterMap)
		{
			ShadowDepthTextureParameter.Bind(ParameterMap,TEXT("ShadowDepthCubeTexture"), TRUE);
			ShadowDepthCubeComparisonSamplerParameter.Bind(ParameterMap,TEXT("ShadowDepthCubeComparisonSampler"), TRUE);
			ShadowDepthCubeSamplerParameter.Bind(ParameterMap,TEXT("ShadowDepthCubeSampler"), TRUE);
			ShadowmapPackedParameters.Bind(ParameterMap,TEXT("ShadowmapPackedParameters"), TRUE);
			...
			if (GLightRenderingShadowInfo)
			{
				if (ShadowDepthTextureParameter.IsBound())
				{
					RHISetTextureParameter(
						...
						GSceneRenderTargets.GetCubeShadowDepthZTexture(GLightRenderingShadowInfo->ResolutionX));
				}
				...
				FVector4 PackedParams(GLightRenderingShadowInfo->ResolutionX, GLightRenderingShadowInfo->FadeAlphas(0), Light->GetRadius() / (Light->GetRadius() - 1.0f), Light->GetRadius());
				SetPixelShaderValue(PixelShader->GetPixelShader(), ShadowmapPackedParameters, PackedParams);
			}
		}
	...
	private:
		...
		FShaderParameter ShadowmapPackedParameters;
...
//@zombie_ps4_begin
		if (GRenderOnePassPointLightShadows && (GRHIShaderPlatform == SP_PCD3D_SM4 || GRHIShaderPlatform == SP_PCD3D_SM5 || GRHIShaderPlatform == SP_ORBIS || GRHIShaderPlatform == SP_DINGO))
//@zombie_ps4_end
		{
			OutInitializers.Add();
