...
IMPLEMENT_CLASS(USeqEvent_PlayerSpawned);
...
IMPLEMENT_CLASS(USeqAct_ToggleCinematicMode);
...
void USequence::BeginPlay()
{
	...
	if (OLPC)
	{
		OLPC->InitializeKismetSequence(this);
	}
}
...
UBOOL USequence::RenameAllObjectsInOtherLevels( const FName& InName, const FName& ParentName, ERenameFlags Flags )
{
	...
	for (TObjectIterator<UObject> It; It; ++It)
	{
		...
		if( InName == Object->GetFName() && Object->GetOuter() && Object->GetOuter()->GetFName() != ParentName )
		{
			...
			Object->Rename(NULL, NULL, Flags | REN_NoRedirector);
...
// rcharpentier - you think this is a nasty hack? it is. sue me.
namespace Utils
{
...
UBOOL USeqEvent_Touch::CheckActivate(AActor *InOriginator, AActor *InInstigator, UBOOL bTest, TArray<INT>* ActivateIndices, UBOOL bPushTop)
{
	...
	//if (InInstigator != NULL && InInstigator->IsPlayerOwned())
		//debugf(TEXT("TOUCH_CHECK: %s instigator=%s bEnabled=%d TriggerCount=%d MaxTriggerCount=%d ReTriggerDelay=%.2f TimeSinceActivation=%.2f"), *GetName(), *InInstigator->GetName(), (INT)bEnabled, TriggerCount, MaxTriggerCount, ReTriggerDelay, GWorld->GetTimeSeconds() - ActivationTime);
		...
		InInstigator != NULL)
	{
		...
		if (bPassed && LastValidCheckpoint != NAME_None)
		{			
			bPassed = !Utils::IsCheckpointCompleted(LastValidCheckpoint);
		}
...
void USeqEvent_Touch::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	UProperty* PropertyThatChanged = PropertyChangedEvent.Property;
	if (PropertyThatChanged != NULL && PropertyThatChanged->GetFName() == FName(TEXT("LastValidCheckpoint")))
	{
		UpdateStatus();
	}
	Super::PostEditChangeProperty(PropertyChangedEvent);
...
void USeqEvent_Touch::UpdateStatus()
{
	bStatusIsOk = LastValidCheckpoint == NAME_None || Utils::IsCheckpointValid(LastValidCheckpoint);
}

#if WITH_EDITOR	
void USeqEvent_Touch::DrawExtraInfo(FCanvas* Canvas, const FVector& BoxCenter)
{
	Super::DrawExtraInfo(Canvas, BoxCenter);
...
UBOOL USeqEvent_Touch::CheckTouchActivate(AActor *InOriginator, AActor *InInstigator, UBOOL bTest)
{
	...
	// Dummy pawns and observer-excluded pawns must never activate Kismet triggers.
	if (InInstigator != NULL)
	{
		AOLHero* Hero = Cast<AOLHero>(InInstigator);
		if (Hero != NULL && Hero->bIsDummyPawn)
		{
			return FALSE;
		}
		APawn* InstigatorPawn = Cast<APawn>(InInstigator);
		if (InstigatorPawn != NULL && InstigatorPawn->Controller != NULL)
		{
			AOLPlayerController* InstigatorPC = Cast<AOLPlayerController>(InstigatorPawn->Controller);
			if (InstigatorPC != NULL && InstigatorPC->bExcludeFromKismetPlayer)
			{
				return FALSE;
			}
		}
...
void USeqVar_Player::UpdatePlayersList()
{
	...
	if( GWorld != NULL )
	{
		for (AController *Controller = GWorld->GetFirstController(); Controller != NULL; Controller = Controller->NextController)
		{
			...
			if (Controller->IsPlayerOwner() && !(PC && PC->bExcludeFromKismetPlayer))
			{
...
void USeqAct_ActorFactory::Activated()
{
	...
	if (InputLinks(0).bHasImpulse &&
		Factory != NULL)
	{
		// Observer-only: remote player triggered this factory; don't spawn on the local client.
		if (bObserverOnly)
			return;
		bIsSpawning = TRUE;
...
void USeqAct_ActivateRemoteEvent::Activated()
{
	...
	for (INT Idx = 0; Idx < RemoteEvents.Num(); Idx++)
	{
		...
		if (RemoteEvt != NULL && RemoteEvt->EventName == EventName )
		{
			...
			if ( RemoteEvt->bEnabled)
			{
				...
				UBOOL bActivated = RemoteEvt->CheckActivate(Originator,Instigator,FALSE,NULL);
	...
	if (GOLPC)
	{
		GOLPC->CheckForAchievementOnRemoteEvent(EventName);
		GOLPC->eventOnKismetRemoteEvent(EventName);
	}

	debugf(TEXT("KISMET: Remote event %s activated for %d recipients"), *EventName.ToString(), nbHits);
...
void USeqAct_CameraFade::Activated()
{
	...
	for (INT Idx = 0; Idx < Targets.Num(); Idx++)
	{
		AOLPlayerController* PC = Cast<AOLPlayerController>(Targets(Idx));
		FadeAlpha.X = 0.0f;
		...
		if (PC == NULL)
		{
			...
			if (Pawn != NULL)
			{
				PC = Cast<AOLPlayerController>(Pawn->Controller);
			}
		}
	...
	if (Targets.Num() == 0)
	{
		for (AController *Controller = GWorld->GetFirstController(); Controller != NULL; Controller = Controller->NextController)
		{
			AOLPlayerController* PC = Cast<AOLPlayerController>(Controller);
			FadeAlpha.X = 0.0f;
			if (PC != NULL)
			{
				if (PC->PlayerCamera != NULL)
...
//@zombie_ps4_begin
	else if( Platform == UE3::PLATFORM_Orbis)
	{
		DesiredOutputIndex = 14;
	}
//@zombie_ps4_end
	else
	{
		...
		for (INT I=0; I<Targets.Num(); ++I)
		{
			...
			if (TargetActor)
			{
				...
				if (MeshComponent)
				{
					...
					// rcharpentier
					HeadTrackingComp->DistUninterested = DistUninterested;
					HeadTrackingComp->DistInterested = DistInterested;
					HeadTrackingComp->TooFarWaitTime = TooFarWaitTime;
					HeadTrackingComp->TooLongWaitTime = TooLongWaitTime;
					HeadTrackingComp->bDisableBeyondLimit = bDisableBeyondLimit;					
					HeadTrackingComp->BlendInTime = BlendInTime;
					HeadTrackingComp->BlendOutTime = BlendOutTime;
