 ...
 */
#if 1 //ORBIS
UBOOL GRenderOnePassPointLightShadows = FALSE;
#else
...
public:
	...
	{
		MaterialParameters.RebindFromMap(ParameterMap);
		ShadowParameters.Bind(ParameterMap);
	}
...
public:
	...
	static UBOOL ShouldCache(EShaderPlatform Platform,const FMaterial* Material,const FVertexFactoryType* VertexFactoryType)
	{
		return FShadowDepthVertexShader::ShouldCache(Platform, Material, VertexFactoryType)
			...
			&& (ShaderMode != VertexShadowDepth_OutputDepthToColor || IsPCPlatform(Platform) || Platform == SP_WIIU || Platform == SP_DINGO)	//@igs(nrm):  Not sure if we need this cached on Dingo or not...
			// Only compile one pass point light shaders for SM5
//@zombie_ps4_begin
			&& (ShaderMode != VertexShadowDepth_OnePassPointLight || Platform == SP_PCD3D_SM4 || Platform == SP_PCD3D_SM5 || Platform == SP_ORBIS || Platform == SP_DINGO);
//@zombie_ps4_end
	}
...
public:
	...
	virtual void RebindParameters(const FShaderParameterMap& ParameterMap)
	{
		...
		InvMaxSubjectDepthParameter.Bind(ParameterMap, TEXT("InvMaxSubjectDepth"), TRUE);
		DepthBiasParameter.Bind(ParameterMap, TEXT("DepthBias"), TRUE);
		ShadowCasterPositionParameter.Bind(ParameterMap, TEXT("ShadowCasterPosition"), TRUE);
...
public:
	...
	static UBOOL ShouldCache(EShaderPlatform Platform,const FMaterial* Material,const FVertexFactoryType* VertexFactoryType)
	{
		return (FShadowDepthPixelShader::ShouldCache(Platform, Material, VertexFactoryType) ||
			...
			// Only compile one pass point light shaders for SM5
//@zombie_ps4_begin
			&& (ShaderMode != PixelShadowDepth_OnePassPointLight || Platform == SP_PCD3D_SM4 || Platform == SP_PCD3D_SM5 || Platform == SP_ORBIS || Platform == SP_DINGO);
//@zombie_ps4_end
	}
...
THREAD_LOCAL_STORAGE_MT const FProjectedShadowInfo* FShadowDepthDrawingPolicy::ShadowInfo = NULL;
...
//@zombie_ps4_begin
		(GRHIShaderPlatform == SP_PCD3D_SM5 || GRHIShaderPlatform == SP_ORBIS || GRHIShaderPlatform == SP_DINGO)
//@zombie_ps4_end
...
#if _WINDOWS || DINGO
	if (Initializer.DepthBias == 0.0f && Initializer.SlopeScaleDepthBias == 0.0f)
	{
		switch (Initializer.CullMode)
		{
		case CM_None : RHISetRasterizerState(TStaticRasterizerState<FM_Solid, CM_None>::GetRHI()); break;
		case CM_CCW  : RHISetRasterizerState(TStaticRasterizerState<FM_Solid, CM_CCW >::GetRHI()); break;
		case CM_CW   : RHISetRasterizerState(TStaticRasterizerState<FM_Solid, CM_CW  >::GetRHI()); break;
		}
	}
	else
#endif
...
#if ORBIS || DINGO
UBOOL GEnableShadowMapCaching = TRUE;
#else
UBOOL GEnableShadowMapCaching = FALSE;
#endif
...
void FProjectedShadowInfo::RenderDepth(const FSceneRenderer* SceneRenderer, BYTE DepthPriorityGroup, UBOOL bTranslucentPreShadow)
{
	...
	UBOOL bNeedsToClearDepth = FALSE;
	...
	else
	{
...
#endif
		{
			...
			{
// @zombie_ps4_begin - ccooper 9/27/2013 - depth clear doesn't work with viewport areas; the whole buffer was cleared at the beginning
#if !ORBIS
				if (bUsesCachedShadowMap)
				{
					// Lazy clear when shadow map caching is enabled.
					bNeedsToClearDepth = TRUE;
				}
				else
				{
					// Clear depth only.
...
// @zombie_ps4_end
...
#if DINGO
	// On dingo, creating many tiny threads incur a serious overhead so we will 
	// do 2-3 draw calls that mod shadows usually have on the render thread directly.
	UBOOL bAllowRenderOnThread = LightSceneInfo->LightShadowMode != LightShadow_Modulate;
#else
	UBOOL bAllowRenderOnThread = TRUE;
#endif

	FScene* Scene = SceneRenderer->Scene;
	const FViewInfo& View = *FoundView;
	BYTE DPGIndex = DepthPriorityGroup;
	...
	if (IsWholeSceneDominantShadow())
	{
		...
		START_DEFERRED_CONTEXT_THREAD(WholeSceneDominantShadowDepth, FProjectedShadowInfo,
			!SceneRenderer->Scene->DPGs[DPGIndex].WholeSceneShadowDepthDrawList.IsEmpty(), DPGIndex == 1 && !SceneRenderer->bIsSceneCapture,
		{
			SCOPED_DRAW_EVENT(WholeSceneDominantShadowDepth)(DEC_SCENE_ITEMS,TEXT("WholeSceneDominantShadowDepth"));
			FShadowDepthDrawingPolicy::ShadowInfo = This;

			// Use the scene's shadow depth draw list with this shadow's visibility map
			Scene->DPGs[DPGIndex].WholeSceneShadowDepthDrawList.DrawVisible(View, This->StaticMeshWholeSceneShadowDepthMap);
		});
	}
	else if (SubjectMeshElements.Num() > 0)
	{
		UBOOL bRestoredShadowMap = FALSE;
		if (bUsesCachedShadowMap)
		{
			bRestoredShadowMap = const_cast<FLightSceneInfo*>(LightSceneInfo)->RestoreShadowDepthFromCache(bOnePassPointLightShadow, SplitIndex, CachedShadowMapResolution);
			if (bRestoredShadowMap)
			{
				bNeedsToClearDepth = FALSE;
			}
		}

		if (!bRestoredShadowMap)
		{	
			// We did not restore the shadow map, so let's clear and render.
			if (bNeedsToClearDepth)
			{
				RHIClear(FALSE,FColor(255,255,255),TRUE,1.0f,FALSE,0);
				bNeedsToClearDepth = FALSE;
			}
...
#if DINGO
			UBOOL bStaticAllowRenderOnThread = SubjectMeshElements.Num() > 10;
#else
			UBOOL bStaticAllowRenderOnThread = TRUE;
#endif

			START_DEFERRED_CONTEXT_THREAD_TWO_PARAMETERS(WholeSceneDominantShadowDepth, FProjectedShadowInfo, UBOOL, bTranslucentPreShadow, UBOOL, bOnePassPointLightShadow, 
				TRUE, DPGIndex == 1 && !SceneRenderer->bIsSceneCapture && bAllowRenderOnThread && bStaticAllowRenderOnThread,
			{
				SCOPED_DRAW_EVENT(ShadowsStaticMeshThread)(DEC_SCENE_ITEMS,TEXT("ShadowsStaticMeshThread"));

				FShadowDepthDrawingPolicy::ShadowInfo = This;

				// Draw the subject's static elements.
				const FShadowStaticMeshElement& FirstShadowMesh = This->SubjectMeshElements(0);
				...
				FShadowDepthDrawingPolicy SharedDrawingPolicy(
					...
					*FirstShadowMesh.RenderProxy->GetMaterial(),
					...
					This->bFullSceneShadow,
					This->bDirectionalLight,
					FALSE,
				...
				SharedDrawingPolicy.DrawShared(&View, SharedDrawingPolicy.CreateBoundShaderState(FirstShadowMesh.Mesh->GetDynamicVertexStride()));
				if (View.StaticMeshShadowDepthMap(FirstShadowMesh.Mesh->Id))
				{
					if( FirstShadowMesh.Mesh->Elements.Num() == 1 )
					{
						SharedDrawingPolicy.SetMeshRenderState(View, FirstShadowMesh.Mesh->PrimitiveSceneInfo, *FirstShadowMesh.Mesh, 0, FALSE, FMeshDrawingPolicy::ElementDataType());
					...
					else
					{
						...
						FirstShadowMesh.Mesh->VertexFactory->GetStaticBatchElementVisibility(View, FirstShadowMesh.Mesh, BatchesToRender);

						for (INT Index=0;Index<BatchesToRender.Num();Index++)
						{
							INT BatchElementIndex = BatchesToRender(Index);
							SharedDrawingPolicy.SetMeshRenderState(View, FirstShadowMesh.Mesh->PrimitiveSceneInfo, *FirstShadowMesh.Mesh, BatchElementIndex, FALSE, FMeshDrawingPolicy::ElementDataType());
							SharedDrawingPolicy.DrawMesh(*FirstShadowMesh.Mesh, BatchElementIndex);
						}
					}
				}

				for (INT ElementIndex = 1; ElementIndex < This->SubjectMeshElements.Num(); ElementIndex++)
				{
					const FShadowStaticMeshElement& ShadowMesh = This->SubjectMeshElements(ElementIndex);
					...
					if (View.StaticMeshShadowDepthMap(ShadowMesh.Mesh->Id))
					{
							...
							|| SharedDrawingPolicy.IsReversingCulling() != ShadowMesh.Mesh->ReverseCulling)
						{
							...
							SharedDrawingPolicy.DrawShared(&View, SharedDrawingPolicy.CreateBoundShaderState(ShadowMesh.Mesh->GetDynamicVertexStride()));
						}
	...
	//#endif
						...
						{
							SharedDrawingPolicy.SetMeshRenderState(View, ShadowMesh.Mesh->PrimitiveSceneInfo, *ShadowMesh.Mesh, 0, FALSE, FMeshDrawingPolicy::ElementDataType());
						...
						else
						{
							...
							ShadowMesh.Mesh->VertexFactory->GetStaticBatchElementVisibility(View, ShadowMesh.Mesh, BatchesToRender);

							for (INT Index=0;Index<BatchesToRender.Num();Index++)
							{
								INT BatchElementIndex = BatchesToRender(Index);
								SharedDrawingPolicy.SetMeshRenderState(View, ShadowMesh.Mesh->PrimitiveSceneInfo, *ShadowMesh.Mesh, BatchElementIndex, FALSE, FMeshDrawingPolicy::ElementDataType());
			...
			if (bUsesCachedShadowMap)
			{
				const_cast<FLightSceneInfo*>(LightSceneInfo)->ResolveShadowDepthToCache(bOnePassPointLightShadow, SplitIndex, CachedShadowMapResolution);
			}		
		}
	}
...
#if DINGO
	UBOOL bDynamicAllowRenderOnThread = SubjectPrimitives.Num() > 5;
#else
	UBOOL bDynamicAllowRenderOnThread = TRUE;
#endif

	START_DEFERRED_CONTEXT_THREAD_TWO_PARAMETERS(DynamicShadowDepth, FProjectedShadowInfo, UBOOL, bTranslucentPreShadow, ESceneDepthPriorityGroup, DPGToUseForDepths,
		SubjectPrimitives.Num(), DPGIndex == 1 && !SceneRenderer->bIsSceneCapture && bAllowRenderOnThread && bDynamicAllowRenderOnThread,
	{
		SCOPED_DRAW_EVENT(ShadowsDynamicThread)(DEC_SCENE_ITEMS,TEXT("ShadowsDynamicThread"));
		
		FShadowDepthDrawingPolicy::ShadowInfo = This;

		// Draw the subject's dynamic elements.
		TDynamicPrimitiveDrawer<FShadowDepthDrawingPolicyFactory> Drawer(&View, DPGToUseForDepths, FShadowDepthDrawingPolicyFactory::ContextType(This, bTranslucentPreShadow), TRUE);
		for (INT PrimitiveIndex = 0; PrimitiveIndex < This->SubjectPrimitives.Num(); PrimitiveIndex++)
		{
			const FPrimitiveSceneInfo* PrimitiveSceneInfo = This->SubjectPrimitives(PrimitiveIndex);
			...
			{
				Drawer.SetPrimitive(This->SubjectPrimitives(PrimitiveIndex));
				This->SubjectPrimitives(PrimitiveIndex)->Proxy->DrawDynamicElements(&Drawer, &View, DPGToUseForDepths);
			}
		}
	});
}
...
void DrawStencilingVolume(const FLightSceneInfo* LightSceneInfo)
{
	SCOPED_DRAW_EVENT(DrawStencilingVolume)(DEC_SCENE_ITEMS,TEXT("Draw Stenciling Volume"));
	...
	check(bPointLight || bSpotLight);

	WORD* IndicesPtr = NULL;
	UINT  NumIndices = 0;

	TArray<FVector>& VertexArray = const_cast<TArray<FVector>&>(LightSceneInfo->StencilVolumeVertices);

	const UBOOL bVolumeNeedUpdate = LightSceneInfo->StencilVolumeVertices.Num() == 0;
	...
	if (bSpotLight)
	{
		...
		static UBOOL bConeIndicesInitialized = FALSE;
		static TArray<WORD, TInlineAllocator<(NumSlices - 1) * NumSides * 12> > ConeIndices;

		if (bVolumeNeedUpdate)
		{
			const FLOAT InvCosRadiansPerSide = 1.0f / appCos(PI / (FLOAT)NumSides);
			// Use Cos(Theta) = Adjacent / Hypotenuse to solve for the distance of the end of the cone along the cone's Z axis
			const FLOAT ZRadius = LightSceneInfo->GetRadius() * appCos(LightSceneInfo->GetOuterConeAngle());
			const FLOAT TanConeAngle = appTan(LightSceneInfo->GetOuterConeAngle());

			VertexArray.Reserve(NumSides * NumSlices * 2);
			...
			for (INT SliceIndex = 0; SliceIndex < NumSlices; SliceIndex++)
			{
				for (INT SideIndex = 0; SideIndex < NumSides; SideIndex++)
				{
					...
					const FVector LocalPosition(SliceRadius * appCos(CurrentAngle), SliceRadius * appSin(CurrentAngle), ZRadius * SliceIndex / (FLOAT)(NumSlices - 1));
					// Transform to world space and apply pre-view translation, since these vertices will be used with a shader that has pre-view translation removed
					VertexArray.AddItem(LightSceneInfo->LightToWorld.TransformFVector(LocalPosition));
				}
			}

			if (!bConeIndicesInitialized)
			{
				// Generate triangles for the vertices of the cone shape
				for (INT SliceIndex = 0; SliceIndex < NumSlices - 1; SliceIndex++)
				{
					for (INT SideIndex = 0; SideIndex < NumSides; SideIndex++)
					{
						...
						ConeIndices.AddItem(CurrentIndex);
						ConeIndices.AddItem(NextSideIndex);
						ConeIndices.AddItem(NextSliceIndex);
						ConeIndices.AddItem(NextSliceIndex);
						ConeIndices.AddItem(NextSideIndex);
						ConeIndices.AddItem(NextSliceAndSideIndex);
					}
				}
			}

			const INT CapIndexStart = VertexArray.Num();
			// Radius of the flat cap of the cone
			const FLOAT CapRadius = ZRadius * appTan(LightSceneInfo->GetOuterConeAngle());
			...
			for (INT SliceIndex = 0; SliceIndex < NumSlices; SliceIndex++)
			{
				...
				const FLOAT SliceRadius = UnadjustedSliceRadius * InvCosRadiansPerSide;
				// Solve for the Z axis distance that this slice should be at using the Pythagorean theorem
				const FLOAT ZDistance = appSqrt(Square(LightSceneInfo->GetRadius()) - Square(UnadjustedSliceRadius));
				for (INT SideIndex = 0; SideIndex < NumSides; SideIndex++)
				{
					const FLOAT CurrentAngle = SideIndex * 2 * PI / (FLOAT)NumSides;
					const FVector LocalPosition(SliceRadius * appCos(CurrentAngle), SliceRadius * appSin(CurrentAngle), ZDistance);
			...
			if (!bConeIndicesInitialized)
			{
				...
				bConeIndicesInitialized = TRUE;
			}
		}

		IndicesPtr = ConeIndices.GetData();
		NumIndices = ConeIndices.Num();
	}
	else
	{
		const INT NumSides = 18;
		...
		static TArray<FVector, TInlineAllocator<NumVerts> >             UnitSphereVerts;
		static TArray<WORD, TInlineAllocator<NumSides * NumRings * 6> > UnitSphereIndices;
		...
		if (bVolumeNeedUpdate)
		{
			...
			if( UnitSphereVerts.Num() == 0 )
			{
				...
				UnitSphereIndices.Empty(NumSides * NumRings * 6);
				// Add triangles for all the vertices generated
				for (INT s = 0; s < NumSides; s++)
				{
					const INT a0start = (s + 0) * (NumRings + 1);
					...
					for (INT r = 0; r < NumRings; r++)
					{
						UnitSphereIndices.AddItem(a0start + r + 0);
						UnitSphereIndices.AddItem(a1start + r + 0);
						UnitSphereIndices.AddItem(a0start + r + 1);
						UnitSphereIndices.AddItem(a1start + r + 0);
						UnitSphereIndices.AddItem(a1start + r + 1);
						UnitSphereIndices.AddItem(a0start + r + 1);
					}
				}
			}
			...
			const FVector* RESTRICT UnaliasedUnitSphereVerts = UnitSphereVerts.GetData();
			const FVector SpherePosition = LightBounds.Center;

			// Boost the effective radius so that the edges of the sphere approximation lie on the sphere, instead of the vertices
			const FLOAT Radius = LightBounds.W / appCos(RadiansPerRingSegment);

			// @todo: Use vertex shader to transform verts to avoid all CPU cost here!
			VertexArray.Init( NumVerts );
			for( INT VertIndex = 0; VertIndex < NumVerts; ++VertIndex )
			{
				VertexArray( VertIndex ) = SpherePosition + Radius * UnaliasedUnitSphereVerts[ VertIndex ];
			}
		}

		IndicesPtr = UnitSphereIndices.GetData();
		NumIndices = UnitSphereIndices.Num();
	}

	check (IndicesPtr != NULL && NumIndices > 0);

	RHIDrawIndexedPrimitiveUP(PT_TriangleList, 0, 
		VertexArray.Num(), NumIndices / 3, IndicesPtr, sizeof(WORD), 
		VertexArray.GetData(), 
		VertexArray.GetTypeSize());
}
...
void FProjectedShadowInfo::RenderProjection(INT ViewIndex, const FViewInfo* View, BYTE DepthPriorityGroup, UBOOL bRenderingBeforeLight, UBOOL bStencilAlreadyLoaded) const
{
	...
	else
	{
		...
		if (GUseHiStencil && !bStencilAlreadyLoaded)
		{
			RHIBeginHiStencilRecord(TRUE, 0);
		}
...
//@zombie_ps4_begin
			FBranchingPCFProjectionPixelShaderInterface* BranchingPCFPixelShader = SetBranchingPCFParameters(ViewIndex,*View,this,LightSceneInfo->ShadowFilterQuality);
//@zombie_ps4_end
...
//@zombie_ps4_begin
			BranchingPCFPixelShader->SetParameters(ViewIndex, *View, this);
//@zombie_ps4_end
		} 
		else
		{
...
//@zombie_ps4_begin
			if ((GRHIShaderPlatform == SP_PCD3D_SM5 || GRHIShaderPlatform == SP_ORBIS || GRHIShaderPlatform == SP_DINGO) && GSystemSettings.UsesMSAA())
//@zombie_ps4_end
			{
...
//@zombie_ps4_end
	...
	if (GSystemSettings.bAllowSkin && bModulatedShadow)
	{
...
FMatrix FProjectedShadowInfo::GetScreenToShadowMatrix(const FSceneView& View, UBOOL bTranslucentPreShadow, UBOOL bForcePostProjection) const
{
	const FIntPoint ShadowBufferResolution = bUsesCachedShadowMap ? FIntPoint(CachedShadowMapResolution, CachedShadowMapResolution) : GetShadowBufferResolution(bTranslucentPreShadow);
	...
	FMatrix ScreenToShadow = bTranslucentPreShadow || bForcePostProjection ? 
		FMatrix::Identity : 
...
FIntPoint FProjectedShadowInfo::GetShadowBufferResolution(UBOOL bTranslucentPreShadow) const
{
	...
	else
	{
		...
		else
		{
			if (bUsesCachedShadowMap)
			{
				return FIntPoint(CachedShadowMapResolution, CachedShadowMapResolution);
			}
			else
			{
#if ORBIS
				return GSceneRenderTargets.GetShadowDepthTextureResolution(IsPrimaryWholeSceneDominantShadow(), !GRenderOnePassPointLightShadows && IsWholeScenePointLightShadow());
#else
...
UBOOL FSceneRenderer::AreDominantShadowsActive(UINT DPGIndex, UBOOL bBeforePrePass) const
{
	if (GOnePassDominantLight && (ViewFamily.ShowFlags & SHOW_Lighting))
	{
		for(TSparseArray<FLightSceneInfoCompact>::TConstIterator LightIt(Scene->Lights);LightIt;++LightIt)
		{
				...
				&& LightSceneInfo->LightShadowMode == LightShadow_Normal)
			{
				...
				// If we are before the prepass, we perform a super basic frustum culling here.
				if (bBeforePrePass)
				{
					UBOOL bLightIsVisibleBeforePrePass = FALSE;

					if (LightSceneInfo->LightType == (BYTE)LightType_DominantDirectional)
					{
						bLightIsVisibleBeforePrePass = TRUE;
					}
					else
					{
						for(INT ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
						{
							...
							if (View.ViewFrustum.IntersectSphere(LightSceneInfo->GetPosition(), Min(LightSceneInfo->GetRadius(), (FLOAT)WORLD_MAX)))
							{
								bLightIsVisibleBeforePrePass = TRUE;
								break;
							}
					...
					if (ViewFamily.ShouldDrawShadows() && bLightIsVisibleBeforePrePass && LightSceneInfo->LightingChannels.Bitfield != 1)
					{
						return TRUE;
					}
				}
...
#if ORBIS
						// mathieu.gauthier: zombie disables clears on shadows since they clear
						// the entire buffer before rendering the shadow depth. This code path
						// is not used by them and we handle the clear here.
						if (GOnePassDominantLight)
						{
							// Clear depth only.
							RHIClear(FALSE,FColor(255,255,255),TRUE,1.0f,FALSE,0);
	...
	UBOOL bStencilAlreadyLoaded)
{
	...
	for (INT ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		for (INT ShadowIndex = 0; ShadowIndex < Shadows.Num(); ShadowIndex++)
		{
				...
				)
			{
				...
				{
					ProjectedShadowInfo->RenderProjection(ViewIndex, &View, DPGIndex, bRenderingBeforeLight, bStencilAlreadyLoaded);
				}
			}
		}
	...
	if (!(LightSceneInfo->bNonModulatedSelfShadowing && bRenderingBeforeLight) && 
		LightSceneInfo->LightShadowMode == LightShadow_Modulate && GSystemSettings.bAllowSkin)
	{
 ...
 */
UBOOL FSceneRenderer::RenderProjectedShadows( const FLightSceneInfo* LightSceneInfo, UINT DPGIndex, UBOOL bRenderingBeforeLight, UBOOL bAllowSkipProjection, UBOOL bStencilAlreadyLoaded )
{
	SCOPE_CONDITIONAL_CYCLE_COUNTER(STAT_ProjectedShadowDrawTime, !bIsSceneCapture);
...
#if ORBIS
	UBOOL bIsPointLight = bRenderingBeforeLight && Shadows.Num() > 0 && Shadows(0)->IsWholeScenePointLightShadow();
#else
	UBOOL bIsPointLight = FALSE;
#endif

	UBOOL bRenderProjections = !bAllowSkipProjection || Shadows.Num() > 1;
	...
	while (NumShadowsRendered < Shadows.Num())
	{
		...
		else
		{
			...
			const FIntPoint ShadowBufferResolution = GSceneRenderTargets.GetShadowDepthTextureResolution(FALSE, !GRenderOnePassPointLightShadows && bIsPointLight);
		...
		if (bRenderDepthPass)
		{
			...
			INT ShadowResolution = -1;
			if (Shadows.Num() == 1 && Shadows(0)->bUsesCachedShadowMap)
			{
				check(Shadows(0)->X == 0 && Shadows(0)->Y == 0);
				check(Shadows(0)->ResolutionX == Shadows(0)->ResolutionY);
				ShadowResolution = Shadows(0)->CachedShadowMapResolution;
			}
			GSceneRenderTargets.BeginRenderingShadowDepth(bRenderingWholeSceneDominantShadow, bIsPointLight, ShadowResolution);

// @zombie_ps4_begin - ccooper 9/27/2013 - depth clear doesn't work with viewport areas, clear the whole buffer at the beginning
#if ORBIS
			FSurfaceRHIParamRef ShadowDepthSurface = GSceneRenderTargets.GetShadowDepthZSurface(bRenderingWholeSceneDominantShadow, bIsPointLight, ShadowResolution);
			FOrbisRHISurface* OrbisSurface = ShadowDepthSurface;
			if(OrbisSurface)
			{
				RHISetViewport(0.0f, 0.0f, 0.0f, 
					OrbisSurface->GetDepthTarget()->getWidth(),
					OrbisSurface->GetDepthTarget()->getHeight(),
					1.0f
					);
			}
			RHIClear(FALSE,FColor(255,255,255),TRUE,1.0f,FALSE,0);
...
// @zombie_ps4_end
			...
			GSceneRenderTargets.FinishRenderingShadowDepth(bRenderingWholeSceneDominantShadow, bIsPointLight, ResolveRect);
		}

		// Render the shadow projections.
		if (bRenderProjections)
		{
			SCOPED_DRAW_EVENT(EventShadowProj)(DEC_SCENE_ITEMS,TEXT("Shadow Projection"));
			RenderProjections(LightSceneInfo, Shadows, DPGIndex, bRenderingBeforeLight, bStencilAlreadyLoaded);
...
//@zombie_ps4_begin
	if (GRHIShaderPlatform == SP_PCD3D_SM4 || GRHIShaderPlatform == SP_PCD3D_SM5 || GRHIShaderPlatform == SP_ORBIS || GRHIShaderPlatform == SP_DINGO)
//@zombie_ps4_end
	{
		bAttenuationBufferDirty |= RenderOnePassPointLightShadows(LightSceneInfo, DPGIndex, bRenderingBeforeLight, bAllowSkipProjection);
	}
