...
/** mathieu.gauthier: Same as 2 above, but with caching and without pre-view translation (done in shader). */
extern void DrawStencilingVolume(const FLightSceneInfo* LightSceneInfo);
...
public:
	 ...
	 */
	THREAD_LOCAL_STORAGE_MT static const FProjectedShadowInfo* ShadowInfo;
...
public:
	...
	/** mathieu.gauthier: will this shadow restore/resolve to the cache? */
	UBOOL bUsesCachedShadowMap;
	INT CachedShadowMapResolution;
...
public:
	 ...
	 */
	void RenderProjection(INT ViewIndex, const class FViewInfo* View, BYTE DepthPriorityGroup, UBOOL bRenderingBeforeLight, UBOOL bStencilAlreadyLoaded = FALSE) const;
	...
	*/
	...
	FMatrix GetScreenToShadowMatrix(const FSceneView& View, UBOOL bTranslucentPreShadow, UBOOL bForcePostProjection = FALSE) const;
...
public:
	...
	{
//@zombie_ps4_begin
		return (Platform == SP_PCD3D_SM5 || Platform == SP_ORBIS || Platform == SP_DINGO);
//@zombie_ps4_end
	}
};
...
public:
	...
	UINT GetMatrixBaseIndex() const { return ScreenToShadowMatrixParameter.GetBaseIndex(); }
	UINT GetBufSizeBaseIndex() const { return ShadowBufferSizeAndSoftTransitionScaleParameter.GetBaseIndex(); }
	UINT GetTexelBaseIndex() const { return ShadowTexelSizeParameter.GetBaseIndex(); }
	...
	void Set(FShader* Shader, const FSceneView& View, const FProjectedShadowInfo* ShadowInfo, UBOOL bUseHardwarePCF, UBOOL bUseFetch4)
	{
		...
		const FIntPoint ShadowBufferResolution = ShadowInfo->bUsesCachedShadowMap ?
			FIntPoint(ShadowInfo->CachedShadowMapResolution, ShadowInfo->CachedShadowMapResolution) :
			ShadowInfo->GetShadowBufferResolution(FALSE);
		if (ShadowBufferSizeAndSoftTransitionScaleParameter.IsBound() || ShadowTexelSizeParameter.IsBound())
		{
			...
			{
				TransitionScale = ShadowInfo->LightSceneInfo->ContactShadowTransition; //GSystemSettings.PerObjectShadowTransition;
			}
...
#if ORBIS
			//sample the depth texture
			ShadowDepthSampler = GSceneRenderTargets.GetShadowDepthZTexture(ShadowInfo->IsPrimaryWholeSceneDominantShadow(), ShadowInfo->bAllocatedInPreshadowCache, ShadowInfo->IsWholeScenePointLightShadow(), ShadowBufferResolution.X);
#elif DINGO
			//sample the depth texture
			ShadowDepthSampler = GSceneRenderTargets.GetShadowDepthZTexture(ShadowInfo->IsPrimaryWholeSceneDominantShadow(), ShadowInfo->bAllocatedInPreshadowCache, FALSE, ShadowBufferResolution.X);
#else
			//sample the depth texture
			ShadowDepthSampler = GSceneRenderTargets.GetShadowDepthZTexture(ShadowInfo->IsPrimaryWholeSceneDominantShadow(), ShadowInfo->bAllocatedInPreshadowCache, FALSE);
#endif
		} 
		else if (bUseFetch4)
		{
...
public:
	...
	virtual void SetLightChannelParameter(const FProjectedShadowInfo* ShadowInfo)
	{
		...
		if (ShouldUseDeferredShading() && ShadowInfo->LightSceneInfo->LightingChannels.GetDeferredShadingChannelMask() > 0)
		{
			...
			const UINT Mask = (ShadowInfo->bSelfShadowOnly || ShadowInfo->LightSceneInfo->bNonModulatedSelfShadowing) ? 
	...
	{
		ProjectionParameters.Bind(ParameterMap);
		SampleOffsetsParameter.Bind(ParameterMap, TEXT("SampleOffsets"), TRUE);
		ShadowFadeFractionParameter.Bind(ParameterMap, TEXT("ShadowFadeFraction"), TRUE);
		LightingChannelMaskParameter.Bind(ParameterMap, TEXT("LightingChannelMask"), TRUE);
	}
...
public:
	...
	*/
	...
	virtual void RebindParameters(const FShaderParameterMap& ParameterMap)
	{
		TShadowProjectionPixelShader<UniformPCFPolicy>::RebindParameters(ParameterMap);
		ShadowModulateColorParam.Bind(ParameterMap, TEXT("ShadowModulateColor"), TRUE);
		ScreenToWorldParam.Bind(ParameterMap, TEXT("ScreenToWorld"), TRUE);
		LightTypePolicy::ModShadowPixelParamsType::Bind(ParameterMap);
	}
...
public:
	...
	{
//@zombie_ps4_begin
		return (Platform == SP_PCD3D_SM4 || Platform == SP_PCD3D_SM5 || Platform == SP_ORBIS || Platform == SP_DINGO);
//@zombie_ps4_end
	}
...
//@zombie_ps4_begin
		&& (GRHIShaderPlatform == SP_PCD3D_SM4 || GRHIShaderPlatform == SP_PCD3D_SM5 || GRHIShaderPlatform == SP_ORBIS || GRHIShaderPlatform == SP_DINGO)
//@zombie_ps4_end
