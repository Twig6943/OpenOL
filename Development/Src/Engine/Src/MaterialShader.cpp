...
FString GetLightingModelString(EMaterialLightingModel LightingModel)
{
	...
	switch(LightingModel)
	{
		...
		case MLM_Skin: LightingModelName = TEXT("MLM_Skin"); break;
...
const void* GRebindOwner = NULL;

void FMaterialShaderParameters::RebindFromMap(const FShaderParameterMap& ParameterMap, EShaderFrequency Frequency)
{
	const TCHAR* ShaderFrequencyName = GetShaderFrequencyName(Frequency);

	// Ребиндинг всех параметров из байткода 868-шейдера.
	// SetShaderValue для этих параметров вызывается без IsBound() — поэтому важно
	// чтобы BaseIndex был правильным (из 868-байткода), а не нулём.
	LocalToWorldParameter.Bind(ParameterMap, TEXT("LocalToWorldMatrix"), TRUE);
	WorldToLocalParameter.Bind(ParameterMap, TEXT("WorldToLocalMatrix"), TRUE);
	CameraWorldPositionParameter.Bind(ParameterMap, TEXT("CameraWorldPos"), TRUE);
	WorldToViewParameter.Bind(ParameterMap, TEXT("WorldToViewMatrix"), TRUE);
	...
	FoliageImpulseDirectionParameter.Bind(ParameterMap, TEXT("FoliageImpulseDirection"), TRUE);
	FoliageNormalizedRotationAxisAndAngleParameter.Bind(ParameterMap, TEXT("FoliageNormalizedRotationAxisAndAngle"), TRUE);
	SkinDiffuseWeightParameters.Bind(ParameterMap, TEXT("SkinDiffuseWeight"), TRUE);

	// Если массив пуст (шейдер не десериализовал параметры из-за SIZE mismatch) —
	// заполняем его из bytecode: ищем все UniformXxxScalars_N в RebindMap
	if (UniformScalarShaderParameters.Num() == 0)
	{
		for (INT Idx = 0; Idx < 16; Idx++)
		{
			FString ParameterName = FString::Printf(TEXT("Uniform%sScalars_%u"), ShaderFrequencyName, (UINT)Idx);
			FShaderParameter Param;
			Param.Bind(ParameterMap, *ParameterName, TRUE);
			if (Param.IsBound())
			{
				TUniformParameter<FShaderParameter>* Entry = new(UniformScalarShaderParameters) TUniformParameter<FShaderParameter>();
				Entry->Index = Idx;
				Entry->ShaderParameter = Param;
			}
		}
	}

	// Ребиндинг скалярных float параметров
	for (INT i = 0; i < UniformScalarShaderParameters.Num(); i++)
	{
		INT Idx = UniformScalarShaderParameters(i).Index;
		FString ParameterName = FString::Printf(TEXT("Uniform%sScalars_%u"), ShaderFrequencyName, (UINT)Idx);
		UniformScalarShaderParameters(i).ShaderParameter.Bind(ParameterMap, *ParameterName, TRUE);
	}

	// Если массив пуст — заполняем из bytecode
	if (UniformVectorShaderParameters.Num() == 0)
	{
		for (INT Idx = 0; Idx < 32; Idx++)
		{
			FString ParameterName = FString::Printf(TEXT("Uniform%sVector_%u"), ShaderFrequencyName, (UINT)Idx);
			FShaderParameter Param;
			Param.Bind(ParameterMap, *ParameterName, TRUE);
			if (Param.IsBound())
			{
				TUniformParameter<FShaderParameter>* Entry = new(UniformVectorShaderParameters) TUniformParameter<FShaderParameter>();
				Entry->Index = Idx;
				Entry->ShaderParameter = Param;
			}
		}
	...
	// Ребиндинг векторных float параметров
	for (INT i = 0; i < UniformVectorShaderParameters.Num(); i++)
	{
		INT Idx = UniformVectorShaderParameters(i).Index;
		FString ParameterName = FString::Printf(TEXT("Uniform%sVector_%u"), ShaderFrequencyName, (UINT)Idx);
		UniformVectorShaderParameters(i).ShaderParameter.Bind(ParameterMap, *ParameterName, TRUE);
	}

	// Если массив пуст — заполняем из bytecode
	if (Uniform2DShaderResourceParameters.Num() == 0)
	{
		for (INT Idx = 0; Idx < 32; Idx++)
		{
			FString ParameterName = FString::Printf(TEXT("%sTexture2D_%u"), ShaderFrequencyName, (UINT)Idx);
			FShaderResourceParameter Param;
			Param.Bind(ParameterMap, *ParameterName, TRUE);
			if (Param.IsBound())
			{
				TUniformParameter<FShaderResourceParameter>* Entry = new(Uniform2DShaderResourceParameters) TUniformParameter<FShaderResourceParameter>();
				Entry->Index = Idx;
				Entry->ShaderParameter = Param;
			}
		}
	...
	// Ребиндинг 2D texture samplers
	for (INT i = 0; i < Uniform2DShaderResourceParameters.Num(); i++)
	{
		INT Idx = Uniform2DShaderResourceParameters(i).Index;
		FString ParameterName = FString::Printf(TEXT("%sTexture2D_%u"), ShaderFrequencyName, (UINT)Idx);
		Uniform2DShaderResourceParameters(i).ShaderParameter.Bind(ParameterMap, *ParameterName, TRUE);
	}
}
	...
	FShaderFrequencyUniformExpressionValues& InValues) const
{
	...
	TScopeHashLock<FMaterialRenderProxy> CacheLock(&FMaterialRenderProxy::CacheCriticalSectionHash, MaterialRenderContext.MaterialRenderProxy);
#endif
	...
	for (INT ParameterIndex = 0; ParameterIndex < UniformVectorShaderParameters.Num(); ParameterIndex++)
	{
		...
		// LOG: что пишется в UniformPixelVector_0 (c0) для шейдеров
		if (UniformParameter.Index == 0 && UniformParameter.ShaderParameter.GetBaseIndex() == 0)
		{
			static UBOOL bLoggedVec0 = FALSE;
			if (!bLoggedVec0)
			{
				bLoggedVec0 = TRUE;
				warnf(TEXT("UVEC0_SET: BaseIndex=%d Value=(%.4f,%.4f,%.4f,%.4f)"),
					UniformParameter.ShaderParameter.GetBaseIndex(),
...
void FMaterialPixelShaderParameters::RebindFromMap(const FShaderParameterMap& ParameterMap)
{
	FMaterialShaderParameters::RebindFromMap(ParameterMap, SF_Pixel);

	// Rebind cube texture sampler registers by reconstructed name.
	for (INT i = 0; i < UniformPixelCubeShaderResourceParameters.Num(); i++)
	{
		FString ParameterName = FString::Printf(TEXT("PixelTextureCube_%u"), i);
		UniformPixelCubeShaderResourceParameters(i).ShaderParameter.Bind(ParameterMap, *ParameterName, TRUE);
	}
...
void FMaterialPixelShaderParameters::Set(FShader* PixelShader,const FMaterialRenderContext& MaterialRenderContext, ESceneDepthUsage DepthUsage/*=SceneDepthUsage_Normal*/) const
{
	...
	if (SkinDiffuseWeightParameters.IsBound())
	{
		SetPixelShaderValue(
			PixelShaderRHI,
			SkinDiffuseWeightParameters,
			GSystemSettings.bAllowSkin && !MaterialRenderContext.View->bHasOutlastNightVision ? 0.0f : 1.0f);
	}
}
...
FArchive& operator<<(FArchive& Ar,FMaterialPixelShaderParameters& Parameters)
{
	...
	if (Ar.LicenseeVer() >= VER_LIC_SKIN_SHADING)
	{
		Ar << Parameters.SkinDiffuseWeightParameters;
	}
...
void FMaterialVertexShaderParameters::RebindFromMap(const FShaderParameterMap& ParameterMap)
{
	FMaterialShaderParameters::RebindFromMap(ParameterMap, SF_Vertex);
}
...
static void RebindShaderIfNeeded(FShader* Shader, const FString& MatName = TEXT(""), FVertexFactoryType* VFType = NULL)
{
	if (!Shader || Shader->GetCode().Num() == 0)
		return;
...
void FMaterialShaderMap::Serialize(FArchive& Ar)
{
	...
	if(Ar.IsLoading())
	{
		...
		if (Ar.Ver() < GPackageFileVersion)
		{
			// Ребиндим material shaders (FMaterialShaderType)
			FString FriendlyName = GetFriendlyName();
			TMap<FGuid,FShader*> MatShaders;
			TShaderMap<FMaterialShaderType>::GetShaderList(MatShaders);
			for (TMap<FGuid,FShader*>::TIterator It(MatShaders); It; ++It)
				RebindShaderIfNeeded(It.Value(), FriendlyName);

			// Ребиндим mesh material shaders (FMeshMaterialShaderType × VF)
			for (INT MeshIdx = 0; MeshIdx < MeshShaderMaps.Num(); MeshIdx++)
			{
				FVertexFactoryType* VFType = MeshShaderMaps(MeshIdx).GetVertexFactoryType();
				TMap<FGuid,FShader*> MeshShaders;
				MeshShaderMaps(MeshIdx).GetShaderList(MeshShaders);
				for (TMap<FGuid,FShader*>::TIterator It(MeshShaders); It; ++It)
				{
					RebindShaderIfNeeded(It.Value(), FriendlyName, VFType);
				}
			}
		}
	}
