...
namespace UnrealBuildTool
{
	class BuildConfiguration
	{
		...
		public static double ProcessorCountMultiplier = Utils.GetEnvironmentVariable("ue3.ProcessorCountMultiplier", 2.0);
		...
		public static void ValidateConfiguration( CPPTargetConfiguration Configuration, CPPTargetPlatform Platform )
		{
			...
			if( bUseIncrementalLinking )
			{
				...
				if( Platform == CPPTargetPlatform.Win32 || Platform == CPPTargetPlatform.Win64 || 
					Platform == CPPTargetPlatform.Metro || Platform == CPPTargetPlatform.Dingo)
				{
					bUsePDBFiles = true;
				}
				// Disable.
...
//@zombie_ps4_begin
			// Orbis
			if (Platform == CPPTargetPlatform.Orbis)
			{
				bUseUnityBuild = true;
				bAllowXGE = false;
			}
