...
//@zombie_ps4_end
	...
	class UE3ProjectDesc
	{
		...
		/** Extra include paths added only for this project's compilation environment */
		public List<string> AdditionalIncludePaths = new List<string>();
...
//@zombie_ps4_begin
			...
			{
				OutputItems = GetOrbisOutputItems();
			}
//@zombie_ps4_end
			else
			{
...
//@zombie_ps4_begin
					case "ORBIS":
						Platform = UnrealTargetPlatform.Orbis;
						break;
//@zombie_ps4_end
					case "MAC":
					...
					case "METRO":
						...
						break;
					case "DINGO":
						Platform = UnrealTargetPlatform.Dingo;
						break;
					...
					// -Intermediate <path> overrides the base intermediate directory.
					case "-INTERMEDIATE":
						if (ArgumentIndex + 1 >= Arguments.Count)
						{
							throw new BuildException("Expected path after -intermediate argument, but found nothing.");
						}
						ArgumentIndex++;
						BuildConfiguration.BaseIntermediatePath = Arguments[ArgumentIndex];
						break;
			...
			// Enable lean and mean for the Flash platform && force lean and mean for Metro
			if( Platform == UnrealTargetPlatform.Flash || Platform == UnrealTargetPlatform.Metro )
			{
				UE3BuildConfiguration.bCompileLeanAndMeanUE3 = true;
			}
		...
		void SetUpGlobalEnvironment()
		{
			...
			// Compile ImGui as a separate isolated project (avoids Unity batch contamination).
			SetUpImGuiEnvironment();

			// Compile OpenOL embedded relay server as a separate isolated static lib.
			SetUpOpenOLRelayEnvironment();
            ...
            if (Platform == UnrealTargetPlatform.Dingo)
            {
                GlobalCPPEnvironment.Definitions.Add("DINGO_USE_MONOLITHIC_DRIVER=1");
            }

            // display planes only supported on Mono driver
            if (Platform == UnrealTargetPlatform.Dingo && UE3BuildConfiguration.bEnableHUDSwapChain)
            {
                GlobalCPPEnvironment.Definitions.Add("DINGO_USE_HUD_SWAP_CHAIN=1");
            }
            else
            {
                GlobalCPPEnvironment.Definitions.Add("DINGO_USE_HUD_SWAP_CHAIN=0");
            }

            if (Platform == UnrealTargetPlatform.Dingo && UE3BuildConfiguration.bEnable60HzHUD && UE3BuildConfiguration.bEnableHUDSwapChain)
            {
                GlobalCPPEnvironment.Definitions.Add("DINGO_USE_60HZ_HUD=1");
            }
            else
            {
                GlobalCPPEnvironment.Definitions.Add("DINGO_USE_60HZ_HUD=0");
            }
        }
...
//@zombie_ps4_begin
				case UnrealTargetPlatform.Orbis: MainCompilePlatform = CPPTargetPlatform.Orbis; break;
//@zombie_ps4_end
				case UnrealTargetPlatform.Metro: MainCompilePlatform = CPPTargetPlatform.Metro; break;
				case UnrealTargetPlatform.Dingo: MainCompilePlatform = CPPTargetPlatform.Dingo; break;
...
// end WITH_GFx
			...
			bool bForceLinkerDebugInfo =
				( Platform == UnrealTargetPlatform.Win32 ||
				  Platform == UnrealTargetPlatform.Win64 );
			FinalLinkEnvironment.bCreateDebugInfo = GlobalCPPEnvironment.bCreateDebugInfo || bForceLinkerDebugInfo && false;
