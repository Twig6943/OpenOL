	...
	class VCToolChain
	{
		static string GetCLArguments_Global(CPPEnvironment CompileEnvironment)
		{
			...
			if( CompileEnvironment.TargetConfiguration == CPPTargetConfiguration.Debug )
			{
				...
				bUseDebugRuntimeLibrary = CompileEnvironment.TargetPlatform == CPPTargetPlatform.Win32 || CompileEnvironment.TargetPlatform == CPPTargetPlatform.Win64;
				...
				if( CompileEnvironment.TargetPlatform == CPPTargetPlatform.Win32 || CompileEnvironment.TargetPlatform == CPPTargetPlatform.Win64 || 
					CompileEnvironment.TargetPlatform == CPPTargetPlatform.Metro || CompileEnvironment.TargetPlatform == CPPTargetPlatform.Dingo)
				{
			...
			{
                if (CompileEnvironment.TargetPlatform == CPPTargetPlatform.Dingo)
                {
                    // Perfer size over speed on Dingo for the out-of-order execution of the CPU.
                    Result += " /O1";
                    Result += " /Oi";
                }
                else
                ...
                // Better, undocumented, debug info (http://randomascii.wordpress.com/2013/09/11/debugging-optimized-codenew-in-visual-studio-2012/).
                Result += " /d2Zi+";
			...
			//	WinRT
			//
			if (UE3BuildConfiguration.bCompileWinRT || CompileEnvironment.TargetPlatform == CPPTargetPlatform.Metro)
			{
            ...
            else if (CompileEnvironment.TargetPlatform == CPPTargetPlatform.Dingo)
            {
                ...
                // optimize for AMD x64 architecture only
                Result += " /favor:AMD64";

                // enable extra buffer security checks (should already be enabled by default)
                Result += " /GS";
			...
			else
			{
                ...
                // SSE options are not allowed when using CLR compilation or the 64 bit toolchain
                if (CompileEnvironment.CLRMode == CPPCLRMode.CLRDisabled &&
                    CompileEnvironment.TargetPlatform == CPPTargetPlatform.Win32)
                {
		...
		static string GetCLArguments_CPP( CPPEnvironment CompileEnvironment )
		{
			...
			if (CompileEnvironment.TargetPlatform == CPPTargetPlatform.Metro)
			{
		...
		static string GetLinkArguments(LinkEnvironment LinkEnvironment)
		{
			...
			bool bIsTarget64Bit = 
				...
				LinkEnvironment.TargetPlatform == CPPTargetPlatform.Metro || 
				LinkEnvironment.TargetPlatform == CPPTargetPlatform.Dingo;
			...
			if( LinkEnvironment.TargetPlatform == CPPTargetPlatform.Win32 || LinkEnvironment.TargetPlatform == CPPTargetPlatform.Win64 ||
				LinkEnvironment.TargetPlatform == CPPTargetPlatform.Metro || LinkEnvironment.TargetPlatform == CPPTargetPlatform.Dingo)
			{
				if (bIsTarget64Bit)
				{
				...
				// WinRT
				if (LinkEnvironment.TargetPlatform == CPPTargetPlatform.Metro)
				{
					// generate metadata
					Result += " /WINMD:ONLY";

					// location of metadata
					Result += string.Format(" /WINMDFILE:\"{0}\"", Path.ChangeExtension(LinkEnvironment.OutputFilePath, "winmd"));
				}

				// not sure, metro packaged app?
				if (LinkEnvironment.TargetPlatform == CPPTargetPlatform.Metro)
				{
					Result += " /APPCONTAINER";
				}

                if( LinkEnvironment.TargetPlatform == CPPTargetPlatform.Dingo )
                {
                    Result += " /DYNAMICBASE";
                }
				...
				if( !BuildConfiguration.bSupportEditAndContinue && !bIsTarget64Bit)
				{
		...
		public static CPPOutput CompileCPPFiles(CPPEnvironment CompileEnvironment, List<FileItem> SourceFiles)
		{
			...
			if( CompileEnvironment.CLRMode == CPPCLRMode.CLREnabled || CompileEnvironment.TargetPlatform == CPPTargetPlatform.Metro)
			{
		...
		{
			if (Environment.TargetPlatform == CPPTargetPlatform.Dingo)
			{
				Console.WriteLine("I don't think we want .rc files on Dingo!");
				return null;
			}

			CPPOutput Result = new CPPOutput();

			bool bIsTarget64Bit = Environment.TargetPlatform == CPPTargetPlatform.Win64 || Environment.TargetPlatform == CPPTargetPlatform.Metro;
...
//			BuildProjectAction.CommandArguments += " /nologo /verbosity:minimal";
		...
		{	
			...
			// is target 64-bit?
			bool bIsTarget64Bit = 
				Platform == CPPTargetPlatform.Win64 || 
				Platform == CPPTargetPlatform.Metro || 
				Platform == CPPTargetPlatform.Dingo;
			...
			if( ToolName.ToUpperInvariant() == "RC" )
			{
				...
				if (bIsTarget64Bit && bSupports64bitExecutables)
				{
			...
			else
			{
				...
				bool bIsRequestingLinkTool = ToolName.ToUpperInvariant() == "LIB" || ToolName.ToUpperInvariant() == "LINK";
				// 64 bit target on a 32 bit machine
				bool bNeedsCrossCompiler = bIsTarget64Bit && !bSupports64bitExecutables;
				// Compiling code for a 64 bit target when XGE is allowed (XGE does not support intercepting 64 bit tools)
				bool bIsXGE64BitTarget = !bIsRequestingLinkTool && bIsTarget64Bit && BuildConfiguration.bAllowXGE;
				// Both target and build machines are 64 bit
				bool bIs64Bit = bIsTarget64Bit && bSupports64bitExecutables;
				...
				if (bIsTarget64Bit && bSupports64bitExecutables)
				{
					...
					{
                            if (Platform == CPPTargetPlatform.Dingo)
                            {
                                VCToolPath = Utils.ResolveEnvironmentVariable("%DurangoXDK%xdk\\VC\\bin\\x86_amd64\\") + ToolName + ".exe";
                            }
                            else
                            {
					...
					else if (bIs64Bit || bUse64BitLinker)
					{
                        if (Platform == CPPTargetPlatform.Dingo)
                        {
                            VCToolPath = Utils.ResolveEnvironmentVariable("%DurangoXDK%xdk\\VC\\bin\\amd64\\") + ToolName + ".exe";
                        }
                        else
                        {
		...
		static string WindowsSDKDirOld = "";
		...
		static void InitializeEnvironmentVariables( CPPTargetPlatform Platform )
		{
			if( !bAreEnvironmentVariablesAlreadyInitialized )
			{
				...
				if (Platform == CPPTargetPlatform.Dingo)
                {
                    VCVarsBatchFile = Utils.ResolveEnvironmentVariable( "%DurangoXDK%xdk\\DurangoVars.cmd");
                }
                // 64 bit tool chain.
				else if (Platform == CPPTargetPlatform.Win64 || Platform == CPPTargetPlatform.Metro)
				{
				...
				{
                    VCVarsBatchFile = Path.Combine(BaseVSToolPath, "../../VC/bin/vcvars32.bat");
				}
				...
				// retrieve the SDK dir from the environment variable
				WindowsSDKDir = Environment.GetEnvironmentVariable("WindowsSdkDir");
				WindowsSDKDirOld = Environment.GetEnvironmentVariable("WindowsSdkDir_old");

				// Lib and bin folders have a x64 subfolder for 64 bit development.
				string ConfigSuffix = "";
				if (Platform == CPPTargetPlatform.Win64 || Platform == CPPTargetPlatform.Metro || Platform == CPPTargetPlatform.Dingo)
				{
					ConfigSuffix = "\\x64";
				}
				...
				if (Platform == CPPTargetPlatform.Metro)
				{
					Environment.SetEnvironmentVariable( "LIBPATH", Utils.ResolveEnvironmentVariable(WindowsSDKDir + "Windows Metadata" + ";%LIBPATH%") );
				}
