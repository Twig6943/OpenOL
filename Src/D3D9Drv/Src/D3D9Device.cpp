...
#endif
{
	...
	CachedColorWriteStates[0] = 0x0000000F;
	CachedColorWriteStates[1] = 0x0000000F;
	CachedColorWriteStates[2] = 0x0000000F;
	CachedColorWriteStates[3] = 0x0000000F;
	...
	if (SUCCEEDED(Direct3D->GetAdapterIdentifier(AdapterIndex, 0, &deviceId)))
	{
		debugf(TEXT("Using D3D9 device (%d): %s"), AdapterIndex, ANSI_TO_TCHAR(deviceId.Description));
	}	
	...
	if(Viewports.Num())
	{
		...
		if(bRecreateDevice)
		{
			...
			if(Direct3DDevice)
			{
				...
				// Recreate ImGui D3D9 objects after device reset
				OLImGui_CreateDeviceObjects();
			}
			else
			{
