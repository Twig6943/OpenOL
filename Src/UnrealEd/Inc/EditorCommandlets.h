...
BEGIN_COMMANDLET(GenerateStreamingInstallManifest, UnrealEd)
/** 
 * Utility function to gather a list of files matching a given search wildcard in a directory structure
 */
static void GatherFiles(TArray<FString>& OutFoundFiles, const FString& RootDir, const FString& Wildcard, UBOOL bRecursive = TRUE);

/**
 * Utility function to populate a list of all possible variants on a package name once cooked
 */
static void GetCookedPackageVariants(TArray<FString>& OutVariants, const FFilename& PackageBaseName, const UBOOL bIsScriptOrStartup);

/**
 * Utility function to build a mapping between .bnk and .wem output files
 */
static void BuildBankToWEMMap(const FString& RootDirectory, TMap<FFilename, TArray<FFilename> >& OutBankToWEMMap);

/**
 * Utility function to convert a filename into a unique chunk ID
 */
static UINT GetChunkIDForFile(const FFilename& InFilename);

/**
 * Utility function to create an appdata.bin from an AppxManifest.xml
 */
static void GenerateAppDataBinaryFromManifest(const FFilename& AppManifestPath);

private:
	// The following two functions are used to gather the full dependency list for the given persistent maps. All map-related content must be
	// in the Cooked subdirectory
	void CollectPMapDependencies(const FFilename& CookedRootDirectory, const TArray<FString>& PMapList);
	void GetAdditionalMapPackages(const FFilename& CookedRootDirectory, const FString& MapPackageName, TArray<FString>& OutAdditionalPackages);

	// Add all the given chunks as forced dependencies to each map
	void AddForcedDependencies(const TArray<UINT>& ForcedDependencies);

	// Removes all dependencies which were tagged as launch chunk files
	void RemoveLaunchChunkDependencies(const FString& CleanCookedDir, const TArray<FString>& LaunchChunkFiles);

	// Collects a list of all dependencies which are shared by multiple persistent maps
	void FindSharedDependencies(TArray<FString>& OutSharedDependencies);

	// Utility function to get all banks (including localized versions) with a given name
	void GetBanksAndWEMs(const FString& BankName, TArray<FString>& OutDependencies);

	struct FPMapDependencies
	{
		UINT				ChunkId;
		TArray<FString>		FileDependencies;
		TArray<UINT>		ChunkDependencies;
	};

	TCHAR								DingoEngineIni[1024];
	TCHAR								DingoSystemSettingsIni[1024];

	FString								DLCName;
	TMap<FFilename, TArray<FFilename> >	BankToWEMMap;
	UPersistentCookerData*				PersistentCookerData;
	TMap<FString, FPMapDependencies>	PMapDependencies;
	
	static const TCHAR* const			AlignmentChunkFilename;
...
BEGIN_COMMANDLET(CookPackages,UnrealEd)
	...
	*/
	...
	/** mathieu.gauthier: When recooking over already cooked data, will merge the new 
	    ref shader caches with the existing one, leaving any outdated shaders in the 
		exising map. */
	UBOOL							bMergeShaderCaches;

	/** mathieu.gauthier: Does not force a full recoord when outdated shaders are detected. */
	UBOOL							bIgnoreOutdatedShaders;

	/** mathieu.gauthier: A suffix to append to the TFC when generating patches. */
	FString							PatchTextureFileCacheSuffix;

	/** When non-empty, saves the cooked shader cache as RefShaderCache_<Suffix> instead of RefShaderCache,
	    so it can be merged at runtime into the original game's RefShaderCache without replacing it.
	    Set via -SHADEREXT=<Suffix> on the cooker command line. */
	FString							ExtShaderCacheSuffix;
...
// WWISEMODIF_START
	...
	/** Wwise bank definition file content */
	TMap<FString, TSet<FString> > gDefinitionFileContent;

	/** List of Wwise banks to be generated. */
	TArray<FString> gBankNamesToBeGenerated;
...
// WWISEMODIF_START
	void CookEvent( UPackage* Package, UAkEvent* Event, UBOOL bIsSavedInSeekFreePackage );

	void CookSoundBank( UPackage* Package, UAkBank* SoundBank, UBOOL bIsSavedInSeekFreePackage  );
// WWISEMODIF_END
...
// @igs(arm) - Adding commandlet to parse the header output by Microsoft's EtwPlusTool and generate an UnrealScript class
//             that inherits from the OnlineEventTracker and enables event signalling via script on Durango
BEGIN_COMMANDLET(GenerateOnlineEventTracker, UnrealEd)

	enum ESupportedParamDataTypes
	{
		ESPDT_SInt32,
		ESPDT_UInt32,
		ESPDT_SInt64,
	...
	struct RaiseEventFunctionDefinition
	{
		FString									EventName;
		TArray<ESupportedParamDataTypes>		ArgumentTypes;

		RaiseEventFunctionDefinition()
			: EventName(TEXT("")) {}

		FString GetFunctionName() const
		{
			return FString::Printf(TEXT("RaiseEvent%s"), *EventName);
		}

		FString GetFunctionDeclaration() const
		{
			return GetFunctionName() + FString(TEXT("(const TArray<FString>& Params)"));
		}
	};

	// Event parsing/generation
	void ParseRaiseEventFunctionDef(RaiseEventFunctionDefinition& FunctionDef, const FString& Declaration);

	void WriteSignedInt32Param(FString& ParamBuffer, const INT ParamNum);
	void WriteUnsignedInt32Param(FString& ParamBuffer, const INT ParamNum);
	void WriteSignedInt64Param(FString& ParamBuffer, const INT ParamNum);
	void WriteUnsignedInt64Param(FString& ParamBuffer, const INT ParamNum);
	void WriteFloatParam(FString& ParamBuffer, const INT ParamNum);
	void WriteDoubleParam(FString& ParamBuffer, const INT ParamNum);
	void WriteBoolParam(FString& ParamBuffer, const INT ParamNum);
	void WriteStringParam(FString& ParamBuffer, const INT ParamNum);
	void WriteGUIDParam(FString& ParamBuffer, const INT ParamNum);

	// Common
	void WriteAutoGeneratedHeader(FArchive& FileArchive);

	// Script generation
	void WriteScriptOutputMainBody(FArchive& FileArchive, const FFilename& ScriptFilename);

	void WriteScriptOutputCPPText(FArchive& FileArchive, const TArray<RaiseEventFunctionDefinition>& EventFunctionDefs);

	// Source generation
	void WriteSourceOutputHeader(FArchive& FileArchive, const FString& ProjectName, const FFilename& InputHeader, const FFilename& SourceFilename);

	void WriteSourceExecCatcher(FArchive& FileArchive, const FFilename& SourceFilename, const TArray<RaiseEventFunctionDefinition>& EventFunctionDefs);

	void WriteSourceOutputInitFunction(FArchive& FileArchive, const FFilename& SourceFilename, const FString& FunctionBody);
	
	void WriteSourceOutputShutdownFunction(FArchive& FileArchive, const FFilename& SourceFilename, const FString& FunctionBody);

	void WriteSourceOutputRaiseEventFunctions(FArchive& FileArchive, const FFilename& SourceFilename, const TArray<RaiseEventFunctionDefinition>& EventFunctionDefs);

END_COMMANDLET


// @igs(jc)
BEGIN_COMMANDLET(Base64Converter, UnrealEd)
	UBOOL IsDirectory(FFilename& Path);
	UBOOL ConvertFile(UBOOL Encode, FString& InputFilePath, FString& OutputFilePath);
END_COMMANDLET
