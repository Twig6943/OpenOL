...
void UGameEngine::Init()
{
	...
	if(ViewportClient)
	{
		...
		if (ParseParam(appCmdLine(),TEXT("WINDOWED")) || ParseParam(appCmdLine(), TEXT("simmobile")) || bForceWindowed)
		{
			GSystemSettings.bFullscreen = FALSE;
			debugf(TEXT("### (startup) Forcing windowed mode"));
		}
		...
		{
			GSystemSettings.ResX = CustomResX;
			GSystemSettings.ResY = CustomResY;

			debugf(TEXT("### (startup) Using custom resolution %d x %d (Fullscreen: %s)"), GSystemSettings.ResX, GSystemSettings.ResY, GSystemSettings.bFullscreen ? TEXT("yes") : TEXT("no"));
		}
...
#endif // _WINDLL
		{
			...
			AppName = FString::Printf( TEXT( "%s (%s-bit, %s)" ), *GameName, *PlatformBitsString, *RHIName );
#endif		
...
UBOOL UGameEngine::Exec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	else
	{
		...
		const TCHAR* ToggleSmoothCamCmd = Cmd;
		if (ParseCommand(&ToggleSmoothCamCmd, TEXT("TOGGLESMOOTHCAMERA")))
		{
			extern UBOOL GSmoothCamera;
			GSmoothCamera = !GSmoothCamera;
			Ar.Logf(TEXT("GSmoothCamera = %s"), GSmoothCamera ? TEXT("ON") : TEXT("OFF"));
			return TRUE;
		}
		const TCHAR* ToggleUnlockCmd = Cmd;
		if (ParseCommand(&ToggleUnlockCmd, TEXT("TOGGLEUNLOCKDOORS")))
		{
			extern UBOOL GUnlockDoors;
			GUnlockDoors = !GUnlockDoors;
			Ar.Logf(TEXT("GUnlockDoors = %s"), GUnlockDoors ? TEXT("ON") : TEXT("OFF"));
			return TRUE;
		}
		const TCHAR* ToggleGhostCmd = Cmd;
		if (ParseCommand(&ToggleGhostCmd, TEXT("TOGGLEALLOWGHOSTDOORS")))
		{
			extern UBOOL GAllowGhostDoors;
			GAllowGhostDoors = !GAllowGhostDoors;
			Ar.Logf(TEXT("GAllowGhostDoors = %s"), GAllowGhostDoors ? TEXT("ON") : TEXT("OFF"));
			return TRUE;
		}
		const TCHAR* MaxFpsCmd = Cmd;
		if (ParseCommand(&MaxFpsCmd, TEXT("MAXFPS")))
		{
			FLOAT FPS = appAtof(MaxFpsCmd);
			if (FPS > 0.0f)
			{
				UObject::StaticExec(*FString::Printf(TEXT("SET ENGINE MAXSMOOTHEDFRAMERATE %g"), FPS), Ar);
			}
			return TRUE;
		}
...
UBOOL UGameEngine::LoadMap( const FURL& URL, UPendingLevel* Pending, FString& Error )
{
	...
	// WWISEMODIF_START
	if( GEngine->Client && GEngine->Client->GetAkAudioDevice() )
	{
		GEngine->Client->GetAkAudioDevice()->Flush( NULL );
	}
// WWISEMODIF_END
...
// WWISEMODIF_START
		/*if (Client != NULL && Client->GetAkAudioDevice() != NULL)
		{
			GEngine->Client->GetAkAudioDevice()->StopAllSounds();
		}*/
// WWISEMODIF_END
...
//@zombie_ps4_begin - Adding support for gamepad lightbar subsystem
	if( GamePadLightbarSubsystem )
	{
		GamePadLightbarSubsystem->Tick( DeltaSeconds );
	}
//@zombie_ps4_end
	...
	if (bUpdateAllocSizeStats)
	{
		...
		FMemoryAllocationStats MemStats;
		GMalloc->GetAllocationInfo( MemStats );
		SET_MEMORY_STAT(STAT_VirtualAllocSize,MemStats.CPUUsed);
		SET_MEMORY_STAT(STAT_PhysicalAllocSize,MemStats.GPUUsed);
#endif

//@zombie_ps4_begin - Orbis memory stats
#if ORBIS
		OrbisRHIBlock::UpdateAllocationStats();
#endif
//@zombie_ps4_end
		
#if WITH_FACEFX
		SET_MEMORY_STAT(STAT_FaceFXCurrentAllocSize,OC3Ent::Face::FxGetCurrentBytesAllocated());
		SET_MEMORY_STAT(STAT_FaceFXPeakAllocSize,OC3Ent::Face::FxGetPeakBytesAllocated());
#endif
	}
...
// WWISEMODIF_START
	if( Client && Client->GetAkAudioDevice() )
	{
		Client->GetAkAudioDevice()->Update( !GWorld->IsPaused() || (GWorld->GetWorldInfo() && GWorld->GetWorldInfo()->bGameplayFramePause) );
	}
// WWISEMODIF_END
 ...
 * Script to C++ thunking code
 */
void UGameEngine::execDecodeURLString( FFrame& Stack, RESULT_DECL )
{
	P_GET_STR_REF(Str);
	P_FINISH;

	UGameEngine::DecodeURLString(Str);
}

/**
 * Script to C++ thunking code
 */
void UGameEngine::execEncodeURLString( FFrame& Stack, RESULT_DECL )
{
	P_GET_STR_REF(Str);
	P_FINISH;

	UGameEngine::EncodeURLString(Str);
}
...
/**
 * Closes a secondary viewport with the specified name
 */
