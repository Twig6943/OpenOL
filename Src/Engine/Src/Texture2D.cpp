...
inline void IncMemoryStats( const UTexture2D* Texture )
{
	if ( Texture->IsA( ULightMapTexture2D::StaticClass() ) && ((ULightMapTexture2D*)Texture)->IsSimpleLightmap() == FALSE )
	{
		...
		INC_MEMORY_STAT_BY( STAT_TextureLightmapMemory, TextureSize );
		GMaxTextureLightmapMemory = Max<QWORD>(GMaxTextureLightmapMemory, GStatManager.GetStatValueQWORD(STAT_TextureLightmapMemory));
	}
	else if ( Texture->IsA( UShadowMapTexture2D::StaticClass() ) )
	{
		INT TextureSize = Texture->CalcTextureMemorySize( Texture->Mips.Num() );
		INC_MEMORY_STAT_BY( STAT_TextureShadowmapMemory, TextureSize );
		GMaxTextureShadowmapMemory = Max<QWORD>(GMaxTextureShadowmapMemory, GStatManager.GetStatValueQWORD(STAT_TextureShadowmapMemory));
	}
}
...
inline void DecMemoryStats( const UTexture2D* Texture )
{
	...
	{
		INT TextureSize = Texture->CalcTextureMemorySize( Texture->Mips.Num() );
		DEC_MEMORY_STAT_BY( STAT_TextureLightmapMemory, TextureSize );
	}
	else if ( Texture->IsA( UShadowMapTexture2D::StaticClass() ) )
	{
		INT TextureSize = Texture->CalcTextureMemorySize( Texture->Mips.Num() );
		DEC_MEMORY_STAT_BY( STAT_TextureShadowmapMemory, TextureSize );
	}
}
...
inline void IncMemoryStats360( const UTexture2D* Texture )
{
	if ( Texture->IsA( ULightMapTexture2D::StaticClass() ) && ((ULightMapTexture2D*)Texture)->IsSimpleLightmap() == FALSE )
	{
		...
		INC_MEMORY_STAT_BY( STAT_XboxTextureLightmapMemory, TextureSize_360 );
		GMaxTextureLightmapMemoryXbox = Max<QWORD>(GMaxTextureLightmapMemoryXbox, GStatManager.GetStatValueQWORD(STAT_XboxTextureLightmapMemory));
	}
	else if ( Texture->IsA( UShadowMapTexture2D::StaticClass() ) )
	{
		INT TextureSize_360 = Texture->Get360Size( Texture->Mips.Num() );
		INC_MEMORY_STAT_BY( STAT_XboxTextureShadowmapMemory, TextureSize_360 );
		GMaxTextureShadowmapMemoryXbox = Max<QWORD>(GMaxTextureShadowmapMemoryXbox, GStatManager.GetStatValueQWORD(STAT_XboxTextureShadowmapMemory));
	}
}
...
inline void DecMemoryStats360( const UTexture2D* Texture )
{
	...
	{
		INT TextureSize_360 = Texture->Get360Size( Texture->Mips.Num() );
		DEC_MEMORY_STAT_BY( STAT_XboxTextureLightmapMemory, TextureSize_360 );
	}
	else if ( Texture->IsA( UShadowMapTexture2D::StaticClass() ) )
	{
		INT TextureSize_360 = Texture->Get360Size( Texture->Mips.Num() );
		DEC_MEMORY_STAT_BY( STAT_XboxTextureShadowmapMemory, TextureSize_360 );
	}
}
#endif
#endif
...
FTextureResource* UTexture2D::CreateResource()
{
	...
	// If NeverStream was set but mip data is actually stored in a separate TFC file, allow streaming.
	if( NeverStream && TextureFileCacheName != NAME_None )
	{
		for( INT MipIndex = 0; MipIndex < Mips.Num(); MipIndex++ )
		{
			if( Mips(MipIndex).Data.IsStoredInSeparateFile() )
			{
				NeverStream = FALSE;
				break;
			}
...
#else
				// If not found at the top level, search recursively (e.g. TFC in a subdirectory of CookedPCConsole)
				if( GFileManager->FileSize(*Filename) <= 0 )
				{
					TArray<FString> FoundFiles;
					appFindFilesInDirectory(FoundFiles, *CookedPath, FALSE, TRUE);
					for( INT i = 0; i < FoundFiles.Num(); i++ )
					{
						if( FFilename(FoundFiles(i)).GetCleanFilename() == TextureCacheString )
						{
...
void FTexture2DResource::InitRHI()
{
	...
	INC_MEMORY_STAT_BY( STAT_TextureMemory, TextureSize );
	INC_MEMORY_STAT_BY( Owner->LODGroup + STAT_TextureGroupFirst, TextureSize );
	STAT( check( IntermediateTextureSize == 0 ) );
	STATWIN( INC_MEMORY_STAT_BY( STAT_XboxTextureMemory, TextureSize_360 ) );
	...
	if (GIsEditor || (!bSkipRHITextureCreation))
	{
		...
		{
#if DINGO
			if (Owner->RequiredCreationFlags > 0)
			{
				// @igs(jds) - since the layout of textures can change between mip levels and number, we need to reserve enough memory to contain the whole texture and will limit
				// how much is actually used by the GPU so we don't read from uninitialized memory.
				Texture2DRHI	= RHICreateTexture2D(Owner->SizeX, Owner->SizeY, EffectiveFormat, Owner->Mips.Num(), TexCreateFlags | Owner->RequiredCreationFlags | TexCreate_ReserveMemory, ResourceMem);
				Texture2DRHI	= RHICommitTexture2D(Texture2DRHI, Owner->RequestedMips, 0);
			}
			else
#endif
			{
...
#if DINGO
		// indicate which mips are filled out and can be used by the GPU
		RHISetResourceMinLOD(Texture2DRHI);
#endif
	}
...
void FTexture2DResource::ReleaseRHI()
{
	...
	DEC_MEMORY_STAT_BY( STAT_TextureMemory, TextureSize );
	DEC_MEMORY_STAT_BY( Owner->LODGroup + STAT_TextureGroupFirst, TextureSize );
	STAT( check( IntermediateTextureSize == 0 ) );
	STATWIN( DEC_MEMORY_STAT_BY( STAT_XboxTextureMemory, TextureSize_360 ) );
	STATWIN( check( IntermediateTextureSize_360 == 0 ) );
...
//@zombie_ps4_begin
	...
	BYTE *Src = (BYTE*) MipData[MipIndex];
	BYTE *Dst = (BYTE*) Dest;
	appMemcpy(Dest, Src, MipMap.Data.GetBulkDataSize());
//@zombie_ps4_end
#else
...
void FTexture2DResource::UpdateMipCount()
{
	...
	// @igs(jds) - We don't want to create a new texture on Dingo since all the memory has already been reserved, so if the async failed, we already know there isn't room
	// and we cannot defragment to get available memory since the memory is handled at the system level for us using this method.
#if !DINGO
    // Did it fail?
    if ( IsValidRef(IntermediateTextureRHI) == FALSE )
    {
		...
		IntermediateTextureRHI = RHICreateTexture2D( SizeX, SizeY, EffectiveFormat, Owner->RequestedMips, TexCreateFlags | Owner->RequiredCreationFlags, NULL );
...
void FTexture2DResource::LoadMipData()
{
	...
	if ( IsValidRef(IntermediateTextureRHI) && !Owner->bHasCancelationPending )
	{
		...
		INC_MEMORY_STAT_BY( STAT_TextureMemory, IntermediateTextureSize );
		INC_MEMORY_STAT_BY( Owner->LODGroup + STAT_TextureGroupFirst, IntermediateTextureSize );
		STATWIN( INC_MEMORY_STAT_BY( STAT_XboxTextureMemory, IntermediateTextureSize_360 ) );
...
void FTexture2DResource::FinalizeMipCount()
{
	...
	if ( IsValidRef(IntermediateTextureRHI) )
	{
		...
		if( !Owner->bHasCancelationPending )
		{
			bSuccess		= TRUE;
#if DINGO
			// we have our info loaded, so let the GPU know it can use it
			RHISetResourceMinLOD(IntermediateTextureRHI);
#endif
		...
		{
			// need to adjust our memory so that we release what we allocated
			if (bUsingInPlaceRealloc)
			{
				INT CancelledFirstMip = Owner->Mips.Num() - Owner->ResidentMips;
				IntermediateTextureRHI = RHIReallocateTexture2D(IntermediateTextureRHI, Owner->ResidentMips, Owner->Mips(CancelledFirstMip).SizeX, Owner->Mips(CancelledFirstMip).SizeY);
				TextureRHI		= IntermediateTextureRHI;
				Texture2DRHI	= IntermediateTextureRHI;
			...
			DEC_MEMORY_STAT_BY( STAT_TextureMemory, IntermediateTextureSize );
			DEC_MEMORY_STAT_BY( Owner->LODGroup + STAT_TextureGroupFirst, IntermediateTextureSize );
			STATWIN( DEC_MEMORY_STAT_BY( STAT_XboxTextureMemory, IntermediateTextureSize_360 ) );
		}
...
UBOOL FTexture2DResource::TryReallocate( INT OldMipCount, INT NewMipCount )
{
	...
	if ( IsValidRef(NewTextureRHI) )
	{
		...
		DEC_MEMORY_STAT_BY( STAT_TextureMemory, OldSize );
		INC_MEMORY_STAT_BY( STAT_TextureMemory, NewSize );
		...
		STATWIN( DEC_MEMORY_STAT_BY( STAT_XboxTextureMemory, Old360Size ) );
		STATWIN( INC_MEMORY_STAT_BY( STAT_XboxTextureMemory, New360Size ) );
...
//@zombie_ps4_begin
#if ORBIS
	UINT DataSize = DataEntry.MipData(MipIndex).Data.Num();
	appMemcpy(Dest, DataEntry.MipData(MipIndex).Data.GetData(), DataSize);
#elif DINGO
