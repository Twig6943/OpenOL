...
#if DINGO
UBOOL GAllowForwardShadowing = TRUE;
UBOOL GStencilLights = TRUE;
#else
UBOOL GAllowForwardShadowing = FALSE;
UBOOL GStencilLights = FALSE;
#endif

/** 
 * Shadow currently being rendered by a TMeshLightingDrawingPolicy.  
 * This is global so that different shadows can be used with the same static draw list. 
 */
THREAD_LOCAL_STORAGE_MT FProjectedShadowInfo* GLightRenderingShadowInfo = NULL;
...
public:
	...
	{
//@zombie_ps4_begin
		return (Platform == SP_PCD3D_SM5 || Platform == SP_ORBIS || Platform == SP_DINGO) && ShouldCompileDeferredShaders(Platform);
//@zombie_ps4_end
	}
...
*/
class FLightStencilVertexShader : public FGlobalShader
{
	DECLARE_SHADER_TYPE(FLightStencilVertexShader,Global);
public:

	FLightStencilVertexShader() {}
	FLightStencilVertexShader(const ShaderMetaType::CompiledShaderInitializerType& Initializer) : FGlobalShader(Initializer) {}
	static UBOOL ShouldCache(EShaderPlatform Platform) { return FALSE; }
};

IMPLEMENT_SHADER_TYPE(,FLightStencilVertexShader,TEXT("ShadowProjectionVertexShader"),TEXT("LightStencilMain"),SF_Vertex,0,0);
...
#if DINGO
			&& (!Light->bIsHeroSpotLight || StaticMesh->PrimitiveSceneInfo->HeroLightCount == 0)
#endif
...
#if DINGO
			&& (!Light->bIsHeroSpotLight || PrimitiveSceneInfo->HeroLightCount == 0)
#endif
			// Don't render the light affecting this mesh if it will be handled in a deferred pass
...
UBOOL FSceneRenderer::RenderLights(UINT DPGIndex,UBOOL bAffectedByModulatedShadows, UBOOL bWasSceneColorDirty)
{
	...
	UBOOL bClearedLightAttenuation = FALSE;
	...
	for(TSparseArray<FLightSceneInfoCompact>::TConstIterator LightIt(Scene->Lights);LightIt;++LightIt)
	{
		...
		if(bLightIsVisibleInAnyView)
		{
			...
			UBOOL bHasLightFunction = CheckForLightFunction( LightSceneInfo, DPGIndex );;
			bUseAttenuationBuffer |= bHasLightFunction;

			// mathieu.gauthier: We will allow skipping of shadow projections for normal shadows
			// that dont use cached preshadows.
			const UBOOL bAllowSkipProjection = GAllowForwardShadowing && !bHasLightFunction && !RenderCachedPreshadows(LightSceneInfo, DPGIndex, TRUE, TRUE);
			UBOOL bShadowDirtiedLightAttenuation = FALSE;
			UBOOL bStenciledLightVolume = FALSE;

			// Stencil the volume of the light.
			const UBOOL bIsPointOrSpotLight = 
				LightSceneInfo->LightType == LightType_Point         || 
			...
			if (GStencilLights && bIsPointOrSpotLight && !LightSceneInfo->bIsHeroSpotLight && (VisibleLightInfo.AllProjectedShadows.Num() == 0 || VisibleLightInfo.AllProjectedShadows.Num() == 1 && VisibleLightInfo.AllProjectedShadows(0)->bFullSceneShadow))
			{
				TShaderMapRef<FLightStencilVertexShader> VertexShader(GetGlobalShaderMap());
				if (*VertexShader)
				{
					...
					// Z-fail approach. Runs at same speed as Z-pass and handles cases where
					// camera is inside the volume.
					RHISetStencilState(TStaticStencilState<
						TRUE,CF_Always,SO_Keep,SO_Invert,SO_Keep,
						TRUE,CF_Always,SO_Keep,SO_Invert,SO_Keep,
						0xff,0xff
					>::GetRHI());

					static FGlobalBoundShaderState StencilMaskBoundShaderState;
					SetGlobalBoundShaderState(StencilMaskBoundShaderState,GShadowFrustumVertexDeclaration.VertexDeclarationRHI,*VertexShader,NULL,sizeof(FVector));
					DrawStencilingVolume(LightSceneInfo);

					RHISetColorWriteEnable(TRUE);

					bStenciledLightVolume = TRUE;
				}
			}
			...
			if (bUseAttenuationBuffer)
			{
				...
				{
#if DINGO
					// We dont really care about clearing the light attenuation since
					// light functions and shadows are opaque and only cover the region
					// of the light. Also we want to to avoid a lot of "Eliminate Fast Clear"
					// on dingo. But still, it is really safe to clear once otherwise the 
					// render target might be in a weird state.
					if (!bClearedLightAttenuation || !bAllowSkipProjection)
#endif
					{
						RHIClear(TRUE, FLinearColor::White, FALSE, 0, FALSE, 0);
					}
					bClearedLightAttenuation = TRUE;
				}
				
				if (bDrawShadows && bDrawNormalShadows)
				{
					// Render non-modulated projected shadows to the attenuation buffer.
					bShadowDirtiedLightAttenuation = RenderProjectedShadows( LightSceneInfo, DPGIndex, TRUE, bAllowSkipProjection, bStenciledLightVolume );
				}
			}
	
			if (bUseAttenuationBuffer)
			{
				if (bStenciledLightVolume)
				{
					RHISetStencilState(TStaticStencilState<
			...
			GSceneRenderTargets.SetLightAttenuationMode(bAllowSkipProjection ? bShadowDirtiedLightAttenuation : bUseAttenuationBuffer);
			...
			if (bAllowSkipProjection && !bShadowDirtiedLightAttenuation && LightSceneInfo->LightShadowMode == (BYTE)LightShadow_Normal)
			{
				// We will do forward shadowing if no light functions are being used.
				if (VisibleLightInfo.AllProjectedShadows.Num() == 1 && VisibleLightInfo.AllProjectedShadows(0)->bFullSceneShadow && !GSceneRenderTargets.bLightAttenuationEnabled)
				{
					GLightRenderingShadowInfo = VisibleLightInfo.AllProjectedShadows(0);
				}
			}
			else
			{
				GLightRenderingShadowInfo = NULL;
			}
...
UBOOL FSceneRenderer::RenderLight( const FLightSceneInfo* LightSceneInfo, UINT DPGIndex )
{
	...
	for(INT ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
	{
		...
		if( ShouldRenderLight(VisibleLightViewInfo, DPGIndex) )
		{
			...
			UBOOL bLightPossiblyAffectsAnything = 
				LightSceneInfo->GetDPGInfo(DPGIndex)->HasAnythingToDraw() ||
				VisibleLightViewInfo.DPGInfo[DPGIndex].VisibleDynamicLitPrimitives.Num() > 0 || 
				VisibleLightViewInfo.DPGInfo[DPGIndex].VisibleLitDecalPrimitives.Num() > 0;

			START_DEFERRED_CONTEXT_THREAD_TWO_PARAMETERS(Light, FSceneRenderer, const FLightSceneInfo*, LightSceneInfo, const FVisibleLightViewInfo&, VisibleLightViewInfo, 
				bLightPossiblyAffectsAnything, DPGIndex == 1 && !bIsSceneCapture,
			{
				SCOPED_DRAW_EVENT(LightThread)(DEC_SCENE_ITEMS,TEXT("LightThread"));
				...
				// Draw the static batched decals affected by the light
				{
					...
					// scissor rect
					if( bAnyDecalsRendered && !This->bIsSceneCapture )
					{
						LightSceneInfo->SetScissorRect( &View );
					}
...

				{
					...
					// Render decals for visible primitives affected by this light.
					{
						...
						// decal drawing code (FMeshDrawingPolicy::SetMeshRenderState), so we'll reset the light's
						// scissor rect
						if( Drawer.IsDirty() && !This->bIsSceneCapture )
						{
							LightSceneInfo->SetScissorRect( &View );
						}
...
UBOOL FSceneRenderer::PrepareMobileModulatedShadows(UINT DPGIndex)
{
	...
	if( MobileProjectedShadows.Num() )
	{
		...
		GSceneRenderTargets.FinishRenderingShadowDepth(FALSE, FALSE, FResolveRect());
	}
