...
UBOOL FDOFAndBloomBlendPixelShader::Serialize(FArchive& Ar)
{
	...
	Ar << DOFParameters << SceneTextureParameters << PostProcessParameters;
...
void FDOFAndBloomBlendPixelShader::RebindParameters(const FShaderParameterMap& ParameterMap)
{
	DOFParameters.Bind(ParameterMap);
	SceneTextureParameters.Bind(ParameterMap);
	PostProcessParameters = FPostProcessParameters(ParameterMap);
	BloomTintAndScreenBlendThresholdParameter.Bind(ParameterMap, TEXT("BloomTintAndScreenBlendThreshold"), TRUE);
}
...
//@zombie_ps4_begin
	...
	if (!*BlendVertexShader || !*BlendPixelShader) { warnf(TEXT("DOFAndBloom: NULL shader")); return FALSE; }
