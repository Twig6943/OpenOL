...
// WWISEMODIF_START
	...
	#include "../../AkAudio/Inc/AkAudioPrivate.h"
	#include "../../AkAudio/Inc/AkAudioClasses.h"
// WWISEMODIF_END
...
// 	debugf(TEXT("(ComposeSkeleton) [%3.3f] SkelMesh: %s"), GWorld->GetWorldInfo()->TimeSeconds, *SkeletalMesh->GetName());
	...
	for(INT i=0; i<ComposeOrderedRequiredBones.Num(); i++)
	{
		...
		if( BoneIndex != 0 )
		{
			...
			if (GWorld->HasBegunPlay() && ParentIndex == 0)
			{
				// rcharpentier - Push root transform to its children (fixes blending issues)
				FBoneAtom RootXform = LocalAtoms(0);
				FBoneAtom::Multiply(SpaceBasesData + BoneIndex, LocalTransformsData + BoneIndex, &RootXform);
			}
			else
			{
		...
		{
			// No local space transform on the root bone
			if (GWorld->HasBegunPlay())
			{
				SpaceBases(0).SetIdentity();
			}
			else
			{
				SpaceBases(0) = LocalAtoms(0);
...
// WWISEMODIF_START, alessard, nov-11-2008, WwiseAudioIntegration
					else if( CachedFaceFxAkEvent )
					{
						// Wwise doesn't allow controlling frequency on playback directly.
						// Using Wwise, the way to do it would be driving an RTPC for the 
						// playback speed on the associated AudioComponent.
					}
// WWISEMODIF_END
...
// WWISEMODIF_START, alessard, nov-11-2008, WwiseAudioIntegration
						else if( CachedFaceFxAkEvent )
						{
							...
							if ( AudioDevice )
								AudioDevice->PostEvent( CachedFaceFxAkEvent, Owner, TEXT( "headBase" ) );
						}
// WWISEMODIF_END
					}
...
UBOOL USkeletalMeshComponent::PlayFaceFXAnim(UFaceFXAnimSet* FaceFXAnimSetRef, const FString& AnimName, const FString& GroupName,class USoundCue* SoundCueToPlay, class UAkEvent* AkEventToPlay, FaceFXPriority Priority)
// WWISEMODIF_END
{
#if WITH_FACEFX
	...
	if ((Priority < CurrentFaceFXPriorityLevel) && IsPlayingFaceFXAnim())
	{
		debugf(TEXT("Skipping FaceFX %s on %s - lower priority"), *AnimName, Owner ? *Owner->GetName() : TEXT("[none]"));
		return FALSE;
	}
...
// 		}
		...
		// rcharpentier - return if the anim doesn't exist (we have a lot of negatives due to subtitles not making distinction between new face fx anim or not)
		FxActor* faceFXActor = FaceFXActorInstance->GetActor();
		if (faceFXActor)
		{
			if (faceFXActor->GetAnimPtr(TCHAR_TO_ANSI(*GroupName), TCHAR_TO_ANSI(*AnimName)) == NULL)
			{
				return FALSE;
			}
		}

		// rcharpentier - passing FxTrue to start the anim right away (fix issue with lagging anims)
		UBOOL bPlaySuccess = FaceFXActorInstance->PlayAnim(TCHAR_TO_ANSI(*AnimName), TCHAR_TO_ANSI(*GroupName), FxTrue);
...
// WWISEMODIF_START, alessard, nov-11-2008, WwiseAudioIntegration
				...
				if( Sound && CachedFaceFXAudioComp )
				{
					CachedFaceFXAudioComp->Stop();
					CachedFaceFXAudioComp->SoundCue = Sound;
				}
				else if( AkEventToPlay )
				{
					CachedFaceFxAkEvent = AkEventToPlay;
					CachedFaceFXAudioComp = NULL;
				}
				else
				{
					CachedFaceFxAkEvent = NULL; // Outlast - plalonde: reinterpret_cast<UAkEvent*>(Anim->GetAkEventPointer());
					CachedFaceFXAudioComp = NULL;
				}
			}
// WWISEMODIF_END

			CurrentFaceFXPriorityLevel = Priority;
...
// WWISEMODIF_START, alessard, nov-11-2008, WwiseAudioIntegration
		else if( CachedFaceFxAkEvent )
		{
			...
			if ( AudioDevice )
			{
				UAkComponent * pAkComponent = AudioDevice->GetAkComponent( Owner, TEXT( "headBase" ) );
				if( pAkComponent )
				{
					pAkComponent->Stop();
				}
			}
			CachedFaceFxAkEvent = NULL;
		}
// WWISEMODIF_END
		...
		CurrentFaceFXPriorityLevel = FFXP_None;
	}
#endif // WITH_FACEFX
}
...
// WWISEMODIF_START, alessard, nov-28-2008, WwiseAudioIntegration
	P_GET_OBJECT_REF(UAkEvent, AkEventToPlay);
	P_GET_BYTE(Priority);
	P_FINISH;

	*(UBOOL*)Result = PlayFaceFXAnim(FaceFXAnimSetRef, AnimName, GroupName, SoundCueToPlay, AkEventToPlay, (FaceFXPriority)Priority);
// WWISEMODIF_END
}
