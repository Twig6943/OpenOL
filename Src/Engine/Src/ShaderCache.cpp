...
extern const void* GRebindOwner;
    ...
    for (TMap<FShaderType*, FTypeSpecificCompressedShaderCode>::TIterator It(ShaderTypeCompressedShaderCode); It; ++It)
    {
        ...
        if (Type)
        {
            ...
            for (INT ChunkIndex = 0; ChunkIndex < CurrentTypeCompressedCode.CodeChunks.Num(); ChunkIndex++)
            {
                ...
                DEC_MEMORY_STAT_BY_FAST(STAT_ShaderCompression_CompressedShaderMemory, CurrentChunk.CompressedCode.Num());
            }
        }
    }
}
...
DWORD FCompressedShaderCodeCache::AddRef()
{
    ++NumRefs;
    return NumRefs;
}

DWORD FCompressedShaderCodeCache::Release()
{
    if(--NumRefs == 0)
    {
        ...
        for (TMap<FShaderType*, FTypeSpecificCompressedShaderCode>::TIterator It(Ref.ShaderTypeCompressedShaderCode); It; ++It)
        {
            ...
            if (Type)
            {
                ...
                for (INT ChunkIndex = 0; ChunkIndex < CurrentTypeCompressedCode.CodeChunks.Num(); ChunkIndex++)
                {
                    ...
                    INC_MEMORY_STAT_BY_FAST(STAT_ShaderCompression_CompressedShaderMemory, CurrentChunk.GetCompressedCodeSize());
                }
            }
        }
    }
...
void SerializeShaders(const TMap<FGuid,FShader*>& InShaders, FArchive& Ar, UBOOL bSkipVersionCheck);

/** Loads the shader cache. */
void FShaderCache::Load(FArchive& Ar, UBOOL bSkipVersionCheck)
{
    ...
    SerializeShaders(TempShaders, Ar, bSkipVersionCheck);
}
...
void UShaderCache::Load(FArchive& Ar, UBOOL bIsAlwaysLoaded)
{
    ...
    // For the always-loaded RefShaderCache, skip the package version check so that shaders
    // compiled with an older engine version (e.g. original 868 RefShaderCache) are accepted.
    FShaderCache::Load(Ar, bIsAlwaysLoaded);
...
#define MERGE_COMPRESSED_SHADERS (1)
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    if (MaterialShaderIndex->IsCompressedShaderCacheComplete(CompressedCache))
                    {
                    ...
                    {
                        // Compressed data missing — old/incompatible RefShaderCache; discard so the material falls back to default.
                        warnf(NAME_DevShaders, TEXT("Shader map %s is incomplete in CompressedCache and was discarded (incompatible RefShaderCache format)."),
                            *MaterialShaderIndex->GetFriendlyName());
                        DeferredDeleteMaterialShaderMaps.AddItem(MaterialShaderIndex);
                    }
                }
                ...
                {
                    debugf(NAME_DevShaders, TEXT("Shader map %s had an invalid uniform expression set and was discarded."),
                        *MaterialShaderIndex->GetFriendlyName());
                }
            }
...
static void LoadShaderCaches( EShaderPlatform Platform )
{
    ...
    // Scan for and merge any additional per-package reference shader caches (RefShaderCache_*.upk).
    // This allows each new content package to ship its own shader cache alongside the original game's
    // RefShaderCache without replacing it — just drop RefShaderCache_MyMap.upk next to the .upk.
    if( GShaderCaches[SC_Reference][Platform] )
    {
        // Build the search pattern by deriving the directory from the standard RefShaderCache path.
        FFilename BaseRefPath = GetReferenceShaderCacheFilename(Platform);
        FString CacheDir = FFilename(BaseRefPath).GetPath() + PATH_SEPARATOR;
        FString PlatformSuffix = FString(TEXT("-")) + ShaderPlatformToText(Platform) + TEXT(".upk");
        FString SearchPattern = CacheDir + TEXT("RefShaderCache_*") + PlatformSuffix;

        TArray<FString> FoundFiles;
        GFileManager->FindFiles(FoundFiles, *SearchPattern, TRUE, FALSE);

        for( INT FileIndex = 0; FileIndex < FoundFiles.Num(); FileIndex++ )
        {
            FString ExtFilename = CacheDir + FoundFiles(FileIndex);
            FFilename ExtPackageName = FFilename(ExtFilename).GetBaseFilename();

            UShaderCache* ExtCache = NULL;
            if( GUseSeekFreeLoading )
            {
                GUseSeekFreeLoading = FALSE;
                ExtCache = LoadObject<UShaderCache>(NULL, *(ExtPackageName + TEXT(".CacheObject")), NULL, LOAD_NoWarn, NULL);
                GUseSeekFreeLoading = TRUE;
            }
            else
            {
                UPackage* ExtPackage = UObject::LoadPackage(NULL, *ExtFilename, LOAD_NoWarn | LOAD_FindIfFail);
                if( ExtPackage )
                {
                    ExtCache = FindObject<UShaderCache>(ExtPackage, TEXT("CacheObject"));
                }
            }
            if( ExtCache )
            {
                ULinkerLoad* ExtLinker = ExtCache->GetLinker();
                if( ExtLinker )
                {
                    ExtLinker->Preload(ExtCache);
                }
                GShaderCaches[SC_Reference][Platform]->Merge(ExtCache);
                debugf(NAME_DevShaders, TEXT("Merged additional shader cache: %s"), *ExtFilename);
            }
        }
    }
#endif
...
public:
    ...
    FShaderLoadArchive(FArchive& Archive, const TArray<WORD>& InPastSerializations, UBOOL bInEnableAutomaticVersioning, const TCHAR* InDebugShaderName = NULL, UBOOL bInIsOldRefCache = FALSE) :
        FArchiveProxy(Archive),
        NextSerialization(0),
        bMismatch(FALSE),
        FirstMismatchIndex(INDEX_NONE),
        FirstMismatchExpected(0),
        FirstMismatchGot(0),
        PastSerializations(InPastSerializations),
        bEnableAutomaticVersioning(bInEnableAutomaticVersioning),
        DebugShaderName(InDebugShaderName),
        bIsOldRefCache(bInIsOldRefCache && DetectLegacy868(InPastSerializations))
    {
        ArIsSaveGame = bIsOldRefCache;
    }

    // Detect a 868-engine shader by scanning PastSerializations for three consecutive
    // 2B entries where the 882 reader emits three consecutive 4B calls (TArray counts).
    // In 868, TArray::operator<< serialized count as WORD (2B); in 882 as INT (4B).
    // We scan for any run of three consecutive 2s followed by a 4 (or vice-versa pattern)
    // that would indicate mismatched count sizes. Simpler: look for three consecutive 2-byte
    // entries immediately preceded or followed by 4-byte entries at offset > 10 (past header).
    // From log data: 868 pixel shaders always have Serializations.Num() >= 300,
    // while all 882 equivalents have Num <= 230. Use this as the detection criterion.
    static UBOOL DetectLegacy868(const TArray<WORD>& S)
    {
        return S.Num() >= 300;
    }

    UBOOL IsOldRefCache() const { return bIsOldRefCache; }
    ...
    virtual void Serialize( void* V, INT Length )
    {
        ...
        if (NextSerialization >= PastSerializations.Num() || PastSerializations(NextSerialization) != Length)
        {
            if (!bMismatch)
            {
                FirstMismatchIndex = NextSerialization;
                FirstMismatchExpected = (NextSerialization < PastSerializations.Num()) ? PastSerializations(NextSerialization) : -1;
                FirstMismatchGot = Length;
            }
            // For 868 RefShaderCache, don't set bMismatch — use adaptive field-size reading instead.
            if (!bIsOldRefCache)
            {
                bMismatch = TRUE;
            }
        ...
        else if (bIsOldRefCache && IsLoading())
        {
            if (NextSerialization >= PastSerializations.Num())
            {
                // 882 code reads more fields than 868 stored — zero out and don't advance archive.
                appMemzero(V, Length);
            }
            else if (PastSerializations(NextSerialization) != Length)
            {
                // 868 field has a different size than 882 expects. Read the archived bytes
                // and coerce into V: copy as many bytes as fit, zero any remainder.
                const INT ArchivedSize = (INT)PastSerializations(NextSerialization);
                appMemzero(V, Length);
                if (ArchivedSize <= 8)
                {
                    BYTE Buf[8] = {0};
                    FArchiveProxy::Serialize(Buf, ArchivedSize);
                    appMemcpy(V, Buf, Min(ArchivedSize, Length));
                }
                else
                {
                    // Large field (bytecode blob) — seek past it.
                    FArchiveProxy::Seek(Tell() + ArchivedSize);
                }
            }
            else
            {
    ...
    INT GetFirstMismatchIndex() const { return FirstMismatchIndex; }
    INT GetFirstMismatchExpected() const { return FirstMismatchExpected; }
    INT GetFirstMismatchGot() const { return FirstMismatchGot; }
    INT GetNextSerialization() const { return NextSerialization; }
    INT GetPastSerializationsNum() const { return PastSerializations.Num(); }

private:
    INT NextSerialization;
    UBOOL bMismatch;
    INT FirstMismatchIndex;
    INT FirstMismatchExpected;
    INT FirstMismatchGot;
    ...
    // If non-NULL, traces first 20 Serialize calls for debugging
    const TCHAR* DebugShaderName;
    // TRUE if this shader was written by the 868 engine (legacy RefShaderCache)
    UBOOL bIsOldRefCache;
};
...
void SerializeShaders(const TMap<FGuid,FShader*>& InShaders, FArchive& Ar, UBOOL bSkipVersionCheck)
{
    ...
    if (Ar.IsLoading())
    {
        ...
        TArray<FString> LoggedMismatchTypes;
        ...
        for (INT ShaderIndex = 0; ShaderIndex < NumShaders; ShaderIndex++)
        {
            ...
            else
            {
                ...
                else if (!bSkipVersionCheck && ((Ar.Ver() < ShaderType->GetMinPackageVersion()) || (Ar.LicenseeVer() < ShaderType->GetMinLicenseePackageVersion())))
                {
                ...
                else
                {
                    ...
                    FShaderLoadArchive LoadArchive(Ar, Serializations, bSerializeAutomaticVersioningData, NULL, bSkipVersionCheck);

                    // Deserialize the shader into the new instance.
                    const UBOOL bShaderHasOutdatedParameters = Shader->Serialize(LoadArchive);

                    if (bSkipVersionCheck && LoadArchive.HadSerializationMismatch())
                    {
                    }
                    if (!bSkipVersionCheck && (LoadArchive.HadSerializationMismatch() || (bShaderHasOutdatedParameters && ShouldReloadChangedShaders())))
                    {
                    ...
                    else
                    {
                        if (Ar.Tell() != SkipOffset)
                        {
                            ...
                            {
                                // Old RefShaderCache may serialize a different amount — seek past to stay in sync.
                                Ar.Seek(SkipOffset);
                            }
                            else
                            {
                        ...
                        if (bSkipVersionCheck)
                        {
                            if (Shader->GetCode().Num() > 0)
                            {
                                FShaderParameterMap RebindMap;
                                RHIBuildShaderParameterMap(Shader->GetCode(), RebindMap);
                                GRebindOwner = Shader;
                                Shader->RebindParameters(RebindMap);
                                GRebindOwner = NULL;
                            }
                            else
                            {
                                Shader->bNeedsParameterRebind = TRUE;
                            }
                        }
                    }
                }
