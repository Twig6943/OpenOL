...
enum FSceneRenderTargetUsage
{
	...
	RTUsage_SwapToESRAM		= 0x0010,		// (Dingo) If the rendertarget swapped so it is no longer in ESRAM and its resolve is, then swap it back to ESRAM
	RTUsage_DWORD			= 0x7fffffff
};

enum
{
	ShadowMapCacheSize2D   = 6,
	ShadowMapCacheSizeCube = 4
};
...
enum ESceneRenderTargetTypes
{
	...
	FilterColor3,
	// RGB Render target for post process filter colors.
	RGBFilterColor1,
	// Second RGB Render target for post process filter colors.
	RGBFilterColor2,
	// Third RGB Render target for post process filter colors.
	RGBFilterColor3,
	...
	CubeShadowDepthZ3,
	CubeShadowDepthZ4,
	// mathieu.gauthier: Render target for per-object shadow depths when using point light (cube).
	// This is only until we support one-pass point lights.
	CubeFaceShadowDepthZ,
...
static const INT NumShadowDepthSurfaces = 5;
...
enum FSceneRenderTargetIndex
{
	...
	SRTI_FilterColor1,
	SRTI_FilterColor2,
	SRTI_RGBFilterColor0,
	SRTI_RGBFilterColor1,
	SRTI_RGBFilterColor2	
};

/** mathieu.gauthier: shadow map caching. */
struct FShadowMapCacheEntry2D : public FRefCountedObject
{
	INT Resolution;
	FTexture2DRHIRef Texture;
};

struct FShadowMapCacheEntryCube : public FRefCountedObject
{
	INT Resolution;
	FTexture2DRHIRef   Texture[6];
...
public:
	...
	void BeginRenderingShadowDepth(UBOOL bIsWholeSceneDominantShadow, UBOOL bIsCubeMapFace = FALSE, INT ShadowResolution = -1);
	 ...
	 * @param ResolveParams - optional resolve params
	 */
	void FinishRenderingShadowDepth(UBOOL bIsWholeSceneDominantShadow, UBOOL bIsCubeMapFace = FALSE, const FResolveRect& ResolveRect = FResolveRect());
	...
	const FTexture2DRHIRef& GetShadowDepthZTexture(UBOOL bIsWholeSceneDominantShadow, UBOOL bCachedPreshadow, UBOOL bIsCubeMapFace = FALSE, INT ShadowResolution = -1) const 
	{ 
		if (bCachedPreshadow)
		{
		...
		{
#if !ORBIS
			check(bIsCubeMapFace == FALSE);
#endif
			return bIsWholeSceneDominantShadow ? RenderTargets[DominantShadowDepthZ].Texture : (bIsCubeMapFace ? RenderTargets[CubeFaceShadowDepthZ].Texture : RenderTargets[ShadowDepthZ0 + GetShadowDepthZIndex(ShadowResolution)].Texture); 
		}
	}
	...
	*/
	...
	const FSurfaceRHIRef& GetShadowDepthZSurface(UBOOL bIsWholeSceneDominantShadow, UBOOL bIsCubeMapFace = FALSE, INT ShadowResolution = -1) const						
	{ 
#if !ORBIS
		check(bIsCubeMapFace == FALSE);
#endif
		return bIsWholeSceneDominantShadow ? RenderTargets[DominantShadowDepthZ].Surface : (bIsCubeMapFace ? RenderTargets[CubeFaceShadowDepthZ].Surface : RenderTargets[ShadowDepthZ0 + GetShadowDepthZIndex(ShadowResolution)].Surface); 
	}
	...
	FIntPoint GetShadowDepthTextureResolution(UBOOL bWholeSceneDominantShadow = FALSE, UBOOL bIsCubeMapFace = FALSE) const;
	...
	INT GetShadowDepthZIndex(INT ShadowResolution) const;
	...
	INT GetShadowDepthZResolution(INT ShadowIndex) const;
	...
	/** mathieu.gauthier: shadow map caching. */
	TRefCountPtr<FShadowMapCacheEntry2D>   AllocateShadowMapCache2D(INT DesiredResolution);
	TRefCountPtr<FShadowMapCacheEntryCube> AllocateShadowMapCacheCube(INT DesiredResolution);
	void ResolveShadowMap2DToCache(FShadowMapCacheEntry2D* Entry);
	void ResolveShadowMapCubeToCache(FShadowMapCacheEntryCube* Entry, INT SplitIndex);
	void RestoreShadowMap2DFromCache(FShadowMapCacheEntry2D* Entry);
	void RestoreShadowMapCubeFromCache(FShadowMapCacheEntryCube* Entry, INT SplitIndex);
#if !FINAL_RELEASE
	void UpdateCachedShadowMapsStats();
#endif
...
public:
	...
	THREAD_LOCAL_STORAGE_MT static UBOOL bLightAttenuationEnabled;
	/** Indicates whether the whole scene dominant shadow projection in the light attenuation alpha is valid. */
	THREAD_LOCAL_STORAGE_MT static UBOOL bWholeSceneDominantShadowValid;
	/** mathieu.gauthier: Indicates if the current thread is rendering velocities. 
	    This is to prevent other thread thinking we are rendering velocities when
		the R-T is rendering them. Would creates crashes on AppendData(). */
	THREAD_LOCAL_STORAGE_MT static UBOOL bRenderingVelocities;
private:
	...
	TRefCountPtr<FShadowMapCacheEntry2D>   ShadowMapCache2D[ShadowMapCacheSize2D * NumShadowDepthSurfaces];
	TRefCountPtr<FShadowMapCacheEntryCube> ShadowMapCacheCube[ShadowMapCacheSizeCube * NumCubeShadowDepthSurfaces];
}; 
...
private:
	...
	FShaderResourceParameter DiffuseGBufferTextureUINT;
	FShaderParameter ScreenToWorldParameter;
};
