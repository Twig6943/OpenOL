...
#endif
#if ORBIS
				extern UBOOL GOrbisRHIInitialized;
				extern DOUBLE GOrbisLastPresentTime;

				if ( GOrbisRHIInitialized && (appSeconds() - GOrbisLastPresentTime) > 3.0f && !RHIIsDrawingViewport())
				{
					debugf( TEXT("Render thread idle for 3.0 seconds. Rendering a black screen to respect submitDone() TRC.") );
					RHIBeginDrawingViewport(NULL);
					RHIClear(TRUE,FLinearColor::Black,FALSE,0.0f,FALSE,0);
					RHIEndDrawingViewport(NULL, TRUE, TRUE);
				}
#endif
			}
