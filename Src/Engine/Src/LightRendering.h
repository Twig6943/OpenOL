 ...
 */
extern THREAD_LOCAL_STORAGE_MT FProjectedShadowInfo* GLightRenderingShadowInfo;
...
public:
		...
		)
	{
		return Super::ShouldCache(Platform, Material, VertexFactoryType, bLightRequiresStaticLightingShaders)
			// Xbox, PS3, WiiU, Dingo has GOnePassDominantLight=TRUE
//@zombie_ps4_begin
			&& (Platform != SP_XBOXD3D && Platform != SP_PS3 && Platform != SP_WIIU && Platform != SP_ORBIS && Platform != SP_PCD3D_SM4 && Platform != SP_PCD3D_SM5 && Platform != SP_DINGO);
//@zombie_ps4_end
	}
...
protected:
	...
	{
		LightTypePolicy::VertexParametersType::Bind(ParameterMap);
		ShadowingTypePolicy::VertexParametersType::Bind(ParameterMap);
...
public:
	...
	virtual void RebindParameters(const FShaderParameterMap& ParameterMap)
	{
		LightTypePolicy::PixelParametersType::Bind(ParameterMap);
		ShadowingTypePolicy::PixelParametersType::Bind(ParameterMap);
		MaterialParameters.RebindFromMap(ParameterMap);
		...
		FMeshMaterialPixelShader(Initializer)
	{
		...
		LightAttenuationSurface.Bind(Initializer.ParameterMap,TEXT("LightAttenuationSurface"),TRUE);
		LightAttenuationMSAAParameter.Bind(Initializer.ParameterMap,TEXT("bLightAttenuationMSAA"),TRUE);
		...
		UBOOL bUseTranslucencyLightAttenuation)
	{
		...
		if (GRHIShaderPlatform == SP_PCD3D_SM5)
		{
			...
			SetPixelShaderBool(
				GetPixelShader(), 
				LightAttenuationMSAAParameter, 
				GSystemSettings.UsesMSAA());
		}
	}

	void SetShadowParameters(const FSceneView& View, const FProjectedShadowInfo* ProjectedShadowInfo)
	{
		ForwardShadowingParameters.Set(View, this, false, GLightRenderingShadowInfo, FALSE);
	}
	...
	virtual UBOOL Serialize(FArchive& Ar)
	{
		...
		Ar << LightAttenuationSurface;
		Ar << LightAttenuationMSAAParameter;
...
private:
	...
	FShaderParameter LightAttenuationMSAAParameter;
...
//@zombie_ps4_end
...
#endif
		{
			...
			PixelShader->SetShadowParameters(*View,GLightRenderingShadowInfo);
		}
	...
	) 
{
	...
	{
		RHISetMRTColorWriteEnable(MaterialResource.GetLightingModel() == MLM_Skin && !View.bHasOutlastNightVision, 1);
	}
...
public:
	...
	virtual UBOOL HasAnythingToDraw() const
	{
		return !NoStaticShadowingDrawList[ELightPass_Default].IsEmpty() ||
		       !NoStaticShadowingDrawList[ELightPass_Decals].IsEmpty() ||
		       !ShadowTextureDrawList[ELightPass_Default].IsEmpty() ||
		       !ShadowTextureDrawList[ELightPass_Decals].IsEmpty() ||
		       !SignedDistanceFieldShadowTextureDrawList[ELightPass_Default].IsEmpty() ||
		       !SignedDistanceFieldShadowTextureDrawList[ELightPass_Decals].IsEmpty() ||
		       !ShadowVertexBufferDrawList[ELightPass_Default].IsEmpty() ||
		       !ShadowVertexBufferDrawList[ELightPass_Decals].IsEmpty();
	}
