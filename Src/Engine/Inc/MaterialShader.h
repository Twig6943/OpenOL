...
public:
	...
	// Rebind only the sampler-register-dependent parameters from bytecode reflection map.
	// Does NOT require UniformExpressionSet — iterates existing slots by reconstructed name.
	void RebindFromMap(const FShaderParameterMap& ParameterMap, EShaderFrequency Frequency);
	...
	*/
	...
	UINT GetPixelTexture2DBaseIndex(INT Index) const
	{
		if (Index < Uniform2DShaderResourceParameters.Num())
			return (UINT)Uniform2DShaderResourceParameters(Index).ShaderParameter.GetBaseIndex();
		return 0xFFFF;
	}
...
protected:
	...
	/** mathieu.gauthier: To support high-end and low-end skin shading. */
	FShaderParameter SkinDiffuseWeightParameters;
...
public:
	...
	void RebindFromMap(const FShaderParameterMap& ParameterMap);
