...
// Declared separately because FShaderParameterMap must be visible (from ShaderManager.h) at point of use.
extern void RHIBuildShaderParameterMap(const TArray<BYTE>& Code, FShaderParameterMap& OutParameterMap);
extern FString RHIDisassembleShader(const TArray<BYTE>& Code);
