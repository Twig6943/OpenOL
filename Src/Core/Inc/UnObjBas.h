...
#define REN_NoRedirector		(0x0010) // rcharpentier - do not create a redirector (fix for infinite rename/redirector loop when cut&pasting kismet sequences)
...
#if !defined(__cplusplus_cli) || WITH_WINRT	// UObject delete operator cannot be made visible to managed code
	// Made public to allow explicit immediate destruction (DestroyNow pattern in OLBehaviorTree).
public:
...
public:
	...
	{ 
		return UObject::GObjObjects.IsValidIndex(Index) ? &PrivateBooleanHelper::Value : false; 
	}
