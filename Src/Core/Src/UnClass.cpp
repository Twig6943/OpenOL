...
void UClass::Bind()
{
	...
	{
		// If this class lives in an _Assets package, it's a duplicate loaded from a .upk
		// asset file alongside a .u script package. Try to find the real native class in
		// the original package (same name, no _Assets suffix) and borrow its constructor.
		UObject* Outer = GetOuter();
		if ( Outer )
		{
			FString OuterName = Outer->GetName();
			if ( OuterName.EndsWith( TEXT("_Assets") ) )
			{
				FString OrigPkgName = OuterName.Left( OuterName.Len() - 7 ); // strip "_Assets"
				UPackage* OrigPkg = FindObject<UPackage>( NULL, *OrigPkgName );
				if ( OrigPkg )
				{
					UClass* OrigClass = FindObject<UClass>( OrigPkg, *GetName() );
					if ( OrigClass && OrigClass->ClassConstructor )
					{
						ClassConstructor    = OrigClass->ClassConstructor;
						ClassStaticConstructor = OrigClass->ClassStaticConstructor;
						ClassStaticInitializer = OrigClass->ClassStaticInitializer;
						return;
					}
				}
			}
