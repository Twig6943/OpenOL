...
UBOOL UObject::Rename( const TCHAR* InName, UObject* NewOuter, ERenameFlags Flags )
{
	...
	if ( HasAnyFlags(RF_Public) )
	{
		...
		const UBOOL bRedirectionAllowed = !GIsGame && !(Flags & REN_NoRedirector);
...
void UObject::StaticTick( FLOAT DeltaTime )
{
	...
	DWORD ReadSize	= GStatManager.GetStatValueQWORD( STAT_AsyncIO_FulfilledReadSize );
	...
	{
		// @zombie_ps4_begin - ccooper 8/15/2013 - Adding extra [/ FLOAT(ReadCount - PrevioudReadCount)] to account for true FIOS2 async loading. Correct Bandwidth calculations.
		FLOAT Bandwidth = (ReadSize - PreviousReadSize) / (ReadTime - PreviousReadTime) / 1048576.f / FLOAT(ReadCount - PrevioudReadCount);
		// @zombie_ps4_end
