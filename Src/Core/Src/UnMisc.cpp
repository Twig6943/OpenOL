...
UBOOL FMapPackageFileCache::CachePackage( const TCHAR* InPathName, UBOOL InOverrideDupe, UBOOL WarnIfExists )
{
	...
	// If one of the two entries is a .u script package and the other is a .upk asset package,
	// always prefer the .u (mod scripts override the cooked asset package) without warning.
	if ( !InOverrideDupe && ExistingEntry )
	{
		const FString InExt       = FFilename(InPathName).GetExtension().ToLower();
		const FString ExistingExt = FFilename(**ExistingEntry).GetExtension().ToLower();
		if ( InExt == TEXT("u") && ExistingExt == TEXT("upk") )
		{
			// Incoming .u wins — replace silently.
			FileLookup.Set( *PackageName, *FFilename(InPathName) );
			return TRUE;
		}
		if ( InExt == TEXT("upk") && ExistingExt == TEXT("u") )
		{
			// Already have a .u — keep it, ignore the .upk silently.
			return FALSE;
		}
	}
