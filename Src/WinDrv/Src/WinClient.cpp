...
UWindowsClient::UWindowsClient()
{
...
#if !FINAL_RELEASE
	// HACK FOR FRENCH KEYBOARD
	TCHAR buf[KL_NAMELENGTH];
	GetKeyboardLayoutName(buf);
	DWORD dwActiveLangID = 0;
	_stscanf(buf, _T("%X"), &dwActiveLangID);
	//debugf(TEXT("Primary Language: %i"), PRIMARYLANGID(dwActiveLangID));
	//debugf(TEXT("Sub Language: %i"), SUBLANGID(dwActiveLangID));

	if (SUBLANGID(dwActiveLangID) == SUBLANG_ENGLISH_CAN)// This is really French Canadian?
	{
		KeyMapVirtualToName.Set(VK_OEM_3,KEY_Quote);
		KeyMapVirtualToName.Set(VK_OEM_7,KEY_Tilde);
	}
	else
	{
	...
	// rcharpentier - modified the mappings for ps2 controller to make sense with the logitech f710 directinput mode (to be tested with other gamepads)
	/*
	...
	*/
	...
	PS2ToXboxControllerMapping[0] = 2;		// triangle
	PS2ToXboxControllerMapping[1] = 0;		// circle
	PS2ToXboxControllerMapping[2] = 1;		// cross
	PS2ToXboxControllerMapping[3] = 3;		// square
	PS2ToXboxControllerMapping[4] = 4;		// L2
	PS2ToXboxControllerMapping[5] = 5;		// R2
	PS2ToXboxControllerMapping[6] = 10;		// L1 
	PS2ToXboxControllerMapping[7] = 11;		// R1
	PS2ToXboxControllerMapping[8] = 7;		// start
	PS2ToXboxControllerMapping[9] = 6;		// select
	PS2ToXboxControllerMapping[10] = 8;		// Left stick push
	PS2ToXboxControllerMapping[11] = 9;		// Righ stick down
...
// WWISEMODIF_START
	TheClass->EmitObjectReference( STRUCT_OFFSET( UWindowsClient, AkAudioDevice ) );
// WWISEMODIF_END
}
...
// WWISEMODIF_START
		AkAudioDevice = ConstructObject<UAkAudioDevice>( UAkAudioDevice::StaticClass() );
...
// WWISEMODIF_START
	Ar << Engine << AudioDeviceClass << AudioDevice << AkAudioDevice;
// WWISEMODIF_END
}
...
void UWindowsClient::ProcessInput( FLOAT DeltaTime )
{
	...
	if ( PollMouseInput() )
	{
		...
		// Drain DirectInput mouse buffer without forwarding to the game while the ImGui overlay is open.
		if ( OLImGui_IsOverlayVisible() )
		{
			FlushMouseInput();
		}
		else if ( GUseNewMouseInput )
		{
